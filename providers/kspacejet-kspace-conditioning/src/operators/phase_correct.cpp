...
//
// Apply an explicit phase phasor to every ky line of channel-major Cartesian
// k-space. The phase model is indexed [channel, kx].
...
constexpr char kOperatorId[] = "phase_correct";
constexpr char kUnsupportedConfigError[] =
  "K-space-conditioning Provider phase_correct requires canonical "
...
[[nodiscard]] bool expected_static_input_byte_count(const ksj_provider_operator& operator_handle,
                                                    std::uint64_t& byte_count) noexcept {
  return is_valid(operator_handle) &&
         complex_matrix_byte_count(operator_handle.channel_count, operator_handle.cols, byte_count);
}
...
[[nodiscard]] bool transform(const ksj_provider_operator& operator_handle, const void* const dynamic_input,
  ...
  try {
    ...
    const auto cols = static_cast<std::size_t>(operator_handle.cols);
    const auto kspace =
      ksj::array::MatrixView<const Complex>{static_cast<const Complex*>(dynamic_input), channels, samples};
    const auto phase = ksj::array::MatrixView<const Complex>{static_cast<const Complex*>(static_input), channels, cols};
    const auto corrected = ksj::array::MatrixView<Complex>{static_cast<Complex*>(output), channels, samples};
    ksj::array::transform_indexed(kspace, corrected, [phase, cols](const auto& index, const Complex value) {
      return value * phase(index[0U], index[1U] % cols);
    });
    return true;
  } catch (...) {
    KSJ_LOG_ERROR("phase_correct trapped an unexpected exception while transforming k-space");
    return false;
  }
}
...
const ConditioningOperatorImplementation& phase_correct_operator() noexcept {
