...
//
// Provider descriptor construction and lifecycle dispatch. Each numerical
// estimator remains isolated in src/operators/<operator>.cpp.
...
using ::ksj::calibration::operators::coil_compression_basis_estimate_operator;
using ::ksj::calibration::operators::noise_model_estimate_operator;
using ::ksj::calibration::operators::phase_correction_estimate_operator;
using ::ksj::calibration::state::CalibrationOperatorImplementation;
using ::ksj::calibration::support::kInputPort;
using ::ksj::calibration::support::kMaximumCalibrationOutputBytes;
using ::ksj::calibration::support::kOutputPort;
using ::ksj::calibration::support::kRequiredAlignment;
...
[[nodiscard]] const std::array<const CalibrationOperatorImplementation*, 3U>& calibration_operators() noexcept;
...
constexpr char kErrorBadAbi[] = "Calibration Provider received incompatible ABI storage";
constexpr char kErrorInvalidArgument[] = "Calibration Provider received an invalid lifecycle argument";
constexpr char kErrorUnsupportedOperator[] = "Calibration Provider does not expose the requested operator";
constexpr char kErrorLease[] =
  "Calibration Provider requires one input, one output grant, and its exact configured host scratch reservation";
constexpr char kErrorScratch[] = "Calibration Provider received an incompatible caller-owned scratch view";
constexpr char kErrorInput[] = "Calibration Provider received an incompatible calibration payload or byte shape";
constexpr char kErrorOutput[] = "Calibration Provider received an incompatible calibration output grant";
constexpr char kErrorEstimate[] = "Calibration Provider could not estimate the requested calibration artifact";
constexpr char kErrorInternal[] = "Calibration Provider trapped an unexpected internal exception";

[[nodiscard]] ksj_operator_descriptor
make_operator_descriptor(const CalibrationOperatorImplementation& implementation) noexcept {
  ...
  descriptor.max_output_bytes_per_firing = kMaximumCalibrationOutputBytes;
  descriptor.max_scratch_bytes_per_firing = implementation.maximum_scratch_bytes_per_firing;
...
struct ProviderMetadata {
  ProviderMetadata() noexcept
      : operator_descriptors{make_operator_descriptor(*calibration_operators()[0U]),
                             make_operator_descriptor(*calibration_operators()[1U]),
                             make_operator_descriptor(*calibration_operators()[2U])},
...
[[nodiscard]] bool is_operator(const ksj_provider_operator* const operator_handle) noexcept {
  return operator_handle != nullptr && operator_handle->implementation != nullptr &&
         operator_handle->implementation->configure != nullptr &&
         operator_handle->implementation->is_valid != nullptr &&
         operator_handle->implementation->matches_input_type != nullptr &&
         operator_handle->implementation->matches_output_type != nullptr &&
         operator_handle->implementation->output_type != nullptr &&
         operator_handle->implementation->has_compatible_input_byte_count != nullptr &&
         operator_handle->implementation->expected_output_byte_count != nullptr &&
         operator_handle->implementation->required_scratch_bytes != nullptr &&
         operator_handle->implementation->estimate != nullptr &&
...
const std::array<const CalibrationOperatorImplementation*, 3U>& calibration_operators() noexcept {
  static const std::array<const CalibrationOperatorImplementation*, 3U> implementations{
    &noise_model_estimate_operator(),
    &phase_correction_estimate_operator(),
    &coil_compression_basis_estimate_operator(),
  };
  return implementations;
}

const CalibrationOperatorImplementation* find_calibration_operator(const ksj_utf8_view& id) noexcept {
  for (const auto* const implementation : calibration_operators()) {
...
ksj_status KSJ_PROVIDER_CALL operator_create(const ksj_operator_create_request* const request,
  ...
  try {
    ...
    *out_operator = nullptr;
    const auto* const implementation = find_calibration_operator(request->operator_id);
    ...
    *out_operator = operator_handle;
    ...
    KSJ_LOG_ERROR("Calibration Provider trapped an unexpected exception while creating an Operator");
    return reject(out_error, KSJ_STATUS_INTERNAL_ERROR, kErrorInternal, sizeof(kErrorInternal) - 1U);
  }
}
...
ksj_status KSJ_PROVIDER_CALL operator_process_batch(ksj_provider_operator* const operator_handle,
  ...
  try {
    ...
    std::uint64_t expected_scratch_byte_count = 0U;
    if (info_status != KSJ_STATUS_OK || !has_full_compatible_header(&info) || info.input_batch_count != 1U ||
        info.output_grant_count != 1U ||
        !operator_handle->implementation->required_scratch_bytes(*operator_handle, expected_scratch_byte_count) ||
        expected_scratch_byte_count > operator_handle->implementation->maximum_scratch_bytes_per_firing ||
        info.reserved_scratch_bytes != expected_scratch_byte_count) {
    ...
    if (expected_scratch_byte_count != 0U) {
      if ((callbacks->abi.capability_bits & KSJ_LEASE_CAP_SCRATCH) == 0U || callbacks->get_scratch == nullptr) {
      ...
      if (!has_full_compatible_header(&scratch) || scratch.data == nullptr ||
          scratch.byte_count != expected_scratch_byte_count ||
    ...
    if (!has_full_compatible_header(&batch) || batch.input_port != kInputPort || batch.item_count != 1U ||
      ...
      return reject(out_error, KSJ_STATUS_CONTRACT_VIOLATION, kErrorInput, sizeof(kErrorInput) - 1U);
    }
    ...
    if (!has_full_compatible_header(&input.payload) || input.payload.data == nullptr ||
        ...
        !valid_borrowed_bytes(input.metadata) || input.metadata.size != 0U ||
        !operator_handle->implementation->matches_input_type(input.payload.type) ||
        !operator_handle->implementation->has_compatible_input_byte_count(*operator_handle, input.payload.byte_count)) {
      return reject(out_error, KSJ_STATUS_CONTRACT_VIOLATION, kErrorInput, sizeof(kErrorInput) - 1U);
    }

    std::uint64_t expected_output_byte_count = 0U;
    if (!operator_handle->implementation->expected_output_byte_count(*operator_handle, expected_output_byte_count) ||
        info.reserved_output_bytes < expected_output_byte_count) {
    ...
    if (!has_full_compatible_header(&output) || output.data == nullptr ||
        output.capacity_bytes < expected_output_byte_count || output.committed_bytes != 0U ||
        ...
        !operator_handle->implementation->matches_output_type(output.type)) {
      return reject(out_error, KSJ_STATUS_CONTRACT_VIOLATION, kErrorOutput, sizeof(kErrorOutput) - 1U);
    }

    if (!operator_handle->implementation->estimate(*operator_handle, input.payload.data, input.payload.byte_count,
                                                   output.data, expected_output_byte_count, scratch_data,
                                                   expected_scratch_byte_count)) {
      return reject(out_error, KSJ_STATUS_CONTRACT_VIOLATION, kErrorEstimate, sizeof(kErrorEstimate) - 1U);
    }
    ...
    seal.produced_byte_count = expected_output_byte_count;
    seal.semantic_key_hash = input.semantic_key_hash;
    seal.order_key = input.order_key;
    seal.type = operator_handle->implementation->output_type();
    ...
    KSJ_LOG_ERROR("Calibration Provider trapped an unexpected exception while processing a firing");
    return reject(out_error, KSJ_STATUS_INTERNAL_ERROR, kErrorInternal, sizeof(kErrorInternal) - 1U);
  }
}
...
ksj_status provider_query(const ksj_provider_query_request* const request,
  ...
  try {
    ...
    *out_api = {};
    ...
    KSJ_LOG_ERROR("Calibration Provider trapped an unexpected exception while answering provider query");
    return reject(out_error, KSJ_STATUS_INTERNAL_ERROR, kErrorInternal, sizeof(kErrorInternal) - 1U);
  }
}
