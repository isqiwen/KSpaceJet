...
#include "image_offset_float32.hpp"
...
constexpr char kOperatorId[] = "image_offset_float32";
constexpr char kUnsupportedConfigError[] = "Image-ops Provider requires canonical {\"offset\":<finite-decimal>}";

[[nodiscard]] bool configure(const ksj_byte_view& config, ksj_provider_operator& operator_handle) noexcept {
  float offset = 0.0F;
  if (!parse_single_finite_decimal_config(config, "{\"offset\":", offset)) {
    return false;
  }
  operator_handle.scalar = offset;
  return true;
}
...
const ImageOperatorImplementation& image_offset_float32_operator() noexcept {
