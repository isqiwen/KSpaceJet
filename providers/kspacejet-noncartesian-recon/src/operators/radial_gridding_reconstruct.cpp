...
//
// Bounded two-dimensional radial linear-gridding reconstruction with explicit
// analytic-ramp density compensation. This development-only Operator does not
// correct a trajectory or estimate coil sensitivities.
...
constexpr char kOperatorId[] = "radial_gridding_reconstruct";
...
constexpr char kErrorUnsupportedConfig[] =
  "Non-Cartesian reconstruction Provider radial_gridding_reconstruct requires canonical "
  "{\"channels\":N,\"density_compensation\":\"radial_analytic_ramp\",\"image_cols\":N,\"image_rows\":N,"
  "\"sample_count\":N,\"trajectory_units\":\"radians_per_pixel\"} with power-of-two image dimensions "
  "within its bounded radial-gridding envelope";
constexpr char kErrorLease[] =
  "Non-Cartesian radial gridding requires two input batches, one output grant, and bounded caller-owned gridding "
  "workspace";
constexpr char kErrorKspace[] =
  "Non-Cartesian radial gridding requires one exact ksj.noncartesian-kspace-frame on input port 0";
constexpr char kErrorTrajectory[] =
  "Non-Cartesian radial gridding requires one exact finite ksj.trajectory-frame on input port 1 in radians_per_pixel "
  "within [-pi, pi]";
constexpr char kErrorIdentity[] =
  "Non-Cartesian radial k-space and trajectory inputs must have equal semantic_key_hash and order_key";
constexpr char kErrorOutput[] = "Non-Cartesian radial gridding requires one exact ksj.coil-image-frame output grant";
constexpr char kErrorScratch[] = "Non-Cartesian radial gridding scratch grant is too small or incompatible";
constexpr char kErrorInternal[] = "Non-Cartesian radial gridding trapped an unexpected internal exception";
...
constexpr std::uint64_t kMaximumScratchBytes =
  (2U * kMaximumPixels + 2U * kMaximumDimension) * sizeof(Complex) + kMaximumSamples * sizeof(float);
...
[[nodiscard]] bool parse_canonical_config(const ksj_byte_view& config, std::uint32_t& channels,
  ...
  constexpr std::string_view kDensityCompensation{",\"density_compensation\":\"radial_analytic_ramp\""};
  ...
  constexpr std::string_view kTrajectoryUnits{",\"trajectory_units\":\"radians_per_pixel\""};
  ...
  const auto density_marker = encoded.find(kDensityCompensation, kPrefix.size());
  const auto cols_marker = density_marker == std::string_view::npos
                             ? std::string_view::npos
                             : encoded.find(kImageCols, density_marker + kDensityCompensation.size());
  ...
  const auto units_marker = samples_marker == std::string_view::npos
                              ? std::string_view::npos
                              : encoded.find(kTrajectoryUnits, samples_marker + kSampleCount.size());
  if (density_marker == std::string_view::npos || cols_marker == std::string_view::npos ||
      rows_marker == std::string_view::npos || samples_marker == std::string_view::npos ||
      units_marker == std::string_view::npos ||
      units_marker + kTrajectoryUnits.size() != encoded.size() - kSuffix.size()) {
    return false;
  }
  const char* const base = encoded.data();
  return parse_positive_bounded(base + kPrefix.size(), base + density_marker, 1U, kMaximumChannels, channels) &&
         ...
         parse_positive_bounded(base + rows_marker + kImageRows.size(), base + samples_marker, 2U, kMaximumDimension,
                                image_rows) &&
         parse_positive_bounded(base + samples_marker + kSampleCount.size(), base + units_marker, 1U, kMaximumSamples,
                                sample_count);
}
...
[[nodiscard]] bool configure(const ksj_byte_view& config, ksj_provider_operator& operator_handle) noexcept {
  ...
  if (!std::has_single_bit(image_rows) || !std::has_single_bit(image_cols)) {
    return false;
  }
...
[[nodiscard]] bool shape_bytes(const ksj_provider_operator& operator_handle, std::uint64_t& pixel_count,
  ...
  if (operator_handle.channels == 0U || operator_handle.channels > kMaximumChannels ||
      ...
      !std::has_single_bit(operator_handle.image_rows) || !std::has_single_bit(operator_handle.image_cols) ||
  ...
  const auto transform_length = static_cast<std::uint64_t>(
    operator_handle.image_rows > operator_handle.image_cols ? operator_handle.image_rows : operator_handle.image_cols);
  scratch_bytes = (2U * pixel_count + 2U * transform_length) * sizeof(Complex) +
                  static_cast<std::uint64_t>(operator_handle.sample_count) * sizeof(float);
  return pixel_count <= kMaximumPixels && kspace_bytes <= kMaximumKspaceBytes &&
...
template <typename T> struct ScratchObjects {
  T* data{nullptr};
  std::size_t count{0U};

  ScratchObjects(T* const storage, const std::size_t element_count) : data(storage), count(element_count) {
    for (std::size_t index = 0U; index < count; ++index) {
      std::construct_at(data + index);
    }
  }

  ScratchObjects(const ScratchObjects&) = delete;
  ScratchObjects& operator=(const ScratchObjects&) = delete;

  ~ScratchObjects() { std::destroy_n(data, count); }
};

[[nodiscard]] bool is_radians_per_pixel_trajectory(const float* const values,
  ...
  for (std::uint64_t index = 0U; index < value_count; ++index) {
    if (!std::isfinite(values[index]) || values[index] < -kPi || values[index] > kPi) {
      return false;
    }
  }
  return true;
...
ksj_status process(ksj_provider_operator& operator_handle, ksj_firing_lease* const lease,
  ...
  try {
    ...
    if (!is_radians_per_pixel_trajectory(trajectory_values, trajectory_value_count)) {
      return reject(out_error, KSJ_STATUS_CONTRACT_VIOLATION, kErrorTrajectory, sizeof(kErrorTrajectory) - 1U);
    }
    ...
    if (!has_full_compatible_header(&scratch) || scratch.data == nullptr || scratch.byte_count < scratch_bytes ||
        ...
        !has_usable_host_memory(scratch.data, scratch.alignment, kRequiredAlignment)) {
    ...
    const auto sample_count = static_cast<std::size_t>(operator_handle.sample_count);
    const auto rows = static_cast<std::size_t>(operator_handle.image_rows);
    const auto cols = static_cast<std::size_t>(operator_handle.image_cols);
    const auto pixel_count_size = static_cast<std::size_t>(pixel_count);
    const auto transform_length = rows > cols ? rows : cols;
    auto* const scratch_bytes_begin = static_cast<std::byte*>(scratch.data);
    auto* const image_plane = reinterpret_cast<Complex*>(scratch_bytes_begin);
    auto* const fft_intermediate = image_plane + pixel_count_size;
    auto* const fft_source = fft_intermediate + pixel_count_size;
    auto* const fft_destination = fft_source + transform_length;
    auto* const density_compensation = reinterpret_cast<float*>(fft_destination + transform_length);
    ScratchObjects<Complex> image_plane_objects{image_plane, pixel_count_size};
    ScratchObjects<Complex> fft_intermediate_objects{fft_intermediate, pixel_count_size};
    ScratchObjects<Complex> fft_source_objects{fft_source, transform_length};
    ScratchObjects<Complex> fft_destination_objects{fft_destination, transform_length};
    ScratchObjects<float> density_compensation_objects{density_compensation, sample_count};

    const auto trajectory_view = ksj::array::MatrixView<const float>{trajectory_values, sample_count, 2U};
    ...
    const auto workspace = ksj::nufft::RadialGridding2Workspace<float>{
      .fft_intermediate = ksj::array::MatrixView<Complex>{fft_intermediate, rows, cols},
      .fft_source = ksj::array::VectorView<Complex>{fft_source, transform_length},
      .fft_destination = ksj::array::VectorView<Complex>{fft_destination, transform_length},
    };
    const auto dcf_view = ksj::array::VectorView<float>{density_compensation, sample_count};
    ksj::nufft::radial_analytic_ramp_dcf2(trajectory_view, dcf_view);
    ...
    const auto image = ksj::array::MatrixView<Complex>{image_plane, rows, cols};
    for (std::uint32_t channel = 0U; channel < operator_handle.channels; ++channel) {
      const auto samples = ksj::array::VectorView<const Complex>{
        kspace_values + static_cast<std::uint64_t>(channel) * operator_handle.sample_count, sample_count};
      ksj::nufft::radial_linear_gridding2_adjoint(grid, samples, trajectory_view, ksj::array::as_const_view(dcf_view),
                                                  image, workspace);
    ...
    KSJ_LOG_ERROR("radial_gridding_reconstruct trapped an unexpected exception while processing a firing");
...
const NonCartesianReconOperatorImplementation& radial_gridding_reconstruct_operator() noexcept {
