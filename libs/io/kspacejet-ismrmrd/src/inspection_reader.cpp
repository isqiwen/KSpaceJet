...
[[nodiscard]] bool validate_limits(const InspectionReadLimits& limits, std::string& error) {
  if (limits.max_xml_header_bytes == 0U || limits.max_acquisition_count == 0U ||
      ...
      limits.max_hdf5_object_attributes == 0U || limits.max_hdf5_attribute_rank == 0U ||
      limits.max_hdf5_attribute_elements == 0U || limits.max_hdf5_attribute_name_bytes == 0U ||
      limits.max_hdf5_attribute_value_bytes == 0U || limits.max_hdf5_attribute_preview_bytes == 0U ||
...
struct AttributeTypeDescription {
  std::string type_name;
  AttributeValueKind value_kind{AttributeValueKind::unsupported};
  std::size_t element_bytes{0U};
};

[[nodiscard]] AttributeTypeDescription describe_hdf5_attribute_type(const hid_t file_type) {
  const auto type_class = H5Tget_class(file_type);
  if (type_class == H5T_INTEGER) {
    H5Handle native_type{H5Tget_native_type(file_type, H5T_DIR_ASCEND), H5Tclose};
    if (!native_type.valid()) {
      return {.type_name = "integer", .value_kind = AttributeValueKind::unsupported};
    }
    const auto bytes = H5Tget_size(native_type.get());
    const auto sign = H5Tget_sign(native_type.get());
    const auto bits = bytes * std::numeric_limits<unsigned char>::digits;
    if (sign == H5T_SGN_NONE) {
      return {.type_name = std::to_string(bits) + "-bit unsigned integer",
              .value_kind = AttributeValueKind::unsigned_integer,
              .element_bytes = bytes};
    }
    if (sign == H5T_SGN_2) {
      return {.type_name = std::to_string(bits) + "-bit signed integer",
              .value_kind = AttributeValueKind::signed_integer,
              .element_bytes = bytes};
    }
    return {.type_name = std::to_string(bits) + "-bit integer", .value_kind = AttributeValueKind::unsupported};
  }
  if (type_class == H5T_FLOAT) {
    H5Handle native_type{H5Tget_native_type(file_type, H5T_DIR_ASCEND), H5Tclose};
    if (!native_type.valid()) {
      return {.type_name = "floating-point", .value_kind = AttributeValueKind::unsupported};
    }
    const auto bytes = H5Tget_size(native_type.get());
    const auto bits = bytes * std::numeric_limits<unsigned char>::digits;
    if (bytes == sizeof(float) || bytes == sizeof(double)) {
      return {.type_name = std::to_string(bits) + "-bit floating-point",
              .value_kind = AttributeValueKind::real,
              .element_bytes = bytes};
    }
    return {.type_name = std::to_string(bits) + "-bit floating-point", .value_kind = AttributeValueKind::unsupported};
  }
  if (type_class == H5T_STRING) {
    if (H5Tis_variable_str(file_type) > 0) {
      return {.type_name = "variable-length string", .value_kind = AttributeValueKind::unsupported};
    }
    const auto bytes = H5Tget_size(file_type);
    return {.type_name = "string, length = " + std::to_string(bytes),
            .value_kind = AttributeValueKind::fixed_string,
            .element_bytes = bytes};
  }
  switch (type_class) {
    case H5T_BITFIELD:
      return {.type_name = "bitfield"};
    case H5T_OPAQUE:
      return {.type_name = "opaque"};
    case H5T_COMPOUND:
      return {.type_name = "compound"};
    case H5T_REFERENCE:
      return {.type_name = "reference"};
    case H5T_ENUM:
      return {.type_name = "enumeration"};
    case H5T_VLEN:
      return {.type_name = "variable-length"};
    case H5T_ARRAY:
    ...
    case H5T_NO_CLASS:
    case H5T_NCLASSES:
      return {.type_name = "unsupported HDF5 type"};
    case H5T_INTEGER:
    case H5T_FLOAT:
    case H5T_STRING:
      break;
...
[[nodiscard]] bool describe_hdf5_attribute_shape(const hid_t dataspace, const InspectionReadLimits& limits,
                                                 InspectionObjectAttributeDescriptor& descriptor, std::string& error) {
  descriptor.dimensions.clear();
  descriptor.element_count = 0U;
  const auto space_class = H5Sget_simple_extent_type(dataspace);
  if (space_class == H5S_SCALAR) {
    descriptor.element_count = 1U;
    return true;
  }
  if (space_class == H5S_NULL) {
    descriptor.dimensions.push_back(0U);
    return true;
  }
  if (space_class != H5S_SIMPLE) {
    error = "HDF5 object attribute has an unsupported dataspace.";
    return false;
  }
  const auto rank = H5Sget_simple_extent_ndims(dataspace);
  if (rank < 0 || static_cast<std::uint32_t>(rank) > limits.max_hdf5_attribute_rank) {
    error = "HDF5 object attribute rank exceeds inspection limit.";
    return false;
  }
  std::vector<hsize_t> native_dimensions(static_cast<std::size_t>(rank));
  if (rank > 0 && H5Sget_simple_extent_dims(dataspace, native_dimensions.data(), nullptr) < 0) {
    error = "HDF5 object attribute dataspace could not be read.";
    return false;
  }
  std::uint64_t element_count = 1U;
  descriptor.dimensions.reserve(native_dimensions.size());
  for (const auto dimension : native_dimensions) {
    if (dimension > std::numeric_limits<std::uint64_t>::max()) {
      error = "HDF5 object attribute dimensions exceed inspection limit.";
      return false;
    }
    const auto converted = static_cast<std::uint64_t>(dimension);
    descriptor.dimensions.push_back(converted);
    if (converted != 0U && element_count > std::numeric_limits<std::uint64_t>::max() / converted) {
      error = "HDF5 object attribute element count overflows.";
      return false;
    }
    element_count *= converted;
  }
  descriptor.element_count = element_count;
  return true;
}

[[nodiscard]] bool can_materialize_attribute_preview(const InspectionObjectAttributeDescriptor& descriptor,
                                                     const AttributeTypeDescription& type,
                                                     const InspectionReadLimits& limits, std::size_t& byte_count) {
  byte_count = 0U;
  return descriptor.element_count <= limits.max_hdf5_attribute_elements &&
         descriptor.element_count <= std::numeric_limits<std::size_t>::max() &&
         checked_multiply(static_cast<std::size_t>(descriptor.element_count), type.element_bytes, byte_count) &&
         byte_count <= limits.max_hdf5_attribute_value_bytes;
}

void mark_attribute_preview_omitted(InspectionObjectAttributeDescriptor& descriptor) {
  descriptor.value_preview = "preview omitted: inspection limit";
  descriptor.value_preview_state = InspectionAttributeValuePreviewState::truncated;
}

void append_attribute_preview(std::string& preview, const std::string_view fragment, const std::size_t limit,
                              bool& truncated) {
  if (preview.size() >= limit) {
    truncated = true;
    return;
  }
  const auto remaining = limit - preview.size();
  const auto copied = std::min(remaining, fragment.size());
  preview.append(fragment.substr(0U, copied));
  truncated = truncated || copied != fragment.size();
}

template <typename Value>
[[nodiscard]] bool read_numeric_attribute_preview(const H5Handle& attribute, const hid_t memory_type,
                                                  const InspectionReadLimits& limits,
                                                  InspectionObjectAttributeDescriptor& descriptor, std::string& error) {
  std::size_t byte_count = 0U;
  const AttributeTypeDescription type{.element_bytes = sizeof(Value)};
  if (!can_materialize_attribute_preview(descriptor, type, limits, byte_count)) {
    mark_attribute_preview_omitted(descriptor);
    return true;
  }
  std::vector<Value> values(static_cast<std::size_t>(descriptor.element_count));
  if (!values.empty() && H5Aread(attribute.get(), memory_type, values.data()) < 0) {
    error = "HDF5 object attribute value could not be read.";
    return false;
  }

  bool truncated = false;
  std::string preview;
  preview.reserve(std::min(limits.max_hdf5_attribute_preview_bytes, byte_count));
  if (values.size() != 1U) {
    append_attribute_preview(preview, "[", limits.max_hdf5_attribute_preview_bytes, truncated);
  }
  for (std::size_t index = 0U; index < values.size(); ++index) {
    if (index != 0U) {
      append_attribute_preview(preview, ", ", limits.max_hdf5_attribute_preview_bytes, truncated);
    }
    append_attribute_preview(preview, std::to_string(values[index]), limits.max_hdf5_attribute_preview_bytes,
                             truncated);
  }
  if (values.size() != 1U) {
    append_attribute_preview(preview, "]", limits.max_hdf5_attribute_preview_bytes, truncated);
  }
  descriptor.value_preview = std::move(preview);
  descriptor.value_preview_state =
    truncated ? InspectionAttributeValuePreviewState::truncated : InspectionAttributeValuePreviewState::available;
  return true;
}

[[nodiscard]] bool read_fixed_string_attribute_preview(const H5Handle& attribute, const H5Handle& file_type,
                                                       const InspectionReadLimits& limits,
                                                       InspectionObjectAttributeDescriptor& descriptor,
                                                       std::string& error) {
  const auto string_bytes = H5Tget_size(file_type.get());
  const AttributeTypeDescription type{.element_bytes = string_bytes};
  std::size_t byte_count = 0U;
  if (string_bytes == 0U) {
    descriptor.value_preview_state = InspectionAttributeValuePreviewState::unsupported;
    return true;
  }
  if (!can_materialize_attribute_preview(descriptor, type, limits, byte_count)) {
    mark_attribute_preview_omitted(descriptor);
  ...
  std::vector<char> values(byte_count);
  if (!values.empty() && H5Aread(attribute.get(), file_type.get(), values.data()) < 0) {
    error = "HDF5 object attribute value could not be read.";
    return false;
  }
  ...
  std::string preview;
  preview.reserve(std::min(limits.max_hdf5_attribute_preview_bytes, byte_count));
  if (descriptor.element_count != 1U) {
    append_attribute_preview(preview, "[", limits.max_hdf5_attribute_preview_bytes, truncated);
  }
  for (std::size_t index = 0U; index < static_cast<std::size_t>(descriptor.element_count); ++index) {
    if (index != 0U) {
      append_attribute_preview(preview, ", ", limits.max_hdf5_attribute_preview_bytes, truncated);
    }
    const auto offset = index * string_bytes;
    const auto* begin = values.data() + offset;
    const auto* end = begin + string_bytes;
    const auto null_terminator = std::find(begin, end, '\0');
    append_attribute_preview(preview, std::string_view(begin, static_cast<std::size_t>(null_terminator - begin)),
                             limits.max_hdf5_attribute_preview_bytes, truncated);
  }
  if (descriptor.element_count != 1U) {
    append_attribute_preview(preview, "]", limits.max_hdf5_attribute_preview_bytes, truncated);
  }
...
[[nodiscard]] bool read_hdf5_attribute_preview(const H5Handle& attribute, const H5Handle& file_type,
                                               const AttributeTypeDescription& type, const InspectionReadLimits& limits,
                                               InspectionObjectAttributeDescriptor& descriptor, std::string& error) {
  switch (type.value_kind) {
    case AttributeValueKind::unsigned_integer:
      switch (type.element_bytes) {
        case sizeof(std::uint8_t):
          return read_numeric_attribute_preview<std::uint8_t>(attribute, H5T_NATIVE_UINT8, limits, descriptor, error);
        case sizeof(std::uint16_t):
          return read_numeric_attribute_preview<std::uint16_t>(attribute, H5T_NATIVE_UINT16, limits, descriptor, error);
        case sizeof(std::uint32_t):
          return read_numeric_attribute_preview<std::uint32_t>(attribute, H5T_NATIVE_UINT32, limits, descriptor, error);
        case sizeof(std::uint64_t):
          return read_numeric_attribute_preview<std::uint64_t>(attribute, H5T_NATIVE_UINT64, limits, descriptor, error);
        default:
          return true;
      }
    case AttributeValueKind::signed_integer:
      switch (type.element_bytes) {
        case sizeof(std::int8_t):
          return read_numeric_attribute_preview<std::int8_t>(attribute, H5T_NATIVE_INT8, limits, descriptor, error);
        case sizeof(std::int16_t):
          return read_numeric_attribute_preview<std::int16_t>(attribute, H5T_NATIVE_INT16, limits, descriptor, error);
        case sizeof(std::int32_t):
          return read_numeric_attribute_preview<std::int32_t>(attribute, H5T_NATIVE_INT32, limits, descriptor, error);
        case sizeof(std::int64_t):
          return read_numeric_attribute_preview<std::int64_t>(attribute, H5T_NATIVE_INT64, limits, descriptor, error);
        default:
          return true;
      }
    case AttributeValueKind::real:
      if (type.element_bytes == sizeof(float)) {
        return read_numeric_attribute_preview<float>(attribute, H5T_NATIVE_FLOAT, limits, descriptor, error);
      }
      if (type.element_bytes == sizeof(double)) {
        return read_numeric_attribute_preview<double>(attribute, H5T_NATIVE_DOUBLE, limits, descriptor, error);
      }
      return true;
    case AttributeValueKind::fixed_string:
      return read_fixed_string_attribute_preview(attribute, file_type, limits, descriptor, error);
    case AttributeValueKind::unsupported:
      return true;
  }
  return true;
}

[[nodiscard]] bool resolve_hdf5_attribute_object_path(const hid_t file_id, const InspectionDatasetMetadata& metadata,
                                                      const std::string& image_series_parent_group,
                                                      const bool standalone_image_series,
                                                      const InspectionObjectLocator& object, std::string& path,
                                                      std::string& error) {
  path.clear();
  switch (object.kind) {
    case InspectionObjectKind::container:
      path = metadata.group;
      break;
    case InspectionObjectKind::xml:
      if (standalone_image_series) {
        error = "A standalone ISMRMRD image series has no XML header object.";
        return false;
      }
      path = make_dataset_path(metadata.group, "xml");
      break;
    case InspectionObjectKind::acquisitions:
      if (standalone_image_series) {
        error = "A standalone ISMRMRD image series has no acquisition object.";
        return false;
      }
      path = make_dataset_path(metadata.group, "data");
      break;
    case InspectionObjectKind::waveforms:
      if (standalone_image_series) {
        error = "A standalone ISMRMRD image series has no waveform object.";
        return false;
      }
      if (!has_standard_waveforms(file_id, metadata.group)) {
        error = "The selected ISMRMRD container has no standard waveform object.";
        return false;
      }
      path = make_dataset_path(metadata.group, "waveforms");
      break;
    case InspectionObjectKind::image_series:
      {
        const auto found =
          std::find_if(metadata.image_series.begin(), metadata.image_series.end(), [&object](const auto& series) {
            return series.series_id == object.image_series_id;
          });
        if (object.image_series_id.empty() || found == metadata.image_series.end()) {
          error = "ISMRMRD image series was not found.";
          return false;
        }
        path = make_dataset_path(image_series_parent_group, object.image_series_id);
        break;
      }
  }

  bool exists = false;
  if (!h5_link_exists(file_id, path, exists) || !exists) {
    error = "ISMRMRD semantic object is unavailable for HDF5 attribute inspection.";
    return false;
  }
  return true;
}

[[nodiscard]] bool read_hdf5_object_attributes(const hid_t file_id, const std::string& path,
                                               const InspectionReadLimits& limits,
                                               std::vector<InspectionObjectAttributeDescriptor>& attributes,
                                               std::string& error) {
  attributes.clear();
  H5Handle object{H5Oopen(file_id, path.c_str(), H5P_DEFAULT), H5Oclose};
  if (!object.valid()) {
    error = "ISMRMRD semantic object could not be opened for HDF5 attribute inspection.";
    return false;
  }
  const auto attribute_count = H5Aget_num_attrs(object.get());
  if (attribute_count < 0) {
    error = "HDF5 object attributes could not be enumerated.";
    return false;
  }
  if (static_cast<std::uint32_t>(attribute_count) > limits.max_hdf5_object_attributes) {
    error = "HDF5 object attribute count exceeds inspection limit.";
    return false;
  }

  std::vector<InspectionObjectAttributeDescriptor> next;
  next.reserve(static_cast<std::size_t>(attribute_count));
  for (int index = 0; index < attribute_count; ++index) {
    H5Handle attribute{H5Aopen_by_idx(file_id, path.c_str(), H5_INDEX_NAME, H5_ITER_INC, static_cast<hsize_t>(index),
                                      H5P_DEFAULT, H5P_DEFAULT),
                       H5Aclose};
    H5Handle file_type{attribute.valid() ? H5Aget_type(attribute.get()) : H5I_INVALID_HID, H5Tclose};
    H5Handle dataspace{attribute.valid() ? H5Aget_space(attribute.get()) : H5I_INVALID_HID, H5Sclose};
    if (!attribute.valid() || !file_type.valid() || !dataspace.valid()) {
      error = "HDF5 object attribute could not be opened.";
      return false;
    }
    const auto name_length = H5Aget_name(attribute.get(), 0U, nullptr);
    if (name_length < 0 || static_cast<std::size_t>(name_length) > limits.max_hdf5_attribute_name_bytes) {
      error = "HDF5 object attribute name exceeds inspection limit.";
      return false;
    }
    std::string name(static_cast<std::size_t>(name_length) + 1U, '\0');
    if (H5Aget_name(attribute.get(), name.size(), name.data()) < 0) {
      error = "HDF5 object attribute name could not be read.";
      return false;
    }
    name.resize(static_cast<std::size_t>(name_length));
    ...
    const auto type = describe_hdf5_attribute_type(file_type.get());
    descriptor.type_name = type.type_name;
    if (!describe_hdf5_attribute_shape(dataspace.get(), limits, descriptor, error) ||
        !read_hdf5_attribute_preview(attribute, file_type, type, limits, descriptor, error)) {
      return false;
    }
...
bool InspectionReader::read_object_attributes(const InspectionObjectLocator& object,
                                              std::vector<InspectionObjectAttributeDescriptor>& attributes,
                                              std::string& error) {
  error.clear();
  attributes.clear();
  ...
  std::string path;
  if (!resolve_hdf5_attribute_object_path(impl->dataset->hdf5_file_id(), metadata, impl->image_series_parent_group,
                                          impl->standalone_image_series, object, path, error)) {
    return false;
  }
  std::vector<InspectionObjectAttributeDescriptor> next;
  if (!read_hdf5_object_attributes(impl->dataset->hdf5_file_id(), path, limits, next, error)) {
    return false;
  }
  attributes = std::move(next);
  return true;
