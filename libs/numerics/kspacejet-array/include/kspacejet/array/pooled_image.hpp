...
/// Owning two-dimensional row-major image storage with image-specific View and subview accessors.
...
template <typename T> class PooledImage final : private detail::PooledDense2D<T> {
  using dense_base_type = detail::PooledDense2D<T>;

public:
  using value_type = typename dense_base_type::value_type;
  using view_type = ImageView<T>;
  using const_view_type = ImageView<const T>;
  ...
  [[nodiscard]] static PooledImage constant(std::size_t rows, std::size_t cols, const Value& value,
                                            ksj::memory::AllocationProperties properties = {}) {
    auto output = PooledImage(rows, cols, std::move(properties));
    output.set_constant(value);
    return output;
  }

  [[nodiscard]] static PooledImage zeros(std::size_t rows, std::size_t cols,
                                         ksj::memory::AllocationProperties properties = {}) {
    return constant(rows, cols, T{}, std::move(properties));
  }

  [[nodiscard]] static PooledImage ones(std::size_t rows, std::size_t cols,
                                        ksj::memory::AllocationProperties properties = {}) {
    return constant(rows, cols, T{1}, std::move(properties));
  }

  template <typename StartT, typename StopT>
  [[nodiscard]] static PooledImage linspace(std::size_t rows, std::size_t cols, const StartT& start, const StopT& stop,
                                            ksj::memory::AllocationProperties properties = {}) {
    auto output = PooledImage(rows, cols, std::move(properties));
  ...
  [[nodiscard]] static PooledImage uniform_random(std::size_t rows, std::size_t cols, const LowerT& lower,
  ...
  PooledImage(PooledImage&&) noexcept = default;
  PooledImage& operator=(PooledImage&&) noexcept = default;

  void swap(PooledImage& other) noexcept { dense_base_type::swap(other); }
  ...
  [[nodiscard]] VectorView<T> reshape_view(std::size_t element_count) {
    detail::validate_reshape_count(size(), element_count, "PooledImage reshape_view cannot change the element count");
  ...
  [[nodiscard]] view_type reshape_view(std::size_t new_rows, std::size_t new_cols) {
    ...
    detail::validate_reshape_count(size(), count, "PooledImage reshape_view cannot change the element count");
  ...
  template <typename Value> PooledImage& fill(const Value& value) {
    ksj::array::fill(view(), value);
    return *this;
  }

  template <typename Value> PooledImage& set_constant(const Value& value) { return fill(value); }

  PooledImage& set_zero() { return fill(T{}); }
  PooledImage& set_ones() { return fill(T{1}); }

  template <typename StartT, typename StopT> PooledImage& set_linspace(const StartT& start, const StopT& stop) {
    ksj::array::fill_linspace(view(), start, stop);
    return *this;
  }

  template <typename LowerT, typename UpperT, typename UniformRandomBitGenerator>
  PooledImage& set_uniform_random(const LowerT& lower, const UpperT& upper, UniformRandomBitGenerator& generator) {
    ksj::array::fill_uniform_random(view(), lower, upper, generator);
    return *this;
  }

  template <typename LowerT, typename UpperT>
  PooledImage& set_uniform_random(const LowerT& lower, const UpperT& upper) {
    ksj::array::fill_uniform_random(view(), lower, upper);
    return *this;
  }

  template <typename InputT> PooledImage& assign(ImageView<InputT> input) {
    resize(input.rows(), input.cols());
    copy(input, view());
    return *this;
  }

  template <typename InputT> PooledImage& assign(const PooledImage<InputT>& input) { return assign(input.view()); }

  template <typename UnaryFunction> PooledImage& transform_in_place(UnaryFunction&& function) {
    ...
    requires(is_complex_v<T>)
  {
    ...
    auto output = PooledImage<output_type>(rows(), cols());
  ...
  [[nodiscard]] auto conjugate() const
    requires(is_complex_v<T>)
  {
    auto output = PooledImage<T>(rows(), cols());
  ...
  using dense_base_type::width;
};

template <typename T> void swap(PooledImage<T>& lhs, PooledImage<T>& rhs) noexcept {
  lhs.swap(rhs);
}

template <typename T>
[[nodiscard]] PooledImage<T> make_pooled_image(std::size_t rows, std::size_t cols,
                                               ksj::memory::AllocationProperties properties = {}) {
  return PooledImage<T>(rows, cols, std::move(properties));
}

template <typename T>
[[nodiscard]] PooledImage<std::remove_const_t<T>> make_pooled_image(ImageView<T> input,
                                                                    ksj::memory::AllocationProperties properties = {}) {
  using value_type = std::remove_const_t<T>;
  auto output = make_pooled_image<value_type>(input.rows(), input.cols(), std::move(properties));
  copy(input, output.view());
  return output;
}

template <typename T>
[[nodiscard]] PooledImage<T> make_pooled_image(const PooledImage<T>& input,
                                               ksj::memory::AllocationProperties properties = {}) {
  return make_pooled_image(input.view(), std::move(properties));
}
