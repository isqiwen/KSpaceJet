...
Result<RunRecord> RunRecord::create(const RunRecordSpec& specification) {
  if (specification.schema_version != kRunRecordSchemaVersion) {
    return validation("RunRecord schema_version must be '" + std::string(kRunRecordSchemaVersion) + "'.");
  }
  ...
  if (specification.secondary_causes.size() > kMaxRunSecondaryCauses) {
    return validation("secondary_causes exceeds the bounded v1 limit of " + std::to_string(kMaxRunSecondaryCauses) +
                      ".");
  }
