...
inline constexpr std::string_view kRunRecordSchemaVersion = "kspacejet.run-record/v1";
...
// v1 deliberately does not offer a durable-checkpointed value.  That claim
...
struct RunRecordSpec {
  std::string schema_version = std::string(kRunRecordSchemaVersion);
  std::string run_id;
...
// not report durable recovery or exactly-once semantics: v1 provides only a
