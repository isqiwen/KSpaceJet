 ...
 *   - struct_size is the forward-extension mechanism. Callers must reject a
 *     descriptor that is smaller than the fields they require, and must not
 *     read fields beyond the returned struct_size.
...
#ifndef KSPACEJET_PROVIDER_PROVIDER_H_
#define KSPACEJET_PROVIDER_PROVIDER_H_
...
/*
 * Exact TypeDescriptor enum encodings.
 *
 ...
 * the complete descriptor (including type_identity_digest); matching one enum
...
/*
 * Exact type/layout descriptor used at the plugin boundary. `type_ref` is the
 * human-readable registry key such as `ksj.image-frame`; its one
 * type_identity_digest commits to the complete registry definition. The
 * dimension-name storage is borrowed under the same lifetime as the parent
 * descriptor. A host must reject a payload if its type identity, domain,
 * alignment, mutability, or layout requirements do not match the frozen plan.
 *
 * Provider authors must obtain expected values from
 * <kspacejet/provider/type_registry.h>, never duplicate type references or
 * digest literals in a Provider module.
 */
typedef struct ksj_type_descriptor_view {
  ksj_provider_abi_header abi;
  ksj_utf8_view type_ref;
  ksj_digest256 type_identity_digest;
...
typedef struct ksj_host_services ksj_host_services;
...
typedef struct ksj_operator_create_request {
  ...
  const ksj_host_services* host_services;
...
typedef struct ksj_output_grant_callbacks ksj_output_grant_callbacks;
 ...
 */
struct ksj_output_grant_callbacks {
  ksj_provider_abi_header abi;
  void* host_context;
...
typedef struct ksj_firing_lease_callbacks ksj_firing_lease_callbacks;
 ...
 */
struct ksj_firing_lease_callbacks {
  ksj_provider_abi_header abi;
  void* host_context;
  const ksj_output_grant_callbacks* output_grants;
...
typedef ksj_status(KSJ_PROVIDER_CALL* ksj_operator_on_start_fn)(ksj_provider_operator* operator_handle,
                                                                ...
                                                                ksj_error_view* out_error);
typedef ksj_status(KSJ_PROVIDER_CALL* ksj_operator_process_batch_fn)(ksj_provider_operator* operator_handle,
                                                                     ksj_execution_context* context,
                                                                     ksj_key_state* key_state, ksj_firing_lease* lease,
                                                                     const ksj_firing_lease_callbacks* lease_callbacks,
...
typedef ksj_status(KSJ_PROVIDER_CALL* ksj_operator_on_scan_end_fn)(
  ...
  const ksj_firing_lease_callbacks* lease_callbacks, ksj_process_result* out_result, ksj_error_view* out_error);
 ...
 * corresponding descriptor capability bit. The host never invents a call that
 * was not made admissible by the resolved OperatorContract.
 */
typedef struct ksj_provider_api {
  ksj_provider_abi_header abi;
...
typedef ksj_status(KSJ_PROVIDER_CALL* ksj_provider_query_fn)(const ksj_provider_query_request* request,
                                                             ...
                                                             ksj_provider_api* out_api, ksj_error_view* out_error);
...
#endif // KSPACEJET_PROVIDER_PROVIDER_H_
