...
[[nodiscard]] Result<std::string> require_bounded_string(const Json& object, const std::string_view name,
  ...
  if (code_point_count.value() < minimum || code_point_count.value() > maximum) {
    return validation_error(child_path(path, name), "has a Unicode length outside its current schema bounds");
  }
  return value;
}
...
[[nodiscard]] Result<Quantity> require_quantity(const Json& object, const std::string_view name,
  ...
  if (result > kMaxCanonicalJsonInteger) {
    return validation_error(child_path(path, name), "exceeds the exact current integer range");
  }
  return result;
}
...
public:
  ...
  bool number_float(number_float_t /*value*/, const string_t& /*raw*/) override {
    return fail_validation("floating-point JSON values are not permitted in current records");
  }
  ...
  bool binary(binary_t& /*value*/) override {
    return fail_validation("binary JSON values are not permitted in textual current records");
  }
...
[[nodiscard]] Result<Json> canonicalize_value(const Json& value, const std::string_view path) {
  ...
  if (value.is_number_unsigned()) {
    ...
    if (number > kMaxCanonicalJsonInteger) {
      return validation_error(path, "exceeds the exact current integer range");
    }
    return Json(number);
  }
  ...
  if (value.is_number_float()) {
    return validation_error(path, "is a floating-point JSON value; current records require exact values");
  }
  if (value.is_array()) {
...
[[nodiscard]] Result<RunOutcome> parse_run_outcome(const std::string_view value, const std::string_view path) {
  ...
  return validation_error(path, "is not a current RunOutcome");
}
...
[[nodiscard]] Result<RecoveryClass> parse_recovery_class(const std::string_view value, const std::string_view path) {
  ...
  return validation_error(path, "is not a current RecoveryClass");
}
...
[[nodiscard]] Result<EgressVisibility> parse_egress_visibility(const std::string_view value,
  ...
  return validation_error(path, "is not a current EgressVisibility");
}
...
[[nodiscard]] Result<RunCauseKind> parse_run_cause_kind(const std::string_view value, const std::string_view path) {
  ...
  return validation_error(path, "is not a current RunCauseKind");
}
...
Result<AdmissionRecord> parse_admission_record_json(const std::string_view document) {
  ...
  const auto root_status = validate_object_members(
    root.value(), "$", {"kind", "execution_plan_digest", "verification_record_digest", "outcome", "reservation"},
    {"$schema", "kind", "execution_plan_digest", "verification_record_digest", "outcome", "reservation", "reason"});
  if (!root_status.ok()) {
    return root_status;
  }
  ...
  for (const auto* result : {&kind, &plan_digest, &verification_digest, &outcome_text}) {
    if (!result->ok()) {
      return result->status();
    }
  }
  ...
  return AdmissionRecord::create({.execution_plan_digest = std::move(plan_digest).value(),
...
Result<RunRecord> parse_run_record_json(const std::string_view document) {
  ...
  const auto root_status = validate_object_members(
    root.value(), "$",
    {"kind", "run_id", "execution_profile", "outcome", "recovery_class", "egress_visibility", "secondary_causes"},
    {"$schema", "kind", "run_id", "execution_profile", "execution_plan_digest", "verification_record_digest",
     "admission_record_digest", "outcome", "recovery_class", "egress_visibility", "last_committed_ordinal",
     "primary_cause", "secondary_causes", "replay_of_run_id"});
  if (!root_status.ok()) {
    return root_status;
  }
  const auto schema_status = validate_schema_declaration(root.value(), "$");
  ...
  for (const auto* result : {&kind, &run_id, &profile_text, &outcome_text, &recovery_text, &egress_text}) {
    if (!result->ok()) {
      return result->status();
    }
  }
  ...
  if (!profile.ok()) {
    return validation_error("$.execution_profile", "is not a current ExecutionProfile");
  }
  ...
  if (secondary_json.size() > kMaxRunSecondaryCauses) {
    return validation_error("$.secondary_causes", "exceeds the current maximum cause count");
  }
  ...
  return RunRecord::create({.run_id = std::move(run_id).value(),
