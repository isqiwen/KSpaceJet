...
> 文档状态：建议冻结版  
> 评估对象：`pipeline_definition_v1.md`  
> 建议版本策略：如果原 v1 尚未对外发布，直接以本文为基础重冻结 v1；如果已经产生兼容性承诺，应升级为 v2。  
...
但当前文本更接近“目标架构 + 形式化研究纲领”，还不能直接作为 production v1 开工。主要原因不是设计思想错误，而是：
...
> 应保留其数据流、资源和生命周期内核，但将 v1 收缩为一组可组合、可强制的 MRI 有限流式原语，并把高级证明、通用动态 Provider 和复杂调度逐步开放。
...
| Artifact 分层 | 8.5/10 | 主体正确，但 profile、contract snapshot、certificate 边界需修正 |
...
本评估只基于所提供的 `pipeline_definition_v1.md`，不假设其引用的其他规划或理论文档已经补齐本文发现的缺口。
...
- v1 的 `cancel_cleanup.outputs` **必须**为 `{}`。`on_cancel` 及其 cleanup
  只能释放或 quarantine 已有资源、settle 已注册 async work，并写入独立诊断/审计通道；
...
| 编号 | 问题 | 直接影响 | 优化决定 |
| --- | --- | --- | --- |
| P0-01 | v1 范围同时覆盖通用数据流理论、插件 ABI、调度、证明和硬件执行 | 工程无法形成最小闭环 | v1 只支持有限 MRI motifs；高级 CSDF/通用 merge/任意 automaton 延后 |
...
| P0-03 | MergeSpec 同时属于 OperatorContract，又列具体 source edge id | Provider 静态合同不可能知道用户图中的 edge id | Contract 提供 MergeCapability；Pipeline 提供 MergeBinding；compiler 生成 MergePlan |
...
| P0-05 | OperatorContract payload 内含自身 digest | 形成自引用 hash | digest 放 bundle manifest，使用 detached digest 与 domain separation |
| P0-06 | ResolvedPipeline 与“actual ABI descriptors”都像编译权威 | resolver 与运行前加载结果可能漂移 | ResolvedPipeline 冻结 contract snapshot；加载后只做 digest attestation |
| P0-07 | 端口类型只有字符串，layout/shape/memory domain 未闭合 | 无法安全跨 DLL、CPU、GPU 或验证零拷贝 | 新增冻结 TypeDescriptor，严格匹配；转换必须显式 |
| P0-08 | ResourceExpr 无单位、无 key 环境、无 bounded reducer | 无法正确计算多 encoding、per-key、bytes/count | 使用 typed ResourceExpr 和 compiler-normalized scalar symbol table |
...
| P0-15 | crash 后部分输出、恢复和 exactly-once 范围未定义 | 下游可能收到重复或把失败 run 当成功 | v1 明确 fail-stop-no-resume；replay 创建新 run id |
...
| 问题 | 建议 |
| --- | --- |
| authored node 复制完整 ports，形成双权威 | node 只引用 operator 与可选 interface digest/revision；ports 在 resolution 展开 |
...
| required/optional input 的 close 语义不完整 | 所有已绑定 lane 必须 closed；未绑定 optional 在 start 时置 AbsentClosed |
...
| ExecutionPlan | 某 scan、profile、机器快照下唯一可执行物理计划 |
...
compiler 不能仅凭 shape 猜测某个 Operator 能否安全分块或融合。只有 Provider 合同明确给出 `PartitionCapability/WorkPartitionSpec` 或 `FusionCapability`，且 verifier 能验证其前置条件时，才允许对应转换；没有 capability 时必须保持原逻辑 node 和执行边界。
...
1. ResolvedPipeline 不冻结 requested profile；其可行 profile 集合按下式计算，而不是先把同一 node 的不同 variant 互相求交：

   ```text
   pipeline_profiles
   ∩ intersection_over_nodes(
       union_over_eligible_variants(node.profile_set)
     )
   ```

   PlanBuildRequest 阶段再为每个 node 冻结唯一 variant；
...
artifact_digest = SHA-256(
  "kspacejet:artifact:<artifact-kind>:<schema-major>\0"
  || canonical_full_bytes
)

semantic_digest = SHA-256(
  "kspacejet:semantic:<artifact-kind>:<schema-major>\0"
...
- 可选的 interface revision/digest 断言。
...
如果作者需要防止 Provider 的兼容演化改变图语义，可使用：

- `requires_interface_revision`；
- 或 exact `requires_interface_digest`。

这样避免 PipelineDefinition 与 OperatorContract 两份 port schema 同时演化。

### 7.4 Merge 的三层所有权

| 层 | 对象 | 内容 |
| --- | --- | --- |
| Provider | MergeCapability | 是否支持多源、最大 lane 数、支持的 order/close/retention 模式 |
| Pipeline | MergeBinding | 本图具体 source edge id、order projection、tie-breaker |
| ExecutionPlan | MergePlan | lane、容量、close fence、retention、调度和 proof feature |

ProviderContract 绝不能引用某个用户 pipeline 的具体 edge id。

---

## 8. OperatorContract、类型与执行能力

### 8.1 一个合同、两类内容

为避免 artifact 数量膨胀，保留一个 OperatorContract，但内部明确分为：

1. `interface`：稳定的语义端口、类型、completion、order、side effect；
2. `implementation_variants`：CPU AVX2、CPU AVX-512、CUDA 等资源和执行实现。

compiler 在 ExecutionPlan 中冻结唯一 variant，不能在运行中静默切换。

### 8.2 TypeDescriptor

字符串 `ksj.kspace-frame/v1` 只作为 type id，不足以保证二进制兼容。每个 resolved port 必须引用冻结 TypeDescriptor：
...
- Provider ABI 只交换 opaque handle 和 descriptor，不交换 STL 容器或编译器相关对象。

### 8.3 建议的 OperatorContract 骨架

```json
{
  "schema_version": "kspacejet.operator-contract/v1",
  "provider_id": "org.kspacejet.reference",
  "operator_id": "cartesian_frame_assemble",
  ...
  "interface": {
    ...
    "firing_model": {
      ...
      "completion": {
        "kind": "exact_index_coverage",
        "coverage_spec": "facts.frame_coverage_plan_id",
        "on_end_of_input": "fail"
      }
    },
    "ordering": {
      "domain": "per_key",
      "late_event_policy": "fail"
    },
    "state_scope": {
      "kind": "per_semantic_key"
    },
    "terminal": {
      "normal": "bounded_flush",
      "abort": "no_data_output_no_allocation"
    },
    "determinism": "bitwise_on_qualified_platform"
  },
  "implementation_variants": [
    {
      ...
      "concurrency": {
        "reentrancy": "serial_per_key_reentrant_across_keys",
        "max_supported_in_flight": 8
      },
      "resources": {
        "workspace_bytes": {"ref": "facts.frame_slot_bytes"},
        "scratch_bytes_per_firing": {"const": 0, "unit": "bytes"},
        "private_threads": 0,
        "private_scan_heap": false
      },
      "service_demand": {
        "model_id": "qualified:cpu-reference:1",
        "max_nonpreemptible_us": 200
      }
    }
  ]
}
...
`facts.frame_coverage_plan_id` 必须来自冻结的 NormalizedScanFacts/ScanDescriptor 或经过验证的 acquisition plan。用户 config 只能在 Provider 预先声明的 completion 策略中选择，不能自行提供任意 sampling mask 并成为提前 seal frame 的权威。

#### v1 terminal RateSpec 规则

终止路径仍必须在合同中声明 firing、scratch、async token 和 cleanup 资源上界，但其
ordinary-data 语义不对称：`normal_flush.outputs` 可以声明每个 port 的有限普通 MRI data
上界；`cancel_cleanup.outputs` 则必须精确为 `{}`。因此 compiler、verifier 和 runtime
只能把 normal terminal output 计入 edge/output capacity，不能把 cancel cleanup 作为
普通 data 的潜在生产者。cleanup 的诊断写入独立审计通道，不是 PipelineDefinition data edge。

并发字段按三层合成：

- semantic interface 声明 state scope 和正确性约束；
- implementation variant 声明 reentrancy 与 `max_supported_in_flight` 硬能力；
- MachinePolicy 声明部署上限；
- compiler 取合法最小值并把 chosen value 冻结到 ExecutionPlan。

### 8.4 Partition 与 fusion capability
...
在 frame assembler 前必须有显式、版本化的 acquisition classifier。分类只能基于：
...
#### M1 串行 Cartesian 实现边界（非规范性）

M1 当前实现的是有界 `FrameSlot`、同步 callback 和串行 drain 的正确性基线，不是
`ReorderPlan` 的替代品。为避免在未知的更小 ordinal 前过早发布，某个
...
compiler 生成并验证 bounded `ReorderPlan` 后才开放。
...
max_live_keys
slot_count
mapping_algorithm_id
slot_generation
seal_on_completion
eviction_policy = completed_only
late_event_policy = fail
```

已知稠密 domain 使用 mixed-radix dense index；稀疏 domain 使用固定容量 hash，Running 期间禁止 rehash/扩容。
...
```text
(run_id, egress_port, image_key, ordinal, revision)
```
...
gateway/control/admission
  -> version-pinned worker process/pool
       -> Provider bundle
...
这里的隔离边界是**完整 reconstruction-service/worker 进程**，不是给每个 node 发明 Provider RPC，也不是扩展 scanner 的公开 MRD 协议。进程间共享内存仅属于部署内部实现，不进入 PipelineDefinition、OperatorContract 或公开 wire semantics；如果产品 v1 明确禁止任何内部进程数据面，则 `isolated-strict-online` 必须延后，不能用进程内 watchdog 冒充隔离。
...
3. 启动 version-pinned 新 worker pool；
4. canary 只接新 scan；
...
spec/
  ...
  schemas/
    ...
    provider-bundle-manifest-v1.schema.json
    provider-qualification-record-v1.schema.json
    operator-contract-v1.schema.json
    type-descriptor-v1.schema.json
    resolved-pipeline-v1.schema.json
    scan-descriptor-v1.schema.json
    target-envelope-v1.schema.json
    adapter-contract-v1.schema.json
    machine-policy-v1.schema.json
    performance-policy-v1.schema.json
    execution-plan-v1.schema.json
    verification-record-v1.schema.json
    admission-record-v1.schema.json
...
| ABI | struct_size、minor mismatch、exception、double free、late callback | host/controller 不崩溃，稳定错误码 |
...
### M3：host-enforced bounded CPU runtime
...
| 原方案 | 优化版 |
| --- | --- |
| authored node 复制 ports | node 引用 operator；resolution 展开 ports；可选 pin interface digest |
| MergeSpec 在 contract 中列 edge id | MergeCapability + MergeBinding + MergePlan |
| profile 冻结到 ResolvedPipeline | ResolvedPipeline profile-neutral；PlanBuildRequest 选择 |
| contract 内含 contract digest | detached manifest digest |
| type string + capability | exact TypeDescriptor |
...
2. 把 v1 收缩到可组合、可强制的 MRI 有限 motifs；
