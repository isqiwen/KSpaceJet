...
| P8-004 | 等待 P0-010 解除后重新建立/运行 focused Linux Viewer unit/app-test tree；并在 Windows Release tree 上重建 `ksj_viewer_presentation_tests`、运行 CTest/U… |
...
**当前执行任务**：无；`P8-004` 已完成 persistent recent-files 修正并恢复为 `BLOCKED`。File bar 的 **Recent Files** 将成功打开的 local MRD 与 PipelineDefinition 合并为最多五项的 typed MRU，按成功时间去重置顶；每项以保存的 source type 重开，重启时只保留仍存在的本地普通文件。它与已实现的 shared remembered open-directory preference 使用同一 user-scoped Qt settings，但不记录 payload、URL、credential 或 display state，不改变 read-only/local-only input、blue/red arrShow tag 语义、Image 固定 native X/Y 或 bounded derivative 边界。base `f4126e1` 和 user-directed dirty worktree 保留。不得写入 raw-data sibling，且不运行 GitHub CI。
...
| P8-004 | BLOCKED | P8-003 | 13.28–13.56 的既有 HDFView/MRD/arrShow work 保持。K-space 与 Image 现在共用 `ArrShowDimensionStrip` 的 dimension-column/plane-selection foundation：K-space 的 two blue/red `:` tags 是可替换成员，column order 才定义 X/Y；Image 则锁定 native X/Y 为第一、第二 `:`，只允许后续 varying source dimension 作固定 index。Viewer 以 user-scoped settings 记住最后成功 local MRD/Pipeline parent directory，并保存同一类 source 的最多五项 typed MRU：仅成功打开入列、去重置顶、重启恢复，失效 local entry 在启动时清理。 | 等待 P0-010 解除后重新建立/运行 focused Linux Viewer unit/app-test tree；并在 Windows Release tree 上重建 `ksj_viewer_presentation_tests`、运行 CTest/UI/export smoke、PE GUI-subsystem/Explorer no-console 复核，以及用户以真实 standard MRD source 作视觉复核。 | P0-010 仍禁止建立/运行 Linux unit/app-test tree，且当前无 Windows Release tree；本轮 Linux source/build/smoke/static contract 不能替代 Windows PE subsystem、terminal automation、Explorer 实测或真实 source 用户视觉复核。显示仍限 app-local bounded QImage derivative；不得引入 MATLAB runtime/source/assets、workspace mutation、destructive FFT/crop/ROI、Python/QML/Quick/WebEngine、reconstruction、Provider/gateway 或 MRI artifact。 |
