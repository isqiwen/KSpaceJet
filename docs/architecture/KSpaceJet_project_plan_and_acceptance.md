...
**当前执行任务**：`P8-004`（VERIFYING）。下一精确动作是用户用更新后的 Viewer 打开 `E:\KSpaceJet-ismrmrd-data\datasets\zen-2d-cartesian-2025\cart_t1.mrd`，在 tree 中选择 `[RAW] /dataset_2`（2-D T1 的 256 acquisitions，而非 `/dataset_1` 的 AdjCoilSens），再双击 `Acquisitions / K-space`；核对默认 RSS、centered-readout × `kspace_encode_step_1` 的 512 × 256 plane、frame/aggregation summary、空/重复 cell 计数和 Export JSON provenance。非 Cartesian trajectory 必须继续被明确拒绝，不能伪装成 Cartesian grid。不得写入 raw-data sibling，且不运行 Linux 或任何 GitHub CI 工作。
