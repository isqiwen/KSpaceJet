...
| P0-007 | IN_PROGRESS | P0-002 | 2026-08-22：已在隔离 branch `codex/p0-007-release-ci` 创建并推送 Release-only workflow，PR `#1` 已三次产生实际 `linux-release-quality` 与 `windows-release-build-install` contexts。前两次修复 hosted runner 的 `winget`/旧 `just` 与 Windows PowerShell 差异；第 3 次确认两台干净 runner 都缺少 Conan default build profile。已将 selected project profile 显式同时用作 Conan host/build profile，并通过本机 Windows prepare/check；完整逐次证据见第 13.13 节。 | 提交并推送显式 Conan build-profile 修复，等待 PR `#1` 的第四次两个 Release check；只有均成功后才按实际 context 配置并读取复验 `main` classic protection。 | 不得将 Debug、clean-install qualification、benchmark、Provider load 或性能门禁混入本 P0 workflow；任何第四次 CI 失败先记录实际日志并在本项范围内修复，未成功前不得配置 required checks。 |
