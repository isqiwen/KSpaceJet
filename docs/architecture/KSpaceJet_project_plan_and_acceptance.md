...
| --- | --- | ---: | ---: | ---: | --- |
| P0 | 规范、基线和工程治理 | 6 | 10 | 60% | ACCEPTED: 6 · BLOCKED: 4 |
| P1 | 可信离线 reference 基线 | 3 | 9 | 33.3% | ACCEPTED: 3 · PLANNED: 6 |
| P2 | 图、artifact、compiler、verifier 和 CLI 计划工具 | 1 | 7 | 14.3% | ACCEPTED: 1 · BLOCKED: 1 · PLANNED: 5 |
...
| 工作项 | 解锁后动作 |
| --- | --- |
| P0-005 | 先按 `P0-010` 的 exact unblock action 取得独立 Linux app-test tree；然后构建并运行 `ctest --test-dir out/build/linux-release-app-tests -R '^apps[.… |
...
| P0-010 | 在已认证的 Linux terminal 先执行 `sudo -v`（不要提供或记录密码），再依次执行 `just prepare-release`、`just build-release-applications` 和 `just check`；将 Qt 配置… |
| P2-002 | 先完成 P0-010 的 `sudo -v` / Qt rebuild / `just prepare-release`、`just build-release-applications`、`just check` unblock sequence；再创建/构建… |
| P8-004 | 建立 Windows Release tree，运行 focused Viewer/reader CTest 与 terminal UI/export smoke，核验 Windows GUI subsystem/Explorer 无控制台，并由用户在真实 st… |
...
| ADR-010 | `ksj-viewer` 的 source navigation 以 `InspectionReader` 递归、有界验证的**标准 ISMRMRD container**为根，而非硬编码 `/dataset` 或仅根级 group。每个 container 都按标准语义识别 raw acquisition、image 和/或 waveform；树的可见主标签只保留 HDF5 path，semantic child、tooltip 和 inspector 提供内容说明，用户只从可读 container 中选择，再进入 header、acquisition/k-space、image、waveform 等内容。HDF Group 的 `HDFView` 是文件树、对象检查、typed data view 与状态反馈的功能/交互参考；KSpaceJet 独立以 C++/Qt 实现。 | 用户于 2026-08-23 明确要求标准 ISMRMRD 文件直接打开、查看和重建，且明确指出标准 HDF5 group 名不是固定 API；真实 `cart_t1.mrd` 同时包含空的 `/dataset` 和可读的 `/dataset_1`、`/dataset_2`，手输默认组既不可靠也不符合 inspection workbench 的可发现性。必须保持 P8 的标准语义、只读、有界和无 Python/Matplotlib 依赖。 | P8-004 用单一有界 container discovery 取代仅根级 group discovery，并将 Viewer 重组为 HDFView 式 file root → concise container path → semantic content navigation、对象 inspector、typed `Inspect`/`Open As`、页化 acquisition/coil/trajectory 检查、image cine/window-level；不能把任意 HDF5 浏览或无界缓存搬进 UI。 |
...
| ADR-012 | `ksj-viewer` 的主工作流采用 HDFView 式**File → hierarchy tree → selected-object inspector/contextual data tab → info/status**：菜单/工具栏承载打开、关闭、视图与帮助；打开 MRD 后默认只显示语义树和单一 primary inspector，不设置固定的 `Dataset overview` 或 `Image series` dashboard，也不保留第二个下方 `XML | K-space | Pipeline` tab bar。`General Object Info` 始终存在；**K-space 紧跟 General Object Info**，XML、K-space、Image 只在当前选中的 active standard semantic object 具备相应内容时显示，Pipeline 只在已成功打开 PipelineDefinition 时显示。树 selection 只更新 object summary 与可用的 contextual tabs，不 materialize raw payload；进入 tab/`Inspect`/`Open As` 才开始其有界 inspection，K-space/image rendering 仍需要显式 bounded action。General 页使用紧凑 Name/Path/Type/Access form、标准 dataset semantics 与 member tables；XML preview 只位于 XML contextual tab；原生 HDF5 Attribute 不构成 Viewer 页面。image series 属于 `Images` 语义对象，raw acquisition container 的零 image series 是正常状态。它是 single-purpose、read-only ISMRMRD desktop application，而不是 HDFView 的 Java 代码移植、通用 HDF browser 或 editor。 | 用户于 2026-08-24 明确要求 UI、操作逻辑和功能以本地源码 `E:\hdfview`、安装目录 `D:\HDFView` 与 `HDFGroup/hdfview` 为参考，并使用 C++ Qt 重实现。内置 HDFView manual 证实其主窗口由 menu/toolbar/file bar/tree/metadata/info panel 构成，树选中对象驱动 metadata，`Open`/`Open As` 打开 table/image typed view。用户随后在真实 raw MRD 的视觉复核中指出 file-level `Dataset overview` 和空 `Image series` 区域重复且无价值，并于 2026-08-26 明确要求 K-space/Pipeline 成为 General Object Info 后的独立主标签、移除下方重复页签且缺少 Pipeline 时不显示。KSpaceJet 复用这些可用性原则，但不能扩大 MRI 数据和持久化边界。 | P8-004 替换当前 card/dashboard 重心，完成 HDFView 式 shell、语义对象树、统一的 `General Object Info`/contextual data tabs、context `Inspect`/`Open As`；测试必须证明只读、有界、标准 MRD-only、默认不显示冗余 dashboard 或缺失内容 tab、无 generic HDF traversal/edit/save/URL/file conversion/format support。 |
| ADR-013 | Image 与 K-space 以 app-local Qt `ArrShowDisplay` 和 `InspectionCanvas` 共享 compact controls → dark array canvas → display-derivative probe 的显示/交互基础：Magnitude/Real/Imaginary/Complex/Phase（complex source 默认 Complex）、Gray(256)、`martin_phase(256)` complex/phase colour、degree/radian phase、独立 normal/phase C/W、min/max 或 symmetric percentile range，以及 arrShow 的三种跨平面 C/W policy（reset、relative、absolute）、fit/zoom/pan、image ordinal stepping、K-space sparse-coordinate stepping、middle-button C/W、double-click reset 和 cursor readout。K-space 恰有两条可替换的 `:` display-axis role（初始为 readout × `kspace_encode_step_1`）；任何 observed semantic dimension（包括 raw coil）可成为 X/Y，所有其余 field 是真实 sparse index。它展示原始 Cartesian complex bins，不作 RSS、log transform 或 Fourier transform，且不把 coordinate 称为 Frame。 | 用户于 2026-08-26 要求以本地 `~/workspace/arrShow` 的实际 display semantics 作为两个 Viewer 的共用基础；2026-08-27 又要求多 slice/NEX/echo 等 acquisition dimension 均可查看，并明确 `RO × Co, PE=0` 必须是真实数据投影。arrShow 为 Biomedizinische NMR Forschungs GmbH / Tilman Johannes Sumpf 的 Boost Software License 1.0 work；`apps/kspacejet-viewer/THIRD_PARTY_NOTICES.md` 保留完整 attribution 与 license。适用的 viewer 语义可由 C++/Qt port 实现，但 MATLAB/source/assets、workspace mutation、FFT/crop/ROI/coil-combine、无界 array retention 和任何新的 MRI artifact 都不属于 KSpaceJet Viewer。 | P8-004 保持 app-local、QImage-only bounded derivative 边界；Image 仅在 `with_image_pixels` callback 内 materialize 当前 plane，K-space 仅在 `visit_acquisition()` callback 内 materialize 当前 bounded complex bins。CSV/JSON provenance 必须记录 X/Y axis、fixed coordinate、component、range、phase unit、C/W policy 与 colour mapping。继续以 focused widget/presentation corpus、build-tree UI smoke、Windows CTest 和真实 sibling-data visual review 验证。 |
...
| MachinePolicy 与部署拓扑 | `config/machine-policy.json` 已按用户要求收敛为 local-development source configuration：`32G` host memory、8 CPU permits、2 I/O slots、`gpu_enabled: false`。当前 host 观测为 Linux `6.12.86` x86_64、Intel Xeon Silver 4410Y、48 logical CPUs、2 NUMA nodes、约 125.5 GiB RAM、一个约 24 GiB GPU；这些观测只解释保守本地上限，不成为部署或 GPU capability。 | config/fixture 都没有 deployment owner、scope 或 review date；当前 host 只是本次开发主机。源码中的 C++ value model 仍有更广的 multi-domain fields，但没有 JSON loader。 | **BLOCKED**：deployment owner 必须为每个目标环境给出 CPU affinity/permits、NUMA、RAM reserve、OS、container/bare-metal、NIC 需求，以及明确 CPU-only 或 GPU backend/device/VRAM/driver/stream budget、适用范围与 review date。本机 GPU 不能推断 runtime support；P6-004 仍是 GPU DevicePlan 的前置项。 |
| 性能 SLO | benchmark CMake/工具和空的 report template 存在；默认 preset 关闭 performance/benchmark/native-arch 选项。reference route 的 `acquisitions_read` 等计数只是内部派生/测试值，不是测量结果。 | `docs/benchmark_reports/` 没有已接受 report；无 timed boundary、样本、raw artifacts、owner 或 review。 | **BLOCKED**：performance owner 必须提供 first output、completion、throughput、queue time、p50/p95/p99（含单位）、case/目标机/版本、timed boundary、样本和 raw artifacts，并指定 review date。 |
...
| 参数表示权威 | JSON schema 现刻意只定义小型 human-authored source configuration；C++ `planning_inputs.hpp` value model 仍是更广的内部 planning shape，且未发现 JSON loader/parser/serializer。两者都不能单独成为 deployment 参数 authority。 | schema 仅结构验证，source config 也没有 owner/source/review metadata。 | **BLOCKED**：architecture owner 必须指定可审计的单一参数 artifact/serialization 和与 C++ model 的一致性计划；在此之前不得从任何一侧生成或接受产品 policy，也不得为 source config 省略的内部字段猜测默认值。 |
...
| ID | 通过条件 |
| --- | --- |
| AC-VWR-001 | `ksj-viewer` 是由 CLI11 声明 help/version 的 Qt Widgets desktop application；正常 UI 路径实际创建 `QApplication` 与主窗口。在 Windows 上它必须使用 GUI subsystem（`WIN32_EXECUTABLE`），从 Explorer 启动不得额外创建控制台窗口；从已有终端启动时 `--help`、`--version`、`--ui-smoke`、`--export-smoke` 的 stdout/stderr 自动化协议仍须可用。 |
...
| AC-VWR-006 | viewer 在 1280×800 及以上提供 HDFView 式 Qt Widgets 主窗口：File/Tools/Help 菜单、常用操作工具栏、左侧 hierarchy tree、右侧单一 primary tabbed inspector/contextual data views 及跨宽度的 info/status panel。不得保留常驻当前文件栏；source opening/reopening 通过 File 与 File → Recent Files，完整 source path 仅由 tree-root tooltip、General Object Info 的 Path 或 Info/status 按需提供。`General Object Info` 与 K-space（紧随 General）同级；XML/Image 只在当前 active standard semantic object 可用时显示，Pipeline 只在已打开时显示；不得保留第二个下方 data-view tab strip、原生 HDF5 Attribute tab 或缺失内容 placeholder。树 selection 只更新 General/ISMRMRD Header inspector 与可用 tab，不 materialize raw payload；打开 bounded data view 使用对应 tab、`Inspect` 或 `Open As`，K-space/image rendering 仍明确发生。界面可现代化，但功能层级和信息密度必须可对照 HDFView；不得改变 P8-003 的只读数据、parser、export 或依赖边界。以 widget structure/state tests、真实 QApplication UI smoke 与用户视觉复核验收。 |
| AC-VWR-007 | Viewer 只显示由 `InspectionReader` 在 `InspectionReadLimits` 内递归验证通过的标准 ISMRMRD semantic object tree；不得假设路径为 `/dataset`。文件 root 下以简短 container path 展示可切换 container，Header / Acquisitions (k-space) / Images / Waveforms semantic child 与 tooltip/General inspector 说明 raw acquisition、image、waveform 内容；空、私有或非标准 group 不会阻止有效 container 的发现且不作为标准 source。对象 context menu 只能给出受支持的 `Inspect`、`Open As…`、copy path 或关闭 source；无对应内容必须明确不可用，不得伪造图像或 waveforms。UI 不得直接进行任意 HDF5 traversal、保存 raw payload、无界预读、generic file/object/attribute edit/save、URL loading 或非-MRD format support。以 synthetic nested empty+raw+image+waveform container、focused reader/presentation/widget tests 及 Windows Release UI smoke 验证。 |
| AC-VWR-008 | `Open As…` 以类型特定且有界的 option dialog 开启 acquisition table/k-space 或 image view：Acquisition 使用 Reader 的 header-only、页化索引显示标准 ordinal/flags/encoding/sample/channel/trajectory facts，只在用户选择时读取一个 bounded payload 生成 coil-aware k-space 与 trajectory 显示派生。Cartesian K-space 固定 readout × `idx.kspace_encode_step_1` 为显示轴，并显式列出/选择所有可渲染的 non-axis standard index tuple（`encoding_space_ref`、`idx.kspace_encode_step_2`、`average`、`slice`、`contrast`、`phase`、`repetition`、`set`、`segment`、`user[0..7]`）；鼠标滚轮可按稳定顺序切换 tuple，且不得将它称为 Frame 或 acquisition ordinal。Image 在不保留 source pixels 的前提下支持 standard `image_x` series 的 ordinal cine、z/channel、arrShow per-plane/relative/absolute C/W、zoom、pixel probe 和 histogram（后两者仅对当前有界 display derivative）。不支持的内容必须显式不可用。以 header-only reader、presentation/widget tests 与 Windows Release UI smoke 验证。 |
...
| P8-004 | 按用户反馈将 `ksj-viewer` 重构为 HDFView-inspired、MRD-aware Qt desktop workbench：File/Tools/Help、tool bar、standard MRD semantic tree、单一 primary tabbed object inspector/contextual data views、typed `Inspect`/`Open As…`、页化 acquisition/coil/trajectory、`image_x` cine/window-level/zoom/probe/histogram、信息状态区和空状态；不保留顶部 current-file bar，打开/重开通过 File 与 File → Recent Files，source path 仅在 tree tooltip、General Object Info 和 Info/status 按需提供。`General Object Info | K-space | XML | Image | Pipeline` 处于同一主标签组，K-space 紧随 General，XML/K-space/Image 按当前 active standard semantic object 显隐，Pipeline 仅在已打开时显示，不保留下方重复 tab bar 或缺失内容 placeholder；不提供原生 HDF5 Attribute 浏览页。打开 MRD 默认仅显示语义树加 General Object Info，不显示固定 `Dataset overview` 或 `Image series` dashboard，进入 contextual tab/显式 inspect 才执行有界 data view，raw source 的零 image series 视为正常；以 HDF Group HDFView 的 UI 与操作逻辑为参考，但独立以 C++/Qt 实现。 | apps/kspacejet-viewer、libs/io/kspacejet-ismrmrd、tests/unit/apps、tests/unit/libs/io/kspacejet-ismrmrd、docs。 | P8-003。 | AC-VWR-006、AC-VWR-007、AC-VWR-008；recursive container/header 与 widget structure/state tests、Windows Release build/UI smoke 和用户视觉复核。 |
...
**更新时间**：2026-08-27，`P0-002` 已以用户指定的 Windows Release 路径完成 bootstrap、Conan/CMake configure、四个 application build/install、installed-help/version 和 DLL closure；`P0-007` 继续因用户要求暂缓 GitHub CI 而 BLOCKED。Linux `P0-010` 已实际越过 Qt/X11 apt prerequisite，但 Qt 6.8.3 在严格 `-std=c++20` 的 libstdc++ `__int128` trait 限制处失败；远端 Qt-only GNU C++20 Conan profile 修复已合并，当前仍因缺少 sudo 认证而 BLOCKED，等待 Qt rebuild、application build 和 check。用户此前完成 `P8-004` Object Attribute Info 的真实 Viewer 视觉复核并推动 native HDF5 Attribute reader 修正；随后旧的单条 `sample × channel` 幅度投影已被替换为真正的 raw Cartesian K-space Viewer：header-only frame indexing、`kspace_encode_step_1`/centered-readout grid、RSS/单 coil、重复/空 cell 语义、非 Cartesian 拒绝及 Qt/export provenance 测试均已完成。2026-08-26 用户又要求以 `tsumpf/arrShow` 为 Image/K-space 交互参考，P8-004 因此新增共享 bounded-derivative `InspectionCanvas` 和 Image complex component projection；当前保持 BLOCKED：本 checkout 仅有 Linux application build tree，Windows focused CTest/UI smoke 与真实 standard image/`cart_t1.mrd` 视觉复核仍未完成。图像 `MetaAttributes` 保持 Image details 语义。`P1-002` 的 runtime-owned 标准 ISMRMRD source/sink 边界已通过 focused Windows Release 验证并 ACCEPTED；`P1-008` 的统一 ISMRMRD semantic-frame ingress 与 `P1-009` 的 development-only 2-D radial gridding/analytic DCF Provider 也均已 ACCEPTED。用户已冻结 `ksj-recon --input <scan.mrd> --pipeline <pipeline.json> --output <image.mrd>` 为唯一未来重建入口，并进一步冻结标准-first MRD container 原则：不假设 `/dataset`，任何 private `ksj_*` group 均不得成为查看或重建的前提。`P2-002` 的固定 `dataset` profile 因此 REOPENED，待 P8 的 shared discovery evidence 后改为 auto-or-explicit container selection。
**本轮更新时间**：2026-08-28；`P8-004` 的 arrShow-style dimension-column fill 修正已完成并记录于 13.58，13.59 的 text-only centre drawing 已由 13.60 补正为真正 full-width button-cell geometry。用户随后要求每个 dimension 的**外层列宽**也必须一致；13.61 已将 shared control 固定为共同宽度并完成本 checkout 可执行的受限验证。用户随后要求去除顶部常驻当前文件路径栏；13.62 已删除它并完成本 checkout 可执行的受限验证。用户现已要求并完成移除对普通标准 MRD 通常为空的 Object Attribute Info：General Object Info 成为默认第一页，native-HDF5-attribute reader API 也已撤销，标准 Image `MetaAttributes` 保留在 Image details；最终验证记录于 13.63。用户现明确授权实现 reconstruction framework 的 Pipeline 核心机制；P2-002 已完成 auto-or-explicit standard raw-container selector 的 direct replacement、产品构建、CLI/protocol/smoke 与 no-tree test syntax 验证，记录于 13.64；因 P0-010 禁止独立 Linux unit/app-test CMake tree，转为 BLOCKED 而非 ACCEPTED。
**当前执行任务**：无。`P2-002` 已从 `IN_PROGRESS → VERIFYING → BLOCKED`；解除条件是按 P0-010 的 exact unblock action 建立独立 Linux unit/app-test tree，并运行 P2-002 focused graph/Viewer/app CTest 与相关 protocol gate。P2-003/P3 不开始，因 P0-006 仍缺 MachinePolicy/TargetEnvelope authority；P2-007 仍是 selector 的 runtime-owned HDF5 discovery/binding 后续项。不得写入 raw-data sibling，且不运行 GitHub CI。
**P8-004 当前补充**：K-space 的 `Data` 下拉优先默认可渲染的 `Imaging data`；若当前 container 没有可渲染 Imaging coordinate，则自动选择稳定顺序中的第一个可渲染 auxiliary type。header-only catalog 只保留有界、实际可渲染的 Cartesian source coordinate；恰有两条 caller-selected blue/red `:` tag，任意 observed varying semantic field（包括 `Co`）可成为 K-space plane member，其余字段是 actual compatible sparse coordinate。普通滚轮和 `+`/`-` 只浏览非-axis observed values；没有 record ordinal 或 Frame selector。Image 的 native X/Y 不可重选，后续可见维度也不能成为 `:`。Imaging data 对映射到 acquisition header 的所有 declared XML encoding limits 执行 hard eligibility validation，auxiliary type 保留 observed phase-encode range。完整证据见 13.54。
**P8-004 2026-08-27 缩写控件历史更新**：13.38 的 concise labels、tooltip/accessibility、observed-varying membership 与无 Frame 语义保持；其“固定保留 Readout `:` 与 Phase encode `:`”表述已由 13.52 取代。当前 `:` 表示任意两条可替换 display-axis role，不再固定到特定字段。
**P8-004 2026-08-27 XML 当前更新（以此为准）**：13.39 将显式 Inspect 的标准 Header/XML contextual tab 从单一纯文本框改为 container/byte/acquisition/image-series 摘要、最多 2,048 节点的结构大纲，以及格式化、语法高亮的只读 XML source。每个大纲 value 预览最多 120 字符；只有最多 64 层、4,096 elements 的 preview 才会 in-memory pretty print，避免 indentation 放大内存。超过格式化阈值或解析失败时仍显示原有有界 source preview；不会重写或写入 source XML。完整证据见 13.39；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 XML 宽度当前更新（以此为准）**：13.40 将 Header structure/source 的可拖动 splitter 默认尺寸改为 500 px / 760 px；左侧 Header structure 最小 460 px、其 Element 列初始 230 px，右侧 Formatted XML 最小 320 px。两侧均为 non-collapsible、等 stretch，用户仍可通过中间分隔条按需调整。该 UI geometry 修正不改变 13.39 的 context、只读 source、outline/formatter bounds 或标准 ISMRMRD 语义。完整证据见 13.40；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 XML 模式当前更新（以此为准）**：13.41 以一个内容区域的显式 `Header structure | Formatted XML` mode tabs 替代 13.39/13.40 的 side-by-side splitter。默认始终为第一个 **Header structure** mode，显示完整宽度的有界 outline 与摘要；用户切换到 **Formatted XML** 才显示完整宽度的只读、语法高亮 source preview。该切换不改变 source XML、standard ISMRMRD 语义、2,048-node outline limit 或 64-depth/4,096-element formatting limit。完整证据见 13.41；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 全屏当前更新（以此为准）**：13.42 将唯一 application startup path 的 `ViewerWindow::show()` 改为 `showFullScreen()`。普通交互启动、`--ui-smoke` 和 `--export-smoke` 都走这个 Qt startup path；后两者仍由已有 zero-delay quit 自动退出。该改动不改变 Windows GUI-subsystem、logger、standard ISMRMRD、只读、bounded display derivative 或任何 MRI artifact 语义。完整证据见 13.42；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 Header structure 列宽当前更新（以此为准）**：13.43 将 **Element** Qt header section 的初始宽度从 230 px 增至 360 px，并显式设为 `Interactive`，因此其余 **Value** 列仍 stretch、用户仍可拖动分隔线调整。该 geometry 修正不改变 Header structure/Formatted XML mode、outline/source bounds、standard ISMRMRD 语义或 source XML。完整证据见 13.43；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 启动窗口当前更新（以此为准）**：13.44 根据用户对关闭按钮的反馈，以 `showMaximized()` 取代 13.42 的 literal `showFullScreen()`。普通 Viewer 默认占满 desktop work area，同时保留 native title-bar 的最小化、最大化与关闭按钮；`--ui-smoke`/`--export-smoke` 仍沿用同一 startup path 和自动退出。该更正不改变 Windows GUI-subsystem、logger、standard ISMRMRD、只读、有界 display derivative 或 MRI artifact 语义。完整证据见 13.44；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 XML 标签当前更新（以此为准）**：13.45 将 13.41 的 **Header structure | Formatted XML** 改为 **XML Tree | XML Text**。二者都是同一 selected standard XML 的 alternate bounded/read-only presentations；Tree 表示 hierarchical element/value view，Text 表示 syntax-highlighted text view。Text 的 indentation 只可为 presentation 而 normalized，故不称 byte-identical `Raw XML`；无论哪种 view 都不写入 source XML。完整证据见 13.45；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 K-space 控件当前更新（以此为准）**：13.46 移除了与既有 Data、dimension、raw-coil 和 display-control automatic refresh 重复的 **Render** 按钮。K-space 的 **View** 组现在在 zoom 后提供 **Reset**：它重置 view scale 到 100% 并清除 pan，不重置 C/W；独立的 **Reset C/W** 继续恢复当前 arrShow normal/phase C/W。完整证据见 13.46；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 K-space 布局当前更新（以此为准）**：13.47 删除了常驻的 **K-space plane details** summary/table 组件和横向 splitter。画布现在占满 K-space 内容区域；Data、arrShow dimensions、C/W、View/Reset、pixel probe 与 display-derivative export provenance 保持。用户不再需要阅读重复的 selected coordinate、source bounds、type-membership 和 skipped-acquisition diagnostics；完整 coordinate 仍随 JSON export provenance 保留。完整证据见 13.47；P0-010 与缺失 Windows Release tree 仍使 P8-004 BLOCKED。
**P8-004 2026-08-27 任意二维 K-space 轴历史启动记录**：用户进一步澄清：`:` 不表示永远固定的 `RO | PE`，而是 arrShow 的两条可选择 display-axis role。例如将 `Co` 设为 `:` 后，原来的 `PE` 成为真实索引（可为 `0`），实际显示平面必须是 `RO × Co`，不是仅把控制文字改为 `:`。P8-004 当时从 BLOCKED 重新进入 IN_PROGRESS，以替换原有固定 plane request、single-coil filter 和 readout×phase-only aggregation；保留两条 axis、observed sparse tuple、raw payload callback lifetime、有界 derivative、无 FFT/RSS/reconstruction 与 `Data` acquisition-type filter。该轮的完成与新的 BLOCKED evidence 见 13.52；13.50/13.51 的“two fixed display axes”实现不再代表当前语义。
...
| P0-005 | BLOCKED | P0-001, P0-004 | 原 ACCEPTED evidence 见 13.7。2026-08-27 app logging regression implementation/partial validation 见 13.31：five entries retain common `KSpaceJet::core` linkage and fallback init; every CLI failure writes one core WARN/ERROR, Viewer GUI error and Qt diagnostic paths are relayed, and JSON stdout isolation remains tested. | 先按 `P0-010` 的 exact unblock action 取得独立 Linux app-test tree；然后构建并运行 `ctest --test-dir out/build/linux-release-app-tests -R '^apps[.](logging_contract|json_cli_protocol)$' --output-on-failure`，再恢复本项验收。 | 直接 Python protocol/source-contract、existing product build 和 clean-environment Viewer smoke 已通过，但不替代独立 app-test CTest gate；不改变 JSON/text command-result schema、Logger plain-text policy、Viewer offline/read-only scope 或 P8-004 的 BLOCKED gate。 |
| P0-006 | BLOCKED | P0-001 | 基线为 `8c31b30419ed330688b3f1b90f14a4498503317d` / tree `4455dc2fb45214952b710fa5519d8d084e9efad5`，并包含 P0-001/P0-004/P0-005 的未提交改动。第 6.3.1/13.8 节证明没有 committed、deployment-owned TargetEnvelope 或 MachinePolicy；fixture、reference defaults、research case 和本机硬件均已明确降级为非产品证据。2026-08-23 用户新增真实 Gateway 方向后，第 6.3.1 已追加公开 profile、PKI、Connector、网络区、egress 和保留 policy 的缺失输入。2026-08-28 用户要求根 `config/` 直接保存真实、保守且小型的开发 source configuration：MachinePolicy 仅保留 profile、32 GiB host memory、8 CPU permits、2 I/O slots、GPU disabled；TargetEnvelope 仅保留 XML/acquisition/frame/image、samples、channels 和 single-scan ceilings。物理容量只接受严格紧凑的 `B`、`K`、`M`、`G`、`T` 字符串，其中 `K/M/G/T` 为二进制单位；未来 loader 必须 checked-normalize 为 canonical integer。二者不自动加载，且因 source schema/C++ value model/parser 未统一而不能成为 runtime input；它们不是 deployment/product/Gateway authority，不解除本项阻塞。 | 收集第 6.3.1 所列 case、deployment、performance、data-governance、output、security/release、architecture 及 GWY-DEC-001 至 007 owner/source/scope/review inputs；收到后重新打开 P0-006 并按每项复核。 | 不得把 development target 的 channel ceiling 推断为框架或 scanner channel 上限；不得把 `gpu_enabled: false` 或当前 host 观测推断为 GPU runtime capability；也不得猜测 SLO、Gateway Profile、TLS/PKI、Connector 或数据保留，或把 fixture、reference-route value 或当前 Linux host 自动提升为产品 envelope。 |
...
| P0-010 | BLOCKED | P0-003 | 本次 Qt profile repair 基线为 `960612178077855a38759db686ae7afc7f750e9b` / tree `1f8dd6a228b91cbbfef2c8021f37e6bfa1bfb0b6`。用户在 Linux host 实际运行 `just prepare-release` 后，Qt 已通过 configure 和 X11 discovery，但以 `-std=c++20` 编译 `qtbase/src/corelib/global/qtypes.cpp` 时在 line 508 报 `Qt needs to be compiled in a mode that enables INT128`；缓存 toolchain 明确设置 `CMAKE_CXX_EXTENSIONS OFF`。两个 Linux profile 现仅为 `qt/*` 设 `compiler.cppstd=gnu20`，root、debug、release 和 unit-test graph 均保留 KSpaceJet `20`、Qt `gnu20` 与新 package ID；feature/static/doc gates 已通过，完整记录见 13.12。 | 在已认证的 Linux terminal 先执行 `sudo -v`（不要提供或记录密码），再依次执行 `just prepare-release`、`just build-release-applications` 和 `just check`；将 Qt 配置/编译旗标与最终结果记录后重新打开本项。 | 当前工具会话的 `sudo -n true` 输出 `sudo: a password is required`，不能执行实际 apt、Qt source rebuild、application build 或完整 check；不得改 Windows profile、Qt source/cache、全局 Conan config、KSpaceJet target C++ extension policy 或产品 API/ABI/claim。 |
...
| P2-002 | BLOCKED | P2-001 | 基线 `cdc05cf`；13.64 已完成 PipelineDefinition input-profile 的 direct replacement：闭合 auto-or-explicit container-mode grammar、explicit absolute non-private HDF5 container path、canonical artifact identity、schema/fixtures/parser/Resolver documents/CLI report/Viewer presentation/docs，以及临时 route-generated PipelineDefinition。现有产品构建、CLI/protocol/offscreen smoke 与 direct no-tree syntax 全部通过。实际 HDF5 raw-container discovery/binding 仍由 P2-007 runtime-owned source adapter 验证。 | 先完成 P0-010 的 `sudo -v` / Qt rebuild / `just prepare-release`、`just build-release-applications`、`just check` unblock sequence；再创建/构建独立 Linux unit/app-test tree，运行 focused `ksj_recon_graph_tests`、`ksj_viewer_presentation_tests` 和 application JSON protocol CTest，并将结果写入 13.64 后重新进入 VERIFYING。 | 不能把现有 application build、direct `fsyntax-only`、CLI protocol 或 offscreen smoke 当作独立 unit/app-test CTest 的替代；不保留 `dataset_group`/固定 `dataset` compatibility 字段，不把 group path 移到 CLI，不触及 route-local source binding 或 P2-003/P3。P2-003 仍被 P0-006 MachinePolicy/TargetEnvelope authority 阻塞。 |
...
| P8-004 | BLOCKED | P8-003 | 13.28–13.63 的既有 HDFView/MRD/arrShow work 保持。K-space 与 Image 现在共用 `ArrShowDimensionStrip` 的 dimension-column/plane-selection foundation：K-space 的 two blue/red `:` tags 是可替换成员，column order 才定义 X/Y；Image 则锁定 native X/Y 为第一、第二 `:`，只允许后续 varying source dimension 作固定 index。Viewer 以 user-scoped settings 记住最后成功 local MRD/Pipeline parent directory，并保存同一类 source 的最多五项 typed MRU：仅成功打开入列、去重置顶、重启恢复，失效 local entry 在启动时清理。13.57 将 MRU access 改为 **File → Recent Files** dynamic submenu；13.58 使 dimension column 的非输入区域与 controls panel 一致，只有 editable index input 保持白底；13.60 令所有 button row 与 value/`:` 使用完整的 column content width；13.61 令每个 outer dimension column 固定为相同的 42 logical-pixel 宽度；13.62 删除 current-file bar，仅通过 File/Recent Files 打开或重开 source。13.63 已移除 Object Attribute Info 和其唯一的 native-HDF5-attribute API；标准 Image MetaAttributes 仍仅位于 Image details。 | 建立 Windows Release tree，运行 focused Viewer/reader CTest 与 terminal UI/export smoke，核验 Windows GUI subsystem/Explorer 无控制台，并由用户在真实 standard MRD 视觉复核。 | P0-010 仍禁止建立/运行 Linux unit/app-test tree，且当前无 Windows Release tree；本轮 Linux source/build/smoke/static contract 不能替代 Windows PE subsystem、terminal automation、Explorer 实测或真实 source 用户视觉复核。显示仍限 app-local bounded QImage derivative；不得引入 MATLAB runtime/source/assets、workspace mutation、destructive FFT/crop/ROI、Python/QML/Quick/WebEngine、reconstruction、Provider/gateway 或 MRI artifact。 |
...
- 2026-08-24 VS Code shared-task consolidation: 用户明确要求将所有可共享的 VS Code task 合并。`.vscode/tasks.json` 保留直接平台 bootstrap，并将原先 12 个 Linux/Windows post-bootstrap prepare/build/install task 收敛为 6 个按 Debug/Release 区分的 shell/direct-`just` task；根 `justfile` 仍拥有每个 recipe 的 `[linux]` / `[windows]` 实现。构建和安装 task 在 `linux` / `windows` override 中分别保留 `$gcc` / `$msCompile` problem matcher；两个安装 task 只依赖相应的通用 build task。`launch.json` 的全部 8 个 Debug `preLaunchTask` 现指向 `KSJ: build Debug applications`，Linux/Windows debugger configuration 本身未合并。AGENTS、README、developer/build docs 与仍会呈现当前 task 名称的历史参考文档均同步为新名称；历史 12-task evidence 保持原文。`python3 -m json.tool .vscode/tasks.json` 和 `.vscode/launch.json`、managed-Python focused mapping assertion（7 tasks、6 shared recipes、8 F5 references）、`just --unstable --fmt --check`、`just --list`、`just link-check`（76 files / 172 local links）、`just workspace-check`、`just format-changed` 与 `git diff --check` 均 exit 0。随后 `sudo -n true` 仍输出 `sudo: a password is required`，所以没有运行 apt、`just prepare-release` 或 `just check`；状态由 `IN_PROGRESS` 回到 `BLOCKED`，解阻条件保持用户在本机 terminal 完成 `sudo -v` 后的实际 prepare/check 输出。

- 2026-08-25 Qt strict-C++20 repair: 用户提供的真实 `just prepare-release` 输出表明 X11 system requirements 已通过；Qt 6.8.3 configure 后却以 `-std=c++20` 编译 `qtypes.cpp`，在 `qtypes.cpp:508` 报 INT128 mode error。缓存 Qt toolchain 的 `CMAKE_CXX_EXTENSIONS OFF`、Qt `qtypes.h` 对 `__GLIBCXX__ && __STRICT_ANSI__` 的保护，以及 GCC 14 feature probe 共同证明根因：严格模式不提供 libstdc++ `__int128` type traits，而 GNU C++20 提供。`conan/profiles/linux-gcc14-debug` 和 `linux-gcc14-release` 因此仅增加 `qt/*:compiler.cppstd=gnu20`；unit-test profile 继承 release。managed Conan `graph info --no-remote` 对 Debug、Release 和 unit-test graph 均断言 root `kspacejet/7.1.0` 仍为 `compiler.cppstd=20`、`qt/6.8.3` 为 `gnu20` 且有新的 package ID；Conan CMakeToolchain source assertion确认 `gnu20` 映射为 C++20 with extensions ON。root CMake / bootstrap 保持 `CMAKE_CXX_EXTENSIONS OFF`，没有编辑 Qt source/cache、Windows profile 或全局 Conan config。`conan profile show`、GCC strict/GNU feature probe、`bash -n tools/devenv/linux/bootstrap.sh`、`just --unstable --fmt --check`、`just link-check`（76 files / 172 local links）、`just workspace-check`、`just format-changed` 和 `git diff --check` 均通过。`sudo -n true` 仍输出 `sudo: a password is required`，故未在本工具会话实际 rebuild Qt；解阻条件是用户在已认证 terminal 运行 `sudo -v && just prepare-release && just build-release-applications && just check`，并提供非敏感结果。
...
### 13.19 P2-002 历史 ACCEPTED 证据（已因 selector 语义 REOPENED）
...
- Historical remaining-acceptance note: 当时仅剩 Windows-only 用户视觉复核，使用真实 sibling-data `E:\KSpaceJet-ismrmrd-data\datasets\zen-2d-cartesian-2025\cart_t1.mrd` 检查默认 inspector、container tree 和显式 typed views，因此 P8-004 当时保持 IN_PROGRESS；未运行 Linux 或 GitHub CI，且不对 service、clinical、performance 或 release qualification 作任何声明。
...
- Native Attribute correction (2026-08-24): 用户随后恢复 P8-004，并以真实 sibling-data 截图确认原先 Object Attribute Info 的 `MetaAttributes` 复用是语义错误。当前 `InspectionReader::read_object_attributes()` 仅接受已验证的 container/XML/acquisitions/validated-waveforms/advertised image-series selector，读取实际 native HDF5 Attribute 的 name、readable type、dataspace shape、element count 与 bounded value preview；没有 arbitrary HDF5 path。独立 `image_x` 容器显式拒绝 XML/acquisition/waveform selector，避免将其像素 `data` 误称 raw acquisition；非标准同名 `waveforms` 同样拒绝。超出 element/value bound 的 numeric/fixed-string Attribute 保留结构 descriptor 并标记为 omitted/truncated preview，compound/vlen 等不支持类型不 materialize source value。Viewer Object Attribute Info 现在只展示这些 descriptors；Images 作为语义 collection 明确不具备 aggregate HDF5 Attributes，而 Image `MetaAttributes` 只显示在 Image details。
- Native Attribute focused corpus (Windows Release, 2026-08-24): `inspection_reader_tests` 新增 synthetic HDF5 scalar、fixed-string、numeric array、compound、empty Attributes、preview byte/element budget、nonstandard waveform 与 standalone `image_x` selector corpus；`viewer_presentation_tests` 在非 active `/dataset_2/data` 写入真实 `acquisition_count_hint=42` Attribute，断言 selection 只更新 Inspector、显示 `Name / Type / Array Size / Value`，并保持 typed view 不切换、Header 空 Attribute 明确显示、Images 不再伪装 MetaAttributes。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 cmake --build out\build\windows-vs2022-release-unit-tests --config Release --target ksj_ismrmrd_tests ksj_viewer_presentation_tests --parallel 4` exit 0；裸 `ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "ksj_(ismrmrd|viewer_presentation)_tests" --output-on-failure` exit 0（2/2）；未调用 `conanrun.bat` 的 direct `ksj_ismrmrd_tests.exe`/`ksj_viewer_presentation_tests.exe` 分别 23/23、7/7 通过，证明 Windows test DLL staging 仍有效。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 cmake --build out\build\windows-vs2022-release --config Release --target ksj_viewer --parallel 4` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` 均 exit 0。runtime scanner 的系统 component candidate 与 `windeployqt` optional translations catalog warnings 仍不影响实际启动。
- Native Attribute quality checks (Windows Release, 2026-08-24): repository-managed `clang-format -i` 后对七个 changed C/C++ 文件执行 `clang-format --dry-run --Werror` exit 0；`git diff --check` exit 0（仅 Git 的 CRLF normalization warning）；`python tools/checks/check_execution_plan.py --write` 后 `--check` exit 0（69 work items）；`just link-check` exit 0（76 Markdown files / 172 local links）。
- Verification state (2026-08-24): source、synthetic tests、docs 与 Windows focused validation 已具备，P8-004 在第 12 节进入 `VERIFYING`。仍需用户使用修复后的 Viewer 对真实 `cart_t1.mrd` 作最终视觉复核；未运行 Linux 或 GitHub CI，且不作 reconstruction、service、clinical、performance 或 release qualification claim。

- Empty semantic-child refinement (Windows Release, 2026-08-25): 根据用户对 tree 中 `Images (none)` 与 `Waveforms (none)` 的反馈，`ViewerWindow::rebuild_dataset_navigation()` 仅在 container 有至少一个 advertised image series 时创建 `Images`，并仅在验证到标准 waveform storage 时创建禁用的 `Waveforms (discovered)`；缺失内容不再生成伪 object/action。`ksj_viewer_presentation_tests` 创建 raw-only `/dataset_1`、image `/dataset_2` 和 standard-waveform `/dataset_3`，以 navigation data role 定位 parent container，断言 raw-only 节点没有 Images/Waveforms、image container 保留 Images 且不含 Waveforms、waveform container 保留但禁用的 Waveforms。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release-unit-tests','--config','Release','--target','ksj_ismrmrd_tests','ksj_viewer_presentation_tests','--parallel','4')` exit 0；裸 `ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "ksj_(ismrmrd|viewer_presentation)_tests" --output-on-failure` exit 0（2/2）；未调用 `conanrun.bat` 的 direct `ksj_ismrmrd_tests.exe`/`ksj_viewer_presentation_tests.exe` 分别 23/23、7/7 通过。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release','--config','Release','--target','ksj_viewer','--parallel','4')` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` 均 exit 0。runtime scanner unresolved Windows system candidate 和 `windeployqt` 缺失可选 translations catalog 警告不影响直接启动；未运行 Linux 或 GitHub CI。

- Empty Pipeline refinement (Windows Release, 2026-08-25): 用户要求没有打开的 PipelineDefinition 不应作为 tree 占位项，并将实际打开的对象显示为 `Pipeline`。`ViewerWindow::rebuild_dataset_navigation()` 现在只在 `pipeline_presentation_` 已成功加载时创建根级 `Pipeline`；底层 JSON artifact type、parser 和 Inspector type 仍准确称为 `PipelineDefinition`。为稳定 Qt widget 验证，新增 app-local `open_pipeline_source()`：只接收 local JSON、先在局部 presentation 解析成功后替换当前 presentation，不 resolve/compile/load/execute Provider。初始 workbench 断言只有 disabled `No ISMRMRD source open` root、没有 Pipeline；打开 synthetic valid PipelineDefinition 后断言存在可 inspect 的精确 `Pipeline` root，显式 Inspect 保持 Pipeline typed view 可见。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release-unit-tests','--config','Release','--target','ksj_ismrmrd_tests','ksj_viewer_presentation_tests','--parallel','4')` exit 0；裸 `ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "ksj_(ismrmrd|viewer_presentation)_tests" --output-on-failure` exit 0（2/2）；direct `ksj_viewer_presentation_tests.exe` 7/7 通过。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release','--config','Release','--target','ksj_viewer','--parallel','4')` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` 均 exit 0。未运行 Linux 或 GitHub CI。

- Semantic-table height refinement (Windows Release, 2026-08-25): 用户截图确认 General Object Info 的 `Dataset Dataspace and Datatype` 表以固定 `rows × 23 + 4` 估算高度时，Windows 10pt styled rows 的首行会被裁切。`ViewerWindow::update_object_inspector()` 现在先调用 `resizeRowsToContents()`，再以每个实际 `rowHeight()` 和两侧 `frameWidth()` 设置固定 content-fit 高度；外层 General Object Info `QScrollArea` 保留页面级滚动责任。`ksj_viewer_presentation_tests` 显式选择 source root，断言三行 semantics、内部 vertical scrollbar maximum 为零，且最后一行底部不超出 table viewport。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release-unit-tests','--config','Release','--target','ksj_ismrmrd_tests','ksj_viewer_presentation_tests','--parallel','4')` exit 0；裸 `ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "ksj_(ismrmrd|viewer_presentation)_tests" --output-on-failure` exit 0（2/2）；未调用 `conanrun.bat` 的 direct `ksj_viewer_presentation_tests.exe` 7/7 通过。`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 -Command @('cmake','--build','out/build/windows-vs2022-release','--config','Release','--target','ksj_viewer','--parallel','4')` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` 均 exit 0。未运行 Linux 或 GitHub CI；仍待用户对真实 sibling-data 的最终视觉复核。
- True Cartesian K-space replacement (Windows Release, 2026-08-25): 用户确认单条 `sample × channel` magnitude projection 不是 K-space Viewer 后，删除 `make_kspace_presentation()`，新增 `CartesianKspaceRequest` / `make_cartesian_kspace_presentation()`。先以新的 `InspectionReader::for_each_acquisition_header()` 读取 owned header records（绝不 materialize `data`/`traj`），按 reference 的 `encoding_space_ref`、`idx.kspace_encode_step_2`、average/slice/contrast/phase/repetition/set/segment 和八个 `idx.user` counters 选择 raw frame；只对匹配 ordinal 逐条 `visit_acquisition()`。显示 grid 的 x 是 `sample - center_sample`（忽略 `discard_pre`/`discard_post`），y 是 XML `kspace_encode_step_1` limit 或 observed range；RSS/单 coil 先计算 source complex magnitude，再对重复或下采样 display bins 求 RMS；空 cell、multi-contribution cell、frame、source/display geometry、coil 与 CSV truncation 都输出到 local JSON provenance，grayscale 使用 `log10(1 + RMS magnitude)`。有 trajectory、non-Cartesian XML、frame/channel/shape 不一致、非有限数或 source/display bounds 超限均失败，不伪装 Cartesian grid。Qt 页签改为 K-space：reference acquisition + RSS/coil selector + Render Cartesian K-space，tree 的 `Acquisitions / K-space` explicit Inspect/double click 直接渲染该平面，reference header 保留为次级上下文。synthetic corpus 覆盖 non-monotonic ky、重复/空 ky、不同 slice frame exclusion、RSS/单 coil numerical result、CSV source-coordinate ranges、Qt explicit Inspect 成功渲染。`cmake --build out/build/windows-vs2022-release-unit-tests --config Release --target ksj_ismrmrd_tests ksj_viewer_presentation_tests --parallel 4` exit 0；裸 `ctest --test-dir out/build/windows-vs2022-release-unit-tests -C Release -R "^(io\\.kspacejet-ismrmrd\\.ksj_ismrmrd_tests|apps\\.kspacejet-viewer\\.ksj_viewer_presentation_tests)$" --output-on-failure` exit 0（2/2）；direct filtered executable 2/2 passed。`cmake --build out/build/windows-vs2022-release --config Release --target ksj_viewer --parallel 4` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` exit 0。`clang-format --dry-run --Werror`、`git diff --check`、`just link-check` 和 plan checker 均通过；runtime scanner unresolved Windows system candidates 与 optional Qt translations catalog warning 未影响 test/UI 实际启动。

- Cartesian K-space verification refinement (Windows Release, 2026-08-25): `viewer_presentation_tests` 进一步以零 trajectory payload、XML 声明 `radial` 的 corpus 覆盖 XML trajectory 拒绝分支；并为 `encoding_space_ref`、kz、average、slice、contrast、phase、repetition、set、segment 和 `idx.user[7]` 分别加入 nonmatching frame，重复 ky 使用不同振幅以证明 RSS 与 single-coil 的聚合是 RMS 而非算术平均。最终以 `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 cmake --build out\build\windows-vs2022-release-unit-tests --config Release --target ksj_ismrmrd_tests ksj_viewer_presentation_tests --parallel 4`、裸 `ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "^(io\\.kspacejet-ismrmrd\\.ksj_ismrmrd_tests|apps\\.kspacejet-viewer\\.ksj_viewer_presentation_tests)$" --output-on-failure`（2/2）、`cmake --build out\build\windows-vs2022-release --config Release --target ksj_viewer --parallel 4` 与 build-tree `ksj-viewer.exe --ui-smoke --format json` 完成复核；`clang-format --dry-run --Werror`、`git diff --check`、plan/link checks 均通过。Windows system runtime candidate 与 optional Qt translations catalog warning 未影响直接 test/UI 启动。P8-004 保持 `VERIFYING`，只等待真实 sibling-data 的用户视觉复核；未运行 Linux 或 GitHub CI。

- Unified primary inspector tabs (Windows Release, 2026-08-26): 根据用户要求，删除 `ViewerWindow` 下方独立 `viewerDataSurface` / `viewerDataViews`、vertical details splitter 与其 Window “Close typed data view” action；所有页面直接纳入一个 `objectInspector`，固定顺序为 `Object Attribute Info | General Object Info | K-space | XML | Image | Pipeline`。K-space 因此紧随 General；XML/K-space/Image 只在当前 active standard semantic object 有相应内容时可见，Pipeline 只在成功解析 PipelineDefinition 后可见，缺失内容不再保留空 tab。tab visibility 用页面指针而非固定 integer index，当前 display-derivative export 也以当前 page identity 派发，因此动态显隐不能导出错页。widget corpus 断言没有下方 tab strip、初始仅 Attribute/General、K-space 次序、对象选择触发的 XML/K-space/Image 显隐、缺失 Pipeline 隐藏和 Pipeline 打开后的独立显示。`clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 cmake --build out\build\windows-vs2022-release-unit-tests --config Release --target ksj_viewer_presentation_tests --parallel 4` exit 0；`ctest --test-dir out\build\windows-vs2022-release-unit-tests -C Release -R "^(io\\.kspacejet-ismrmrd\\.ksj_ismrmrd_tests|apps\\.kspacejet-viewer\\.ksj_viewer_presentation_tests)$" --output-on-failure` exit 0（2/2）；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\devenv\windows\run.ps1 cmake --build out\build\windows-vs2022-release --config Release --target ksj_viewer --parallel 4` exit 0；build-tree `ksj-viewer.exe --ui-smoke --format json` 返回 `ok:true`；`just link-check`（76 files / 172 links）与 `git diff --check` exit 0。未运行 Linux 或 GitHub CI；P8-004 保持 `VERIFYING`，仍需用户在真实 sibling data 上作最终视觉复核。

- Pipeline authored-DAG visualization (source/static, 2026-08-26): 根据用户新要求，Pipeline 主标签保留在唯一 `objectInspector` 内，新增 Qt Widgets `QGraphicsView` canvas。`load_pipeline_presentation()` 只消费唯一 public `PipelineDefinition::parse_json()` 的结果，投影 ingress、authored Provider alias/operator、egress、ordinary data edges 与 calibration producer/consumer edges；没有 Resolver、ExecutionPlan compiler、Provider loader 或 runtime 调用。graph 是确定性 DAG 分层，calibration 作为真实依赖参与层级；每个节点的 outgoing/incoming side 依端口/edge identity 分配锚点，避免 parallel edge、fan-in 和 fan-out 覆盖。超过 512 semantic items 时 canvas 仅显示明确 bound notice，canonical JSON 仍可检查且不生成 partial topology。Pipeline current-display PNG/SVG 现在从有界 scene render 取得图像；仍标记为 `visualization-derivative`。
- Pipeline graph regression corpus (source/static, 2026-08-26): `viewer_presentation_tests` 的 synthetic PipelineDefinition 覆盖 parser-only node/edge projection、ingress/data/calibration/egress、Provider alias wording、calibration-only ordering、parallel-edge 和 fan-in endpoint slots，以及 515 semantic-item input 的 no-partial-graph notice。Graph item identity 仅供 widget regression 定位；不属于 public artifact 或 runtime contract。
- Validation and BLOCKED state (2026-08-26): `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0；`git diff --check` exit 0；`just link-check` exit 0（76 files / 172 local links）；`python3 tools/checks/check_execution_plan.py --write` 后 `--check` exit 0（69 work items）。诊断 `find out/build -mindepth 1 -maxdepth 1 -type d -printf '%f\\n'` 仅输出 `linux-release`，且 Windows Release build tree absent；P0-010 仍记录用户暂停 Linux 验证。因此没有运行 Windows focused build/CTest/UI smoke，也没有运行 Linux test/build，P8-004 置 `BLOCKED`，等待 Windows Release 环境和用户对真实 sibling data Pipeline graph 的视觉复核；不把历史 Windows evidence 延伸到本次新图形代码。
- Linux Viewer runtime-closure remediation (2026-08-26): root cause was Qt's `libQt6Core.so.6` carrying a local `RUNPATH=$ORIGIN` while its ICU `NEEDED` entries were only present in the Conan package; the executable's Conan search path is not inherited for that transitive lookup. `cmake/KSpaceJetStageLinuxRuntime.cmake` now resolves the viewer and required `qxcb`/`qoffscreen` plugin closure only from approved Conan/Intel roots, copies it with `FOLLOW_SYMLINK_CHAIN`, and rejects unresolved, unowned or divergent dependency candidates. `ksj_deploy_qt_widgets_runtime()` writes `bin/qt.conf` for the shared `bin`/`lib`/`plugins` layout; installation uses the same closure policy. `tools/devenv/linux/run.sh cmake --preset linux-release` exit 0; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0; `just install-release-applications` exit 0. From both `out/build/linux-release/bin` and `out/install/linux-release/bin`, `env -u LD_LIBRARY_PATH -u QT_PLUGIN_PATH -u ICU_DATA QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` exited 0 and emitted `{"schema":"ksj.viewer-ui-smoke","program":"ksj-viewer","ok":true,"status":"inspection"}`; `ldd ./ksj-viewer | rg 'not found'` and `ldd plugins/platforms/libqxcb.so | rg 'not found'` had no output. The Linux-only `apps.viewer_runtime_smoke` CTest registration encodes this clean-environment offscreen assertion, but its separate app-test tree remains absent under the P0-010 bootstrap blocker.

- Container hierarchy label refinement (2026-08-26): user visual review showed that `[RAW] /dataset_1 - 1842 acquisition(s), 0 image series` repeats information already exposed by semantic children and the General inspector, while consuming the narrow navigation pane. `ViewerWindow::rebuild_dataset_navigation()` now renders only the verified container path (for example `/dataset_1`); its tooltip and `Qt::AccessibleDescriptionRole` retain acquisition-record, image-series, XML-header and waveform facts, while `Qt::AccessibleTextRole` preserves the path. The widget corpus asserts raw-only and raw-plus-image labels have no `[RAW]`/`[IMAGE]` prefix or inline count, and verifies the retained tooltip/accessibility metadata. `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0; from `out/build/linux-release/bin`, clean-environment `QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` exited 0 with `ok:true`; `git diff --check`, plan check and `just link-check` exit 0. The separate Linux unit/app-test tree is absent under the recorded P0-010 bootstrap blocker, so the newly added widget test is source-reviewed but not executed here; Windows focused CTest and real-data visual review remain required.

- ArrShow-style Image/K-space canvas refinement (Linux application/source, 2026-08-26): 用户要求以 `tsumpf/arrShow` 的 compact controls → dark canvas → pixel probe 信息层级及 array gestures 作为本地 C++/Qt Viewer 参考；未复制 MATLAB source/assets，未引入 MATLAB/Python/QML/WebEngine。新增 app-local `InspectionCanvas`，它只持有有界 `QImage` display derivative，并提供 crosshair/probe、fit、cursor-centred 1.5× zoom、pan、wheel/`+`/`-` frame stepping、middle-button C/W 和 double-click reset。Image source value 只在 `InspectionReader::with_image_pixels()` callback 内重新投影为 Magnitude/Real/Imaginary/Phase/Complex colour；phase 固定 `[-π, π]`，complex colour 为 phase hue × magnitude brightness。K-space 仍只显示 raw Cartesian `log10(1 + RMS magnitude)`；C/W 仅 remap 已有 8-bit QImage derivative，RSS 不创建 fake phase。新增 focused canvas/component/widget corpus 与 test-target CMake source registration。`tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/inspection_canvas.cpp apps/kspacejet-viewer/src/inspection_canvas.hpp apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_theme.cpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0；`tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0；在 `out/build/linux-release/bin`，`env -u LD_LIBRARY_PATH -u QT_PLUGIN_PATH -u ICU_DATA QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` 与 `--export-smoke --format json` 均 exit 0，分别返回 `ok:true` 与 `artifact_kind:"visualization-derivative"`。诊断 `find out/build -mindepth 1 -maxdepth 1 -type d -printf '%f\\n'` 仅有 `linux-release`，故不把新 tests 记为已执行；P0-010 阻止新 Linux test tree，Windows Release test tree/真实 sibling-data visual review 仍缺失，P8-004 保持 BLOCKED。

### 13.25 P8-004 ArrShow shared-display direct-replacement evidence

- ArrShow shared-display direct replacement (Linux application/source, 2026-08-26): Earlier canvas-refinement evidence described a superseded log/RSS-derived K-space presentation. The current implementation ports the applicable behavior of local `~/workspace/arrShow` `asCmplxChooserClass`, `asWindowingClass`, `complex2rgb`, and `martin_phase` into app-local `ArrShowDisplay`: Complex source defaults to Complex; Magnitude/Real/Imaginary use Gray(256); Complex/Phase use `martin_phase(256)`; Complex evaluates pure phase from raw magnitude and retains `complex2rgb`'s `Cmin` clamp / `Cmax` divide brightness rule. Degree/radian phase conversion preserves absolute C/W, normal/phase C/W are independent, and all three arrShow cross-plane policies are present: reset per plane, keep relative, keep absolute. `InspectionCanvas` uses display-pixel-normalized middle drag with arrShow's 4× width factor and live redraw; right-click/double-click reset, zoom, pan, frame step, and probe consume only a bounded `QImage` derivative. Image and K-space use the same renderer. K-space is one selected raw coil's Cartesian complex plane; only repeated/downsampled cells use disclosed complex arithmetic means, with no RSS, log, FFT, gridding, or reconstruction. `THIRD_PARTY_NOTICES.md` retains Biomedizinische NMR Forschungs GmbH / Tilman Johannes Sumpf attribution and the complete Boost Software License 1.0; README and the InspectionReader architecture note were replaced accordingly. Focused source corpus covers Complex/default/fallback, raw-magnitude pure-phase percentile regression, degree↔radian 360°/2π parity, normal/phase independence, all three C/W policies, raw RGB provenance, right-click reset, and raw single-coil K-space; it was not run. Validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_display.cpp apps/kspacejet-viewer/src/arrshow_display.hpp apps/kspacejet-viewer/src/inspection_canvas.cpp apps/kspacejet-viewer/src/inspection_canvas.hpp apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0 (only CMake runtime-staging ICU search warnings); from `out/build/linux-release/bin`, `env -u LD_LIBRARY_PATH -u QT_PLUGIN_PATH -u ICU_DATA QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` exit 0 with `ok:true`, and `--export-smoke --format json` exit 0 with `artifact_kind:"visualization-derivative"`; `just link-check` exit 0 (77 files / 173 local links) and `git diff --check` exit 0. `find out/build -mindepth 1 -maxdepth 1 -type d -printf '%f\\n'` outputs only `linux-release`; P0-010 prohibits a new Linux unit/app-test tree, and Windows Release plus real-data visual review remain unavailable. Therefore P8-004 is BLOCKED: source/static corpus and Linux application smoke are not substituted for Windows focused CTest or actual visual acceptance.

### 13.26 P8-004 automatic phase-read-plane evidence

- Automatic phase-read-plane replacement (Linux application/source, 2026-08-26): Per user direction, K-space no longer exposes a Frame, a reference-acquisition ordinal, a selected acquisition header, or normal-wheel record stepping. `CartesianKspaceRequest` now contains only a selected raw coil and app-local display settings. Header-only discovery finds the first renderable standard Cartesian plane in storage order: its XML encoding declares Cartesian, its raw header has zero trajectory dimensions, and its readout has nonempty valid samples around `center_sample`. The selected plane holds `encoding_space_ref`, `idx.kspace_encode_step_2`, `idx.average`, `idx.slice`, `idx.contrast`, `idx.phase`, `idx.repetition`, `idx.set`, `idx.segment`, and every `idx.user[]` counter fixed internally; `idx.kspace_encode_step_1` is intentionally omitted because it is the y phase-encode axis. The resulting grid is x = valid `sample - center_sample` readout and y = `idx.kspace_encode_step_1`; `idx.phase` remains a cardiac/physiological counter and `idx.kspace_encode_step_2` remains a 3-D partition counter. K-space opens/inspects the automatic phase-read plane, exposes only raw-coil and arrShow display choices, and reports axes plus fixed plane indices in its bounded display-derivative details. `README`, the InspectionReader architecture note, and the focused presentation/widget corpus now assert this behavior, including the absence of the former acquisition widgets and no normal-wheel K-space change. No raw payload is retained outside the bounded callback.
- Validation and BLOCKED state (2026-08-26): `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_display.cpp apps/kspacejet-viewer/src/arrshow_display.hpp apps/kspacejet-viewer/src/inspection_canvas.cpp apps/kspacejet-viewer/src/inspection_canvas.hpp apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0; `git diff --check` exit 0; `just link-check` exit 0 (77 files / 173 local links); and `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0. The CMake runtime-staging diagnostic only warned that ICU dependencies were found in the approved Conan search directory. From `out/build/linux-release/bin`, clean-environment `QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` and `--export-smoke --format json` both exited 0 with `ok:true`; the export smoke reported `artifact_kind:"visualization-derivative"`. `find out/build -mindepth 1 -maxdepth 1 -type d -printf '%f\\n'` still reports only `linux-release`. P0-010 prohibits creating/running a Linux unit/app-test tree, Windows Release focused CTest is unavailable, and user real sibling-data visual review has not occurred. P8-004 therefore returns to BLOCKED rather than treating source corpus or Linux application smoke as acceptance.

### 13.27 P8-004 image-bearing auxiliary-acquisition repair evidence

- Root cause and scope (2026-08-26): the screenshot selected the real sibling `KSpaceJet-ismrmrd-data/datasets/zen-2d-cartesian-2025/cart_t1.mrd:/dataset_1`, not the imaging container. Its XML declares `kspace_encode_step_1 = 0..31`, while its 1,842 acquisition records are auxiliary: Noise (256), Navigation (1), and surface-coil correction (1,585). The former automatic candidate discovery treated that auxiliary storage as an imaging plane and therefore rejected its out-of-range auxiliary ky value against an imaging XML limit. `/dataset_2` remains the normal `t1_fl2d` imaging container.
- Direct replacement semantics: app-local `StandardAcquisitionFlag` uses ISMRMRD's one-based flag positions and excludes dummy, noise, calibration-only, navigation, phase-correction, high-performance/real-time feedback, surface-coil correction, and phase-stabilization records from the automatic image-bearing Cartesian plane. It explicitly retains `parallel_calibration_and_imaging`. The identical predicate is used in header discovery, bounded header aggregation, and the payload callback consistency check. Included image-bearing lines retain the XML `kspace_encode_step_1` hard bound; no limit was loosened and no raw acquisition view escapes its callback.
- Availability UI and regression corpus: auxiliary-only containers now have no raw-coil entries and disabled Render control; the central empty K-space canvas, summary, and plane metadata state the exact omitted auxiliary reasons and ask the user to select a container with imaging acquisitions. Inspect/Open As does not raise a modal render error when no image-bearing plane is available. The focused corpus adds auxiliary-before-imaging (including a deliberately out-of-bound Noise line), auxiliary-only, out-of-bound image-bearing, and widget availability tests; the corpus also asserts that combined calibration-and-imaging remains included.
- Linux evidence: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0. A no-tree `c++ -std=c++20 -fsyntax-only` check of `viewer_presentation_tests.cpp`, with the existing application/test target include contract, exit 0 (only pre-existing Qt deprecated-event-constructor warnings). `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exit 0; CMake only warned that ICU was found in its approved Conan search directory. From `out/build/linux-release/bin`, `env -u LD_LIBRARY_PATH -u QT_PLUGIN_PATH -u ICU_DATA QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` and the matching `--export-smoke --format json` both exit 0, returning respectively `ok:true` and `artifact_kind:"visualization-derivative"`.
- Real-data read-only verification: a temporary current-object probe opened the sibling file without writing it. `/dataset_1` returned `available=0`, `channels=0`, and `no image-bearing Cartesian phase-read acquisition plane; omitted 1842 auxiliary acquisition(s): noise measurement (256), navigation (1), surface-coil correction (1585). Select a container with imaging acquisitions.` Its presentation was empty. `/dataset_2` returned `available=1`, `channels=16`, then rendered `image=512x256`, `matching=256`, `omitted=0`. The temporary probe was removed after execution.
- Status: the source, docs, focused corpus, static syntax, existing Linux application build, clean-environment smoke, and real-data semantic check are complete. P0-010 still forbids creating/running a Linux unit/app-test tree, the Windows Release tree is unavailable, and the user has not visually reviewed the repaired real file; P8-004 therefore returns to `BLOCKED`, not `ACCEPTED`.

### 13.28 P8-004 selectable acquisition-type evidence

- Scope and replacement: the user required Noise, Navigation, coil-correction auxiliary data and formal imaging acquisitions to be selectable in K-space, with formal `Imaging data` as the default. This replaces only the 13.27 intermediate availability presentation that made an auxiliary-only container a terminal no-coil state; its root-cause analysis and Imaging-data XML-bound rule remain historical evidence. `CartesianKspaceAcquisitionKind` is now a standard-flag membership model. `Imaging data` includes records with no auxiliary membership and `parallel_calibration_and_imaging`; calibration-only is a separate `Parallel calibration` auxiliary type. Each auxiliary predicate is independent, so one record carrying Navigation and Surface-coil correction flags deliberately appears in both explicit choices and their counts may overlap.
- Delivered bounded UI/presentation behavior: `kspaceAcquisitionTypeSelector` precedes the raw-coil selector. It always starts at `Imaging data`, then lists the present standard auxiliary types with counts. Changing the selection clears the previous derivative/C/W plane state, rebuilds the matching raw-coil list, updates details, and automatically renders when a compatible Cartesian plane exists; it does not create a Frame or acquisition-ordinal choice. Header discovery, bounded aggregation, and payload consistency checks use the same selected-type predicate. Imaging keeps the XML `kspace_encode_step_1` limit as a hard error. Explicit auxiliary selections use their own observed ky minimum/maximum, avoiding the false rejection of valid auxiliary data outside the imaging limit.
- Focused corpus and static validation: synthetic fixtures cover default Imaging selection, `parallel_calibration_and_imaging`, individual Noise/Navigation/Surface-coil selections, a Navigation+Surface-coil multi-flag record, auxiliary observed ky outside the XML range, imaging ky outside the XML range, auxiliary-only default-unavailable behavior, and Qt selector auto-render/no-modal behavior. `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exited 0. A direct `c++ -std=c++20 -fsyntax-only` invocation with the existing application/test include contract also exited 0; it emitted only the pre-existing Qt deprecated `QMouseEvent` constructor warnings. P0-010 prohibits configuring or running a Linux test tree, so this does not claim focused CTest execution.
- Linux application/smoke validation: `cmake --build out/build/linux-release --target ksj-viewer -j2` exited 0; runtime staging only warned that ICU libraries were found in the approved Conan search directory. From `out/build/linux-release/bin`, both `env -u LD_LIBRARY_PATH -u QT_PLUGIN_PATH -u ICU_DATA QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` and the matching `--export-smoke --format json` exited 0, returning respectively `ok:true` and `artifact_kind:"visualization-derivative"`.
- Real-data read-only semantic verification: a temporary probe linked only against the already built Viewer presentation objects, opened the sibling `KSpaceJet-ismrmrd-data/datasets/zen-2d-cartesian-2025/cart_t1.mrd:/dataset_1`, and was removed afterwards. It reported `Imaging data count=0` with the explicit-selection diagnostic. It then rendered every present auxiliary choice without modifying the source: Noise measurement `count=256`, `channels=16`, `image=512x128`, `ky=0..127`; Navigation `count=1`, `channels=2`, `image=128x1`, `ky=31..31`; Real-time feedback `count=1`, `channels=2`, `image=128x1`, `ky=31..31`; Surface-coil correction `count=1586`, `channels=16`, `image=128x31`, `ky=1..31`. All four reports identified `observed_selected_acquisition_kind`; thus the original XML-limit failure path is not used for an explicitly selected auxiliary type. No raw MRI payload entered this repository.
- Status and next action: implementation, documentation, focused source corpus, format/static syntax, Linux application build/offscreen smoke, and sibling-data semantic proof are complete. P8-004 remains `BLOCKED`, not accepted: Windows Release focused CTest/UI/export smoke and user visual review of default Imaging plus explicit auxiliary switching on the real standard source remain required.

### 13.29 P8-004 raw-coil automatic-render regression evidence

- Root cause and replacement (2026-08-27): changing `kspaceCoilSelector` invoked the Qt `currentIndexChanged` handler, which prepared C/W state and cleared `kspace_presentation_`, but called only `refresh_kspace()`. The canvas therefore showed its intentional empty-state text, `Open K-space to render the automatic Cartesian phase-read plane.`, exactly as in the user report. The handler now clears stale availability state, resets an active C/W drag, and calls `load_kspace()` whenever the new raw-coil index is valid; only an invalid/empty selector follows the empty-state path. `refresh_kspace_controls()` already blocks programmatic selector changes with `QSignalBlocker`, so a Data-type refresh cannot recursively render while it rebuilds the coil list.
- Regression coverage: the existing automatic K-space widget test now switches its two-channel synthetic source from raw coil 0 to raw coil 1 and requires no modal error, a nonempty canvas, and summary text naming `raw coil 1`. The presentation corpus additionally renders raw coil 1 directly and verifies both `coil_channel = 1` and the independent raw sample value. This verifies selector event behavior and protects against accidentally reintroducing RSS/combined-coil rendering.
- Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` and `git diff --check` exited 0. A direct, no-tree `c++ -std=c++20 -fsyntax-only` invocation using the existing application/test include contract on `viewer_presentation_tests.cpp` exited 0, with only the pre-existing deprecated Qt `QMouseEvent` constructor warnings. `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj-viewer -j2` exited 0. Clean-environment `QT_QPA_PLATFORM=offscreen ./ksj-viewer --ui-smoke --format json` and the matching `--export-smoke --format json` exited 0 with respectively `ok:true` and `artifact_kind:"visualization-derivative"`.
- Real-data interaction proof: a temporary offscreen Qt probe, removed after execution, opened sibling `KSpaceJet-ismrmrd-data/datasets/zen-2d-cartesian-2025/cart_t1.mrd`, selected `/dataset_2`'s K-space semantic object, and changed the actual `kspaceCoilSelector` from raw coil 0 to raw coil 11. It verified 16 selectable coils, no modal widget, selector data 11, and resulting summary text `raw coil 11`; output was `coil-switch-ui-probe: coil=11 auto-rendered`. A separate presentation-layer read-only probe rendered every `/dataset_2` Imaging coil 0 through 15 successfully as a 512 x 256 / 256-line plane. No raw payload was written to KSpaceJet.
- Probe tooling note: the first manual link invocation for the temporary Qt probe omitted the staged `libpugixml.so.1` transitive dependency of `libismrmrd.so` and failed before running the probe. Repeating the same read-only probe with that staged library linked successfully as reported above; this was a probe-command dependency correction, not a Viewer source or data failure.
- Status: the reported regression is repaired, but `P8-004` remains `BLOCKED`, not accepted: P0-010 still prohibits a Linux unit/app-test tree, the Windows Release focused CTest/UI/export smoke is unavailable, and user visual acceptance remains required.

### 13.30 P8-004 renderable-default and post-render-selection evidence

- Root cause and scope (2026-08-27): the screenshot's simultaneous empty canvas, default summary, and one-row Status metadata cannot be a dark/black image or `InspectionCanvas` paint defect. `load_kspace()` writes its success Info record only after it commits and refreshes a nonempty presentation. The exact empty text can only follow a later selector clear, source/container clear, or render failure; a render failure would retain an error diagnostic/modal, which the screenshot lacks. Read-only offscreen probes of the current Linux build cannot reproduce the state for `/dataset_2` coils 0, 1, 10, or 11. The screenshot source is under a separate `/home/qiwen/Workspace` checkout whereas this build is under `/home/administrator/workspace`; the observed state exactly matches the pre-13.29 coil handler and is therefore treated as a stale/different executable symptom, while the source hardening below prevents selector re-entry from clearing a successful presentation.
- Delivered selection transaction: Data and raw-coil `currentIndexChanged` handlers now ignore invalid/reentrant events, use `QScopedValueRollback` around selector refresh, reset only C/W drag state, and call `load_kspace()` without pre-clearing `kspace_presentation_`. A successful render atomically replaces the canvas, summary, plane metadata and export state; a transient invalid selector signal cannot erase the last valid derivative. The existing `QSignalBlocker` remains around programmatic Data/coil list rebuilds.
- Delivered renderable type/default semantics: header-only candidate discovery now groups each selected standard-flag type by its fixed Cartesian plane indices, validates the full bounded candidate (Cartesian XML/zero trajectory dimensions, consistent active coils, usable readout, XML Imaging ky eligibility, source-line/value limits), and chooses the first valid plane in storage order. The Data list omits non-renderable and zero-count types. It prefers renderable Imaging; if no Imaging plane survives, the first stable renderable auxiliary type becomes the actual default. An invalid Imaging plane is not a reason to reject a later independent valid Imaging plane. Auxiliary types still use observed ky bounds; no XML Imaging limit was broadened.
- Regression corpus/docs: widget coverage now drains posted events and requires canvas property/pixmap, summary, plane table and export state to remain coherent through coil 0 → 1 → 0 → 1. The auxiliary-only widget case now verifies automatic Noise default, render, Noise → Surface → Noise switching and no modal. Presentation coverage verifies auxiliary-only options exclude Imaging (0), and a synthetic first-invalid/later-valid Imaging-plane fixture verifies selection of the later valid segment. The Viewer README and inspection-reader architecture replace the obsolete empty-Imaging default wording.
- Linux/source validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exit 0. The direct no-tree `c++ -std=c++20 -fsyntax-only` test-source check with the existing target include contract exit 0; only pre-existing deprecated Qt `QMouseEvent` constructor warnings were emitted. `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 2` exit 0; runtime staging emitted only approved-Conan ICU lookup warnings. Clean-environment offscreen `ksj-viewer --ui-smoke --format json` and `--export-smoke --format json` both returned `ok:true`, with the latter reporting `artifact_kind:"visualization-derivative"`. `just link-check` passed (77 files / 173 local links) and `git diff --check` passed.
- Real-data read-only validation: removed temporary presentation and Qt widget probes opened sibling `KSpaceJet-ismrmrd-data/datasets/zen-2d-cartesian-2025/cart_t1.mrd` without modifying it. The presentation probe reported `/dataset_1` `default-kind=1 options=4 image=512x128` (Noise measurement); the widget probe reported `dataset_1-default=Noise measurement (256) canvas=512x128`, then selected `/dataset_2` raw coil 10 and reported `dataset_2-coil=10 canvas=512x256`. Both temporary source/binaries were deleted afterward. No raw MRI payload entered KSpaceJet.
- Status: source, docs, static source corpus, Linux application build/smoke, and sibling-data interaction evidence are present. `P8-004` returns to `BLOCKED`, not `ACCEPTED`: the next required checks are Windows Release focused `ksj_viewer_presentation_tests` CTest plus Viewer UI/export smoke and user visual review with the rebuilt checkout. P0-010 still prohibits a Linux unit/app-test tree, so this record does not claim focused CTest execution or acceptance.

### 13.31 P0-005 application diagnostic-logging regression evidence

- State and scope (2026-08-27): the user required every application below `apps/` to record diagnostics through `libs/core` logging. A read-only audit verified that the common `ksj_add_application()` already privately links all five executables to `KSpaceJet::core`, and every `main()` already calls `ConfigureDefaultConsole()` before parsing argv. The audit also found seven text-format CLI failure responses that were not accompanied by a core diagnostic, plus raw Qt framework messages from Viewer that bypassed the logger. This invalidated the app-facing portion of the prior `P0-005` observability evidence, so its status changed `ACCEPTED → REOPENED → IN_PROGRESS → VERIFYING` before implementation.
- Delivered surface: `ksj`, `ksj-gateway`, `ksj-recon`, `ksj-research`, and `ksj-viewer` now record every handled CLI failure exactly once through `KSJ_LOG_WARN` or `KSJ_LOG_ERROR`; `ksj-recon` distinguishes invalid requests from reconstruction failures. Direct `std::cerr` rendering was removed from `apps/`: text-mode failures are the core logger's plain-text stderr diagnostic, while JSON remains exactly one object on stdout. Viewer now relays `QtDebugMsg`/`QtInfoMsg`/`QtWarningMsg`/`QtCriticalMsg`/`QtFatalMsg` to matching core levels before `QApplication` construction, and its MRD/pipeline/container/K-space/image/export warning or error dialogs use one bounded GUI-to-core diagnostic helper. The information pane remains a bounded UI history rather than a second logger.
- Regression coverage: new `tests/apps/application_logging_contract_tests.py` verifies shared core linkage, five entry-point header/configuration/startup contracts, the Viewer Qt relay, Viewer GUI warning helper, and absence of direct spdlog/Qt logging/C-stdio/syslog bypasses in `apps/`. `application_json_protocol_tests.py` now requires INFO startup and WARN failure records on stderr while continuing to parse only stdout JSON; it adds Viewer mutually-exclusive smoke-flag coverage. `tests/apps/CMakeLists.txt` registers the static contract as `apps.logging_contract` for the independent app-test tree.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-cli/main.cpp apps/kspacejet-gateway/main.cpp apps/kspacejet-recon/main.cpp apps/kspacejet-research/main.cpp apps/kspacejet-viewer/main.cpp apps/kspacejet-viewer/src/viewer_window.cpp` exit 0; `tools/devenv/linux/run.sh cmake-format --check apps/CMakeLists.txt tests/apps/CMakeLists.txt` exit 0; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_cli ksj_gateway ksj_recon ksj_research ksj_viewer --parallel 4` exit 0 (only existing approved-Conan ICU lookup warnings); `python3 -B tests/apps/application_logging_contract_tests.py` and `python3 -B tests/apps/application_json_protocol_tests.py` against the five build-tree executables exit 0; clean-environment offscreen Viewer UI/export smoke both exit 0 with their JSON contract. An intentional `QT_QPA_PLATFORM=ksj-invalid` Viewer launch exited 134 after correctly emitting core `[WARN] Qt [qt.qpa.plugin]` and `[CRITICAL] Qt [default]` records, proving that framework diagnostics no longer bypass the core logger. `just link-check` exit 0 (77 files / 173 local links) and `git diff --check` exit 0.
- BLOCKED acceptance condition: the required independent `linux-release-app-tests` tree is absent. `P0-010` remains BLOCKED because the user has not completed the authenticated `sudo -v`, `just prepare-release`, `just build-release-applications`, and `just check` sequence; repository rules prohibit creating/running a replacement Linux app-test tree. Therefore the new CTest registrations and all-app protocol execution have only direct-script evidence and cannot restore P0-005 to ACCEPTED. The exact unblock action is recorded in the P0-005 ledger row; no product data, external service, ABI, schema, JSON stdout shape, or Qt UI scope changed.

### 13.32 P8-004 Windows native-GUI subsystem evidence

- Scope and decision (2026-08-27): the user clarified that `ksj-viewer` is a desktop viewer, not a command-line application, so launching `ksj-viewer.exe` from Windows Explorer must not create a console window. This supersedes the obsolete AC-VWR-001 prohibition on `WIN32_EXECUTABLE`; the acceptance criterion now requires the Windows GUI subsystem while retaining terminal-driven help/version/UI/export automation.
- Delivered implementation: only `apps/kspacejet-viewer/CMakeLists.txt` sets `WIN32_EXECUTABLE TRUE` for `ksj_viewer`, inside `if(WIN32)`. The shared `ksj_add_application()` and the other four application targets remain unchanged. The existing `Qt6::Core` dependency supplies Qt's Windows entry-point bridge, so the source retains `int main(int, char**)`; no custom `WinMain`, linker override, console allocation, or platform-specific runtime code was added. The fallback core logger continues to use stderr and does not allocate a Windows console; GUI dialogs remain the normal direct user-facing error path.
- Contract/docs: new `tests/apps/viewer_windows_gui_contract_tests.py`, registered as `apps.viewer_windows_gui_contract`, prevents the target property from being removed or broadened into a directory-wide `CMAKE_WIN32_EXECUTABLE` default. The Viewer README now distinguishes Explorer launch (no console) from a PowerShell/cmd launch (inherited stdout/stderr for smoke automation). AC-VWR-001 is the single normative requirement for this behavior.
- Performed Linux validation: `tools/devenv/linux/run.sh cmake-format --check apps/kspacejet-viewer/CMakeLists.txt tests/apps/CMakeLists.txt`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; and clean-environment `QT_QPA_PLATFORM=offscreen` Viewer `--ui-smoke --format json` plus `--export-smoke --format json` all exited 0. `git diff --check` and `just link-check` also passed.
- BLOCKED acceptance condition: this Linux checkout has no Windows Release build tree, so it cannot inspect the PE subsystem or reproduce Explorer behavior. The required Windows evidence is a rebuilt `ksj_viewer`, `dumpbin /headers <ksj-viewer.exe>` showing `Windows GUI`, both smoke commands from an existing terminal retaining their documented stdout/stderr results, and user visual confirmation that an Explorer launch creates no black console window. Linux CMake/source checks do not substitute for that evidence.

### 13.33 P8-004 observed ISMRMRD K-space plane-coordinate browsing evidence

- Scope and terminology (2026-08-27): the user correctly identified that one displayed raw Cartesian plane is not sufficient for a scan with multiple standard acquisition-index values. An acquisition is a record stream, not a Viewer `Frame`. The display axes remain `readout × idx.kspace_encode_step_1`; `encoding_space_ref`, `idx.kspace_encode_step_2`, `idx.average`, `idx.slice`, `idx.contrast`, `idx.phase`, `idx.repetition`, `idx.set`, `idx.segment`, and `idx.user[0..7]` identify the selected plane coordinate. `average` and `contrast` retain their standard names: NEX/NSA and echo are possible protocol conventions, not new Viewer or ISMRMRD fields. `idx.phase` is explicitly labelled physiological rather than phase-encode.
- Delivered bounded selection: `CartesianKspacePlaneIndex` and the header-only `cartesian_kspace_plane_options()` catalog expose only observed, fully validated renderable coordinate tuples, in deterministic first-source order. XML `encodingLimits` remain eligibility bounds and never create synthetic combinations. A requested tuple is carried through `CartesianKspaceRequest`, raw-coil availability is checked for that tuple, and existing JSON/CSV/PNG/SVG visualization-derivative provenance retains the selected `plane_indices`. The source never retains acquisition payload views outside callbacks.
- Delivered interaction: the K-space `Plane indices` selector has its own compact control row and presents the observed tuple catalog, with a complete standard-field tooltip and full coordinate table (including `segment` and all `user` values). Changing coordinate rebuilds the raw-coil list for that coordinate, preserves a still-valid coil, and rerenders transactionally. Normal wheel and `+`/`-` cycle coordinate tuples with wraparound; `Ctrl`+wheel remains canvas zoom. The shared callback was renamed `BrowseStepCallback`, so Image still steps image ordinals while K-space steps coordinates; neither exposes a source-record ordinal or a Viewer Frame control.
- Regression corpus: the existing temporary synthetic Cartesian MRD now has ten valid observed coordinates plus one XML-invalid encoding reference. Presentation coverage requires the catalog to exclude the invalid reference, preserve source order and active-coil counts, render explicit `slice=1` and `user[7]=1` coordinates, and reject the unobserved `average=1,slice=1` Cartesian-product combination. Widget coverage opens the same source, verifies `kspacePlaneSelector`, selects Slice 1, verifies its table/summary provenance, advances normal wheel to the next coordinate, and verifies Ctrl+wheel leaves the coordinate unchanged while zooming. The source-only test check passed; P0-010 prevents its focused Linux CTest execution.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/inspection_canvas.cpp apps/kspacejet-viewer/src/inspection_canvas.hpp apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; a direct no-tree `c++ -std=c++20 -fsyntax-only` invocation for `viewer_presentation_tests.cpp` using the existing application/test include contract; and `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` all exited 0. The syntax check emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings. Clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json` both returned `ok:true`; export reported `artifact_kind:"visualization-derivative"`. `python3 -B tests/apps/application_logging_contract_tests.py`, `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`, and `python3 -B tests/apps/application_json_protocol_tests.py` against all five Linux build-tree applications exited 0. `just link-check` passed (77 files / 173 local links), as did `git diff --check`.
- Status: `P8-004` moved `IN_PROGRESS → VERIFYING → BLOCKED`. The implementation, source corpus, product Linux build, static checks, and offscreen smoke are present, but P0-010 still forbids an independent Linux unit/app-test tree and this checkout has no Windows Release tree. Required unblock evidence remains a Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review on an actual standard MRD source. This record does not claim acceptance, Windows behavior, or clinical/reconstruction capability.

### 13.34 P8-004 arrShow-style observed-dimension controls evidence

- Scope and reference (2026-08-27): the user rejected the long observed-coordinate combination selector and required the K-space controls to match the local `~/workspace/arrShow` arrangement. The local reference establishes a horizontal run of compact four-cell value changers—`+`, current value, `−`, dimension extent—with the first two dimensions displayed as fixed `:` axes; value endpoints do not wrap, normal wheel changes the active dimension, Left/Right selects the active dimension, and Ctrl+wheel zooms. This implementation reads that reference only; it adds no MATLAB runtime, arrShow source/assets, workspace mutation, FFT, crop, ROI, coil-combine, Python/QML/Quick/WebEngine, reconstruction, Provider/gateway, or MRI artifact surface.
- Delivered presentation: new app-local `ArrShowDimensionStrip` independently implements that compact Qt layout. K-space permanently fixes the first two controls as **Readout** and **Phase encode** `:` axes; once rendered, their fourth cells show the raw source-grid extents. Only globally varying, actually observed non-axis standard coordinates receive changing controls. Their visible fourth cell is the count of actual observed values, never an XML-derived maximum. The legacy `kspacePlaneSelector` is removed. Tooltips and the metadata table use readable **Encoding space**, **Partition**, **Average**, **Slice**, **Contrast**, **Physiological phase**, **Repetition**, **Set**, **Segment**, and **User** names; user-visible K-space strings contain no `idx.` implementation prefix.
- Sparse-coordinate and coil semantics: each control offers real catalog values only. A requested value deterministically selects an actual compatible catalog tuple by maximizing unchanged remaining coordinates and retaining catalog order as the tie-breaker, then atomically refreshes all value changers. Thus `Slice=1` followed by `Average=1` selects the real `Average=1, Slice=0` tuple rather than inventing `Average=1, Slice=1`. The raw-coil list is rebuilt for that tuple, preserves the selected coil when still available, otherwise falls back to raw coil 0; raw payload ownership, selected acquisition type, raw Cartesian readout × phase-encode axes, C/W policy, and no-FFT/RSS/reconstruction semantics remain unchanged.
- Delivered interaction and corpus: `+`/`−`, Up/Down, PageUp/PageDown, direct value entry, and normal wheel operate only on the active observed dimension and stop at its actual bounds; canvas Left/Right changes active dimension; Ctrl+wheel continues to zoom. The focused widget corpus now requires the two fixed axes and raw extents, every available variable changer, absence of old selector/Frame/visible `idx.`, endpoint non-wrapping, sparse-tuple synchronization, raw-coil preservation, normal-wheel stepping, Ctrl+wheel isolation, and Right-plus-wheel active-dimension selection. The generic `InspectionCanvas` corpus separately proves the new Left/Right callback.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/inspection_canvas.cpp apps/kspacejet-viewer/src/inspection_canvas.hpp apps/kspacejet-viewer/src/viewer_presentation.cpp apps/kspacejet-viewer/src/viewer_presentation.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp` exited 0. `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4` exited 0; runtime staging emitted only approved-Conan ICU lookup warnings. Clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json` both returned `ok:true`, the latter with `artifact_kind:"visualization-derivative"`. `python3 -B tests/apps/application_logging_contract_tests.py`, `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`, and `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer` all exited 0. `just link-check` passed (77 files / 173 local links), and `git diff --check` passed.
- Direct no-tree syntax command (exit 0; only pre-existing deprecated Qt `QMouseEvent` constructor warnings): `tools/devenv/linux/run.sh c++ -std=c++20 -fsyntax-only -fPIC -O3 -DNDEBUG -g -DFMT_SHARED -DKSJ_MEMORY_ENABLE_LEAK_TRACKING=0 -DKSJ_MEMORY_ENABLE_STATS=0 -DKSJ_MEMORY_HAVE_LIBNUMA=1 -DKSJ_PERFORMANCE_LEVEL=0 -DQT_CORE_LIB -DQT_GUI_LIB -DQT_NO_DEBUG -DQT_WIDGETS_LIB -DSPDLOG_COMPILED_LIB -DSPDLOG_FMT_EXTERNAL -Iapps/kspacejet-viewer/src -Ilibs/core/kspacejet-base/include -Ilibs/core/kspacejet-config/include -Ilibs/core/kspacejet-logging/include -Ilibs/core/kspacejet-crash/include -Ilibs/core/kspacejet-platform/include -Ilibs/core/kspacejet-process-runtime/include -Ilibs/core/kspacejet-performance/include -Ilibs/core/kspacejet-threading/include -Ilibs/core/kspacejet-memory/include -Ilibs/io/kspacejet-ismrmrd/include -Ilibs/recon/kspacejet-recon-graph/include -Ilibs/recon/kspacejet-recon-model/include -I/home/administrator/.conan2/p/b/itkd1e3173e190a3/p/include/ITK-5.4 -I/home/administrator/.conan2/p/b/itkd1e3173e190a3/p/include -I/home/administrator/.conan2/p/b/hdf5a64837be8d78d/p/include/hdf5 -I/home/administrator/.conan2/p/b/ismrmbf56b8040fca7/p/include -isystem /home/administrator/.conan2/p/cli11266c95eaa99d1/p/include -isystem /home/administrator/.conan2/p/b/spdlo0ca3aff65fcb5/p/include -isystem /home/administrator/.conan2/p/b/fmtcc4c039ef681b/p/include -isystem /home/administrator/.conan2/p/b/libnua4ffd2a32ea30/p/include -isystem /home/administrator/.conan2/p/b/qtcab0280950188/p/include -isystem /home/administrator/.conan2/p/b/qtcab0280950188/p/include/QtCore -isystem /home/administrator/.conan2/p/b/qtcab0280950188/p/mkspecs/linux-g++ -isystem /home/administrator/.conan2/p/b/qtcab0280950188/p/include/QtGui -isystem /home/administrator/.conan2/p/b/qtcab0280950188/p/include/QtWidgets -isystem /home/administrator/.conan2/p/b/itkd1e3173e190a3/b/src/Modules/ThirdParty/GoogleTest/src/itkgoogletest/googletest/include tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`.
- Status: after the implementation, focused corpus, registration, documentation, product build, direct syntax validation, and offscreen smoke, `P8-004` transitioned `IN_PROGRESS → VERIFYING → BLOCKED`. `P0-010` prohibits creating/running the independent Linux unit/app-test tree, and this checkout has no Windows Release tree. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review on an actual standard MRD source. No Linux source/build/smoke evidence substitutes for those gates or claims acceptance.

### 13.35 P8-004 Raw-coil arrShow-dimension evidence

- Scope and direct replacement (2026-08-27): the user rejected a separate top-level Coil combo box. `kspaceCoilSelector` is removed. **Raw coil** is now the third arrShow-style control, directly after fixed **Readout** and **Phase encode** axes, rather than a separate selector. The selection state is owned by `ViewerWindow`, not by a transient widget; rendering therefore requests that raw channel directly from the selected real Cartesian plane option.
- Delivered behavior: `ArrShowDimensionSpec::always_visible` retains a meaningful dimension even when it has one observed value. For each selected plane, Raw coil exposes only `[0, active_channel_count)` with the same compact `+` / current value / `−` / extent layout and active-dimension behavior as the other arrShow controls. A single-channel plane visibly reports coil `0` and extent `1`, with stepping disabled. On Data type or sparse standard-coordinate selection, the Viewer preserves the selected raw coil when the next real plane supports it and deterministically clamps an unavailable channel to `0`; a valid Coil change rerenders the same selected coordinate using that channel. This does not add an acquisition record / Frame selector, synthetic coordinate, RSS, FFT, reconstruction, MATLAB runtime/source/assets, workspace mutation, Python/QML/Quick/WebEngine, Provider/gateway route, or MRI artifact.
- Focused corpus and docs: widget coverage asserts that the old combo is absent; Raw coil lives in `kspaceDimensions`; its endpoints, rendered raw-coil provenance, coordinate preservation, auxiliary-type refresh, and `0 ↔ 1` navigation work. A direct strip corpus additionally proves that an `always_visible` single-value coil remains visible with disabled buttons. The Viewer README and inspection-reader architecture note now describe Raw coil as the always-visible third arrShow dimension and document valid-channel preservation / fallback.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. The product build emitted only the pre-existing approved-Conan ICU staging lookup warnings; both clean-environment smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`. The fully expanded no-tree `c++ -std=c++20 -fsyntax-only` test command recorded in 13.34 was rerun after the new single-channel corpus and exited 0, with only the pre-existing deprecated Qt `QMouseEvent` constructor warnings.
- Status: `P8-004` transitioned `IN_PROGRESS → VERIFYING → BLOCKED`. This checkout still has only `out/build/linux-release`; P0-010 forbids creating/running a separate Linux unit/app-test tree and no Windows Release tree is available. Required unblock evidence is a Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of the Raw coil strip on a real standard MRD source. Linux source/build/smoke/static evidence does not claim or replace those gates.

### 13.36 P8-004 strict data-rank arrShow dimension-strip evidence

- Scope and reference (2026-08-27): the user clarified the governing rule: if data has N dimensions, the arrShow dimension strip must have N columns. The local `~/workspace/arrShow` reference derives its changers from `size(data)`, including singleton dimensions; its first two dimensions are `:` axes, and Left/Right traverses every changer. This is a read-only interaction reference only. No MATLAB runtime/source/assets, workspace mutation, FFT, crop, ROI, coil combine, reconstruction, Provider/gateway route, or MRI artifact was added.
- Delivered data-rank projection: every renderable Cartesian K-space view now has exactly 20 arrShow-style columns in stable semantic order: **Readout** `:`, **Phase encode** `:`, **Raw coil**, then **Encoding space**, **Partition**, **Average**, **Slice**, **Contrast**, **Physiological phase**, **Repetition**, **Set**, **Segment**, and **User 0** through **User 7**. The `Data` combo remains a selected ISMRMRD acquisition-flag-membership filter, not a tensor axis. A non-axis column's fourth cell is the cardinality of actual observed source values; it never uses an XML limit to invent extent or a Cartesian tuple. A singleton value such as `0 / 1` is therefore visible and disabled rather than silently omitted.
- Delivered interaction: `ArrShowDimensionStrip` now retains every valid supplied non-axis singleton and no longer needs a Raw-coil-only `always_visible` exception. All columns may become active; default selection is the first column, and Left/Right cycles the complete ordered strip—including both fixed axes and singleton fields—like arrShow. Normal wheel, `+`/`−`, Up/Down and PageUp/PageDown stay no-ops at a fixed/singleton dimension and retain the existing endpoint/no-wrap semantics for observed multi-value fields. Sparse selection still selects a real compatible tuple and synchronizes the remaining actual values; raw coil is still third, preserves a valid selected channel, and otherwise clamps to 0.
- Corpus and documentation: the direct strip corpus proves independently supplied singleton Coil and User 0 controls remain visible, disabled, selectable as active dimensions, and present in relative focus traversal. The K-space widget corpus now asserts all 20 Value/Label object names, validates every singleton `0 / 1` state, and rejects a silently elided field. `apps/kspacejet-viewer/README.md` and `docs/architecture/KSpaceJet_ismrmrd_inspection_reader.md` document the full-rank strip, sparse actual-value rule, and the fact that Data is a filter rather than a dimension.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` test command from 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. The build emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still forbids creating/running a Linux unit/app-test tree, and no Windows Release tree exists. The unblock evidence remains a Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of the complete strip on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates or claim Windows behavior, clinical use, reconstruction, or release qualification.

### 13.37 P8-004 observed-varying K-space dimension-strip correction

- Scope and product correction (2026-08-27): the user clarified that the K-space strip must not have a fixed number of columns. A column represents a currently real, switchable data axis, so a non-axis field is present only when its actual observed cardinality exceeds one. This supersedes 13.36's interpretation that every canonical ISMRMRD index field, including a singleton, must receive a column.
- Delivered selection shape: **Readout** `:` and **Phase encode** `:` remain fixed display axes. **Raw coil** enters the strip only when the selected real Cartesian plane has more than one raw channel. Each canonical standard index field—Encoding space, Partition, Average, Slice, Contrast, Physiological phase, Repetition, Set, Segment, and User 0–7—enters only when its selected-Data renderable catalog contains multiple actual observed values. A singleton is still present in the selected plane metadata but does not consume a compact changer column. `Data` remains a standard acquisition-flag-membership filter, not a tensor axis.
- Delivered interaction: the generic strip now filters non-axis specs with fewer than two selectable values, and the K-space builder does the same before publishing Raw coil or an index field. Sparse selection, source-order tie-breaking, valid-coil preservation/fallback, endpoint no-wrap, raw Cartesian complex display, and no Frame/source-record/synthetic Cartesian product semantics are unchanged. Left/Right cycles every **displayed** column, including the two fixed axes; normal wheel changes only an active, displayed, navigable non-axis dimension.
- Corpus and documentation: the direct strip corpus now proves singleton-only Coil/User fields create no strip control, while a two-channel Coil appears and is the only non-axis control traversed. The synthetic Cartesian widget corpus asserts its dynamic 12-column shape—two display axes, two-channel Coil, and its nine genuinely varying standard fields—and asserts absent singleton Encoding space/User 0–6 controls. `apps/kspacejet-viewer/README.md` and `docs/architecture/KSpaceJet_ismrmrd_inspection_reader.md` now describe the observed-varying rule and retain the sparse actual-value/Data-filter boundaries.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` test command from 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. The build emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. Required unblock evidence remains the Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of dynamic dimensions on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.38 P8-004 K-space dimension top-abbreviation evidence

- Scope (2026-08-27): the user asked for a concise label at the top of every displayed K-space dimension changer, so a compact column is immediately identifiable without relying on its tooltip or the right-side metadata table. The abbreviation must not alter the observed-varying dimension policy from 13.37, the raw Cartesian data/display semantics, or the no-Frame/source-record boundary.
- Delivered UI: `ArrShowDimensionSpec` now carries an abbreviation. Every displayed changer is a five-cell compact column: top abbreviation, `+`, current value, `−`, and observed-value extent. The top abbreviation is a clickable, accessible dimension activator; its tooltip begins with the full semantic name and continues with the existing full ISMRMRD meaning/observed-value navigation information. The numeric extent remains in the existing bottom cell.
- Stable mapping: **RO** = Readout, **PE** = Phase encode, **Co** = Raw coil, **Enc** = Encoding space, **Par** = Partition, **Avg** = Average, **Slc** = Slice, **Con** = Contrast, **Pha** = Physiological phase, **Rep** = Repetition, **Set**, **Seg** = Segment, and **U0** through **U7** = source-defined User counters. Only columns that 13.37 already makes visible receive one of these labels.
- Corpus and documentation: direct-strip coverage checks `Co`, its full Raw coil tooltip, and absence when Coil is singleton. The synthetic widget corpus checks each dynamically displayed `...Abbreviation` object, expected abbreviation/full tooltip, lack of `idx.`, and preservation of the existing `...Label` extent cell for RO/PE/Co/Par/Avg/Slc/Con/Pha/Rep/Set/Seg/U7. The Viewer README and inspection-reader architecture note document both the mapping and full-tooltip discoverability.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` test command from 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. The build emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. A follow-up `IN_PROGRESS` pass corrected only the stale internal four-cell comment to the actual five-cell shape and reran focused formatting/diff/plan checks before returning to the same blocker. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. Required unblock evidence remains the Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of dynamic dimensions and abbreviations on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.39 P8-004 structured XML presentation evidence

- Scope and user-facing correction (2026-08-27): the user found the plain one-column XML text preview difficult to browse. The XML contextual view remains available only after explicit inspection of a selected standard Header/XML semantic object. It is not promoted into a file dashboard, generic HDF5/XML browser, editor, serializer, source-data writer, or MRI artifact path.
- Delivered presentation: the XML tab now has a compact summary of the selected standard container, owned XML byte count, acquisition count, and image-series count; a left **Header structure** tree; and a right **Formatted XML** source pane. The source pane is read-only and syntax-highlights tags, attributes, quoted values, comments, and processing instructions. The outline shows XML hierarchy, attributes, and compact text values, expands two levels initially, and leaves normal tree scrolling/selection to Qt. The source file remains unchanged; formatting occurs only in the Viewer display.
- Boundedness and malformed-preview behavior: the outline accepts at most 2,048 displayed XML nodes and limits each displayed value to 120 characters. A deep/large preview is eligible for in-memory pretty-printing only through 64 nesting levels and 4,096 elements, avoiding indentation-amplified allocation. A preview exceeding either formatter limit, or an XML preview that cannot be parsed, remains in the original bounded source text; the outline reports a bounded truncation or parse-warning item instead of dereferencing or writing XML. The existing reader-owned XML limit remains the outer bound.
- Corpus and documentation: the focused Viewer widget corpus now explicitly inspects a Header/XML object and requires visible `metadataXmlSummary`, `metadataXmlOutline`, and read-only `metadataXmlPreview`, including the `ismrmrdHeader` outline root and selected-container summary. The Viewer README and ISMRMRD inspection-reader architecture document the contextual, bounded, presentation-only behavior.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command from 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. Required unblock evidence remains a Windows Release rebuild, focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of the XML presentation, dynamic dimensions, and abbreviations on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.40 P8-004 XML structure-width correction evidence

- Scope and user-facing correction (2026-08-27): visual review of 13.39 found the left **Header structure** pane too narrow for practical XML hierarchy and attribute-value inspection. This is a presentation-only geometry correction; it does not alter the selected standard container, owned bounded XML preview, parser/formatter limits, source XML, or any MRI artifact semantics.
- Delivered geometry: `metadataXmlSplitter` is horizontal, non-collapsible, and remains directly draggable. It now opens at 500 px for **Header structure** and 760 px for **Formatted XML**; the outline card cannot shrink below 460 px, the source card cannot shrink below 320 px, both have equal stretch priority, and the **Element** column initially receives 230 px. The user can still allocate more space to either side through the same divider.
- Corpus: the focused Viewer widget corpus identifies `metadataXmlOutlineCard` and asserts its 460 px minimum width alongside the existing visible summary/outline/read-only-preview coverage. This pins the default usability correction without turning the XML view into an editor or generic browser.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of the widened XML presentation on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.41 P8-004 XML default-structure mode-switch evidence

- Scope and user-facing correction (2026-08-27): the user clarified that the XML page should default to the useful **Header structure** presentation. **Formatted XML** is a separate way to inspect the same selected bounded header, rather than a second panel that permanently consumes horizontal space. This replaces the side-by-side presentation geometry introduced in 13.39/13.40 only; it neither broadens nor changes the standard ISMRMRD inspection boundary.
- Delivered interaction: `metadataXmlViewModes` is a single Qt content-area tab control with **Header structure** at index 0 and **Formatted XML** at index 1. The former is selected by default and gives the structural outline and compact summary the full content width. Selecting the latter replaces it with the full-width, read-only, syntax-highlighted source preview. No splitter remains in this XML content view. The viewer does not write, serialize, or edit the source XML.
- Boundedness and corpus: both modes reuse the same owned bounded source preview, existing 2,048-node outline limit, per-value limit, and depth/element formatting eligibility checks. The focused widget corpus requires the two named modes, verifies index 0 is the default and only the outline is visible there, then switches to index 1 and verifies only the read-only source preview is visible. The Viewer README and inspection-reader architecture note now describe the default/alternate mode behavior explicitly.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console observation, and user visual review of XML mode switching on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.42 P8-004 default full-screen Viewer startup evidence

- Scope and user-facing correction (2026-08-27): the user requires the desktop Viewer to use the whole display by default. This is a Qt application presentation change only; it does not change file selection, standard ISMRMRD inspection semantics, raw-data ownership, logging policy, CLI JSON protocol, or visualization-derivative/MRI artifact boundaries.
- Delivered behavior: the sole normal Qt startup path in `apps/kspacejet-viewer/main.cpp` now constructs `ViewerWindow` and calls `window.showFullScreen()` instead of `window.show()`. The same path is intentionally used by `--ui-smoke` and `--export-smoke`; their existing zero-delay event-loop quit remains intact, so automation does not remain open. The existing Windows GUI-subsystem CMake target property remains the separate no-console boundary.
- Contract and documentation: the Viewer README states that the workbench opens full screen by default. `viewer_windows_gui_contract_tests.py` retains its target-level Windows GUI-subsystem assertions and additionally requires the main startup source to contain the `ViewerWindow` followed by `window.showFullScreen()` call, preventing an accidental return to windowed startup.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/main.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/main.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/full-screen observation, and user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.43 P8-004 Header structure Element-column width evidence

- Scope and user-facing correction (2026-08-27): after full-screen startup and the default Header structure mode made more room available, the user found the XML outline's **Element** column still too short for common nested standard ISMRMRD element names. This is only a default table geometry change; it does not alter XML parsing, bounds, hierarchy, source text, or any MRI data/artifact semantics.
- Delivered behavior: the first `metadataXmlOutline` header section now opens at 360 px rather than 230 px. It is explicitly `QHeaderView::Interactive`, so the user can still drag the Element/Value separator; the final Value section remains stretched to consume the rest of the full-width structure view.
- Corpus: the focused XML widget coverage now asserts both the 360 px initial Element column width and the `Interactive` resize mode, in addition to the default Header structure/Formatted XML mode-switch behavior. It thereby protects both readability and manual column adjustment without adding an editor or a generic XML browser.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/full-screen observation, and user visual review of the widened Element column on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.44 P8-004 maximized startup with native close controls evidence

- Scope and user-facing correction (2026-08-27): the literal full-screen startup introduced in 13.42 removed the operating system title bar and therefore the visible close control. The user's feedback makes the intended behavior unambiguous: fill the desktop work area by default while retaining native window controls. This supersedes only the startup-window presentation of 13.42.
- Delivered behavior: the unique Qt startup path now calls `window.showMaximized()` rather than `window.showFullScreen()`. The workbench starts maximized, with native minimize, maximize/restore, and close buttons available through the title bar. `--ui-smoke` and `--export-smoke` continue to use that same path and retain their zero-delay automatic quit.
- Contract and documentation: the Viewer README now distinguishes maximized startup from borderless full screen and explicitly promises preservation of native window controls. The existing Windows GUI-subsystem source contract continues to require the narrow target property and now requires `ViewerWindow` followed by `window.showMaximized()`, preventing regression to either windowed startup or titlebar-removing full screen.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/main.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/main.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.45 P8-004 XML Tree/XML Text presentation-label evidence

- Scope and user-facing correction (2026-08-27): the user clarified that the two tabs display the same selected XML rather than different content classes. **Header structure** inaccurately suggested only header/schema structure, **Outline** suggested a summary, and **Formatted XML** described an implementation detail rather than the text view itself.
- Delivered labels: the XML mode tabs, their section hint, and tooltip now read **XML Tree** and **XML Text**. XML Tree is the bounded hierarchy/value presentation; XML Text is the syntax-highlighted textual presentation of that same owned XML. The text view may normalize indentation solely for display, so it is explicitly not labelled raw/byte-identical XML. Both modes remain selected-object-only, read-only, bounded, and source-preserving.
- Corpus and documentation: the focused widget corpus asserts the exact XML Tree/XML Text tab names, their default/switch behavior, 360 px interactive Element section, and the existing read-only text state. The Viewer README and inspection-reader architecture note document the symmetric names and presentation-only indentation behavior.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of XML Tree/XML Text on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.46 P8-004 K-space automatic-refresh and view-reset control evidence

- Scope and user-facing correction (2026-08-27): the former **Render** button merely invoked `load_kspace()`, even though entering K-space and changing Data, an observed dimension, raw coil, component, phase representation, range, percentile, C/W persistence, or C/W values already refreshes the selected bounded Cartesian plane. It was a redundant manual action and made the automatic display workflow appear incomplete.
- Delivered behavior: the obsolete button, connection, enable-state handling, and `renderKspaceButton` object name are removed. The **View** group now has `kspaceViewResetButton` labelled **Reset**. It invokes `InspectionCanvas::fit_to_view()` and sets the zoom control to 100%, thereby clearing pan and restoring the view scale. The distinct `kspaceWindowResetButton` remains labelled **Reset C/W** and continues to restore only the active arrShow normal/phase window.
- Corpus and documentation: the focused Viewer widget corpus requires the new Reset control, verifies the old Render object is absent, verifies automatic rendering while stepping a coil/data choice, and verifies Reset restores zoom without changing explicit C/W values. The Viewer README now documents automatic refresh, the absence of a separate Render command, and the Image **Fit** versus K-space **Reset** labels.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of automatic refresh plus the Reset/Reset C/W distinction on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.47 P8-004 K-space single-canvas layout evidence

- Scope and user-facing correction (2026-08-27): the user rejected the permanent **K-space plane details** side panel because it duplicated the selected Data type, arrShow dimension strip, raw-coil selection, and display state while reducing the K-space canvas. Its diagnostic text—full coordinate, acquisition membership/counts, source bounds, and skipped records—does not belong in the routine viewing layout.
- Delivered layout: `kspaceViewSplitter`, `kspaceDetailsCard`, `kspaceSummary`, `kspacePlaneTable`, and their dedicated refresh routine are removed. `kspaceCanvasCard` is now the sole expanding K-space content card. The Data selector, observed-dimension strip, C/W controls, View/Reset controls, canvas interaction, bottom pixel probe, bounded display derivative, and export eligibility remain unchanged. `KspacePresentation::details` and `summary` remain app-owned bounded presentation/provenance data; they are not materialized as a permanent UI panel and JSON export still carries the selected coordinate.
- Corpus and documentation: focused widget coverage requires `kspaceCanvasCard` and requires the former splitter, details card, summary, and plane table to be absent while preserving automatic selected-coil/data rendering and the existing reset/C/W interaction coverage. The Viewer README now specifies that the full coordinate remains in JSON export provenance rather than a permanent side panel.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of the single-canvas K-space layout on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.48 P8-004 K-space two-row controls-layout evidence

- Scope and user-facing correction (2026-08-27): the one-row K-space controls made the arrShow observed-dimension strip appear as an isolated left rail, while Data/display controls, C/W persistence, C/W values, and View/Reset all competed for the same horizontal line. The user selected a two-row arrangement: display controls first, then the arrShow-style dimensions.
- Delivered layout: `kspaceControlsCard` now owns `kspaceDisplayControlsRow` followed by `kspaceDimensionsRow`. The first row retains Data, Display, Phase, Range, and percentile controls at the left; its flexible gap places a compact **C/W** group (persistence, C, W, Reset C/W) and **View** group (zoom, Reset) to the right. The separate full-width second row starts with the explicit **Dimensions** label and then the existing `kspaceDimensions` strip. The strip retains its exact observed-varying membership, concise column abbreviations, wheel/key navigation, raw-coil selection, sparse tuple synchronization, and no-Frame semantics. No selection, automatic refresh, C/W, zoom/pan, bounded derivative, or export behavior changed.
- Corpus and documentation: focused Viewer widget coverage now requires both named rows and the Dimensions label, asserts that each display/C/W/View control is parented by the first row, that the dimension strip is parented by the second row, and that the rows are vertically ordered after K-space inspection. The Viewer README documents the compact first control row and full-width Dimensions row.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of the two-row K-space controls on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.49 P8-004 native ComboBox and SpinBox affordance evidence

- Scope and user-facing correction (2026-08-27): visual review of the 13.48 K-space layout found an empty right-side button area on every ComboBox and effectively invisible SpinBox increment/decrement affordances. That made selectable and numeric controls look like non-interactive text fields, including Data, Display, Phase, Range, C/W persistence, percentile, C/W values, and View zoom.
- Root cause and delivered behavior: the Viewer already chooses the cross-platform Fusion Qt style and palette, but the stylesheet additionally styled `QComboBox`/`QAbstractSpinBox` as complex controls and styled only the ComboBox `drop-down` border/width. Qt requires all complex-control subcontrols to be styled when any one is overridden; the omitted arrow glyph therefore left a blank button area. The incomplete ComboBox/SpinBox root/focus/drop-down rules are removed. Fusion now owns the complete ComboBox arrow and SpinBox up/down button/arrow painting, including normal, hover, focus, disabled, hit-area, and high-DPI behavior. Popup list styling remains Viewer-owned. No data, display, C/W, keyboard, mouse, export, or two-row-layout semantics changed.
- Corpus: `KeepsNativeComboAndSpinAffordances` applies the Viewer theme, rejects reintroduction of the incomplete ComboBox/SpinBox subcontrol rules, and verifies through Qt `QStyle::subControlRect` that ComboBox arrow and both SpinBox button regions are non-empty, inside their controls, separate, and on the trailing half. It does not use brittle screenshot pixel checks.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/viewer_theme.cpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; syntax validation emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not ACCEPTED. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of visible ComboBox/SpinBox affordances and the two-row K-space controls on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.50 P8-004 default active dimension-column evidence

- Scope and user-facing correction (2026-08-27): a newly opened K-space arrShow dimension strip selected the first visible column, normally `RO`. The requested default is the third visible column when it exists, otherwise the second; for example `RO | PE | Co | Avg` must initially make `Co` active, while `RO | PE` must initially make `PE` active.
- Delivered behavior: `ArrShowDimensionStrip::set_dimensions()` now retains a valid caller-provided preferred identifier first and retains a valid existing user selection second. Only when neither exists does it choose visible control index 2 when at least three controls exist, index 1 for two controls, and index 0 for the safe one-column case. The observed-varying filter runs before that choice, so “third” and “second” mean actual displayed columns; it does not alter column membership/order, shortcuts, wheel/`+`/`−` stepping, raw-coil behavior, sparse tuple selection, C/W, rendering, or export.
- Corpus: `DefaultsTheActiveColumnToThirdThenSecond` directly covers four columns (`coil` is third), two (`phase-encode` is second), one (safe first fallback), explicit caller preference, and preservation of a still-valid user choice. The existing observed-varying corpus continues to cover hidden singleton non-axis dimensions.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34, repeated after the new corpus; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; and `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not `ACCEPTED`. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of the default dimension focus on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.51 P8-004 two-axis active-dimension correction evidence

- Scope and user-facing correction (2026-08-27): the user clarified that `RO | PE` already comprises the minimum two display dimensions for a raw Cartesian phase-read plane. It is not evidence of an additional dimension to browse, so the 13.50 rule that selected `PE` for exactly two visible columns was wrong.
- Delivered behavior: exactly two visible controls now clear the active identifier and remove `arrShowDimensionActive` from both display-axis controls. This takes precedence over a retained/caller-provided identifier. `set_active_dimension()` rejects two-column activation, and `focus_relative_dimension()` returns false without focus or state changes when no independent active dimension exists; wheel and `+`/`−` already no-op because the fixed display axes are not navigable. With three or more visible controls, the third remains the initial default and explicit preference/a valid existing user choice retain their prior behavior. The K-space canvas help text now omits Wheel/`+`/`−`/Left/Right dimension navigation when no active dimension exists.
- Corpus: `DefaultsTheActiveColumnToThirdAndClearsTheDisplayAxes` covers four columns (`coil` is the third default), two columns (empty active identifier, no step/focus traversal), a caller-provided `PE` preference that must still remain inactive, cleared visual state, one-column generic fallback, and a valid explicit/user selection with three columns.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh clang-format --dry-run --Werror apps/kspacejet-viewer/src/arrshow_dimension_controls.cpp apps/kspacejet-viewer/src/arrshow_dimension_controls.hpp apps/kspacejet-viewer/src/viewer_window.cpp apps/kspacejet-viewer/src/viewer_window.hpp apps/kspacejet-viewer/src/viewer_theme.cpp tests/unit/apps/kspacejet-viewer/viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34, repeated after the updated corpus; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; and `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only the pre-existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export reported `artifact_kind:"visualization-derivative"`.
- Status: after `IN_PROGRESS → VERIFYING`, `P8-004` returns to `BLOCKED`, not `ACCEPTED`. P0-010 still prohibits the independent Linux unit/app-test tree and no Windows Release tree exists. The required unblock remains a Windows Release rebuild with focused `ksj_viewer_presentation_tests` CTest, terminal UI/export smoke, PE `Windows GUI` subsystem inspection, Explorer no-console/maximized-window observation, and user visual review of the two-axis inactive policy on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.52 P8-004 arbitrary raw Cartesian display-axis evidence

- Scope and user-facing correction (2026-08-27): `:` is an arrShow display-axis role, not a permanent spelling for Readout and Phase encode. A user must be able to make any two observed raw Cartesian dimensions the X/Y plane; specifically, assigning `Co` as `:` and choosing `PE=0` must render the real `RO × Co` samples, not merely relabel the former `RO × PE` image.
- Delivered model: fixed `CartesianKspacePlaneIndex`/single-coil request state is replaced by bounded `CartesianKspaceDimension`, `CartesianKspaceCoordinate`, `CartesianKspaceAxes`, and header-only `CartesianKspaceCatalog` state. The catalog contains actual renderable sparse coordinates, centered readout ranges, and active-channel ranges only; it never retains payload views or derives a full Cartesian product. Exactly two distinct, validated axis roles are present. The lower arrShow extent cell assigns X with left click and Y with right click; a displaced role becomes an actual fixed coordinate, whose `+`/`-`, wheel, and keyboard navigation resolve to a compatible observed sparse tuple. `count`/unknown axes and duplicate axes fail deterministically.
- Raw projection semantics: Readout maps to `sample - center_sample`, Phase encode to `idx.kspace_encode_step_1`, Raw coil to payload channel, and every remaining selectable dimension to its standard ISMRMRD acquisition-header counter. The generic callback aggregation handles any pair of those axes, applies one fixed raw coil only when coil is not an axis, and takes a complex arithmetic mean only for repeated/bounded-downsampled cells. No FFT, gridding, RSS, log transform, reconstruction, record selector, or Frame selector is introduced. `RO × Co, PE=0` synthetic coverage proves the two coil rows come from raw samples with expected real means `4.5` and `6.0`.
- Coordinate and XML semantics: Imaging eligibility now validates every mapped XML `encodingLimits` counter—`kspace_encode_step_1`, `kspace_encode_step_2`, average, slice, contrast, phase, repetition, set, segment, and `user_0`–`user_7`—while readout, raw coil, and encoding-space reference are not falsely mapped to an XML limit. The phase-encode display axis preserves the XML imaging range or inclusive auxiliary observed range, leaving missing coordinates as empty bins rather than compressing them. Downsampled CSV X/Y coordinate ranges now use the same bin mapping as aggregation.
- Corpus and documentation: synthetic presentation tests cover default `RO × PE`, direct `RO × Co` at fixed `PE=0`, sparse coordinate resolution, invalid duplicate/sentinel axes, XML partition-limit rejection, phase gap preservation, and 2,049-to-2,048 readout downsampling coordinate bounds. The Qt workbench corpus sends the actual right-click to `Co`, asserts that `RO` and `Co` become `:`, `PE` becomes editable, steps it to `0`, and retains a real canvas derivative. `README.md` and the inspection-reader architecture describe arbitrary roles, raw field mapping, XML eligibility, and missing-phase-bin behavior.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over all changed Viewer C++ sources/tests; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the all-application `ksj_cli ksj_gateway ksj_recon ksj_research ksj_viewer` incremental build; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write` plus `--check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only deprecated Qt `QMouseEvent` constructor warnings. Both smokes returned `ok:true`; export reported `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still prohibits the independent Linux unit/app-test tree, and no Windows Release tree exists for the focused Viewer CTest, terminal smoke, PE GUI-subsystem inspection, Explorer no-console/maximized-window observation, or user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not substitute for those gates.

### 13.53 P8-004 shared dimension-column selection evidence

- Scope and user-facing rule (2026-08-27): the two visible `:` characters are membership tags, not permanent X/Y labels. For raw K-space, left-clicking a third extent cell replaces the first tag and right-clicking replaces the second; only after that replacement does the Viewer derive X and Y by scanning selected columns from left to right. Thus the mouse button chooses which existing membership to replace, while column order alone chooses geometry. The UI always keeps exactly two K-space members.
- Delivered shared foundation: `ArrShowDimensionSelectionTag`, `ArrShowDimensionSelection`, and `arrshow_plane_axes_in_column_order()` replace the former role-to-X/Y coupling. `ArrShowDimensionStrip` exposes the same tag and active-fixed-dimension mechanics to both surfaces; K-space normalizes the two valid observed members before resolving a sparse compatible coordinate, and its `CartesianKspaceAxes` are built from the shared column-order result. The focused corpus covers inverted tag colours still producing `RO × PE` by column order, plus a third-column right-click replacement producing `RO × Co`.
- Image restriction: standard image `[x, y, z, channel]` storage reuses the same strip and `ImageRequest`/presentation mapping, but its first native X and Y columns are permanently first/second `:` members with disabled extent cells. Every later varying image dimension is a fixed-coordinate selector: it may receive active focus and `+`/`-`/wheel navigation, but cannot become `:`. `make_image_presentation()` independently rejects a non-native X/Y plane, so the invariant does not rely only on disabled Qt controls. The focused widget corpus checks locked X/Y, rejects a Z colon attempt, and verifies Z browsing retains an image derivative.
- Documentation and validation: `apps/kspacejet-viewer/README.md` and `docs/architecture/KSpaceJet_ismrmrd_inspection_reader.md` now state the K-space replacement/column-order rule and fixed Image X/Y rule. Linux checks all exited 0: `tools/devenv/linux/run.sh clang-format --dry-run --Werror` over changed Viewer sources/tests; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; the direct `c++ -std=c++20 -fsyntax-only` Viewer test-source command; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; and `git diff --check`. The product build emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for the focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user real-source visual review. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.54 P8-004 blue/red tag-identity clarification evidence

- Scope and user-facing correction (2026-08-27): “first/second `:`” was ambiguous because it can be read as the first/second displayed column, while successive replacements may leave the two selected dimensions in columns three and four. The Viewer now calls the two stable arrShow identities **blue** and **red** tags. It never calls them X/Y or a column ordinal.
- Delivered interaction: left-clicking any K-space extent cell replaces whichever dimension currently carries the blue tag; right-clicking replaces the current red-tag dimension. For example, if Co/Avg are the current blue/red `:` dimensions in columns three/four, a left-click on RO produces RO/Avg membership. Only after that membership change does the shared `arrshow_plane_axes_in_column_order()` derive X/Y from the displayed columns. Thus RO/Avg renders as X=RO and Y=Avg, while tag colour remains solely a replacement target. Image retains its fixed native X/Y tags and does not expose a change path for either colour.
- Corpus/documentation: K-space tooltips and page help, README, and the inspection-reader contract now use blue/red language. The focused widget corpus asserts those tooltip labels and verifies the moved Co/Avg selection, blue replacement by RO, and subsequent RO × Avg column-order plane. The internal enum remains `first`/`second` only as a private storage name for the blue/red tags.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` plus `clang-format --dry-run --Werror` over changed Viewer sources/tests; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` of `viewer_presentation_tests.cpp`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `just link-check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids the independent Linux unit/app-test tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not replace those gates.

### 13.55 P8-004 remembered file-open-directory evidence

- Scope and behavior (2026-08-27): Open ISMRMRD and Open PipelineDefinition dialogs no longer receive an empty initial-directory argument after the Viewer has successfully opened a local file. `ViewerWindow` stores the parent directory of every successful MRD or PipelineDefinition open in explicit user-scoped `QSettings` (`KSpaceJet` / `ksj-viewer`, `file_dialogs/last_open_directory`), restores it at window construction when it still exists, and passes it to both Open dialogs. The File bar calls the same successful MRD path. A cancelled dialog, URL rejection, malformed/missing MRD, or invalid pipeline leaves the stored directory untouched.
- Bounded/scope properties: this is one small local UI preference only; it stores a directory string rather than source payload, HDF5 state, credentials, reconstruction settings, or display derivative. It does not make URLs valid, alter read-only source semantics, change the recent-files list contract, or modify source files. If the remembered directory no longer exists, Open falls back to the platform's default directory.
- Corpus/documentation: the focused Viewer corpus isolates `QSettings` to a temporary test user scope and covers successful MRD storage, a failed MRD preserving the old directory, and successful PipelineDefinition replacement. `apps/kspacejet-viewer/README.md` documents the shared MRD/Pipeline default and cross-restart persistence.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` plus `clang-format --dry-run --Werror` over `viewer_window.cpp`, `viewer_window.hpp`, and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` of the Viewer test source; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids the independent Linux unit/app-test tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not replace those gates.

### 13.56 P8-004 persistent five-entry recent-files evidence

- Scope and behavior (2026-08-27): the File bar's existing **Recent Files** picker is now a user-scoped, persistent MRU shared by local standard MRD and PipelineDefinition sources. It stores at most five successful opens under `KSpaceJet` / `ksj-viewer`, `file_dialogs/recent_files`. Every record holds only an absolute local path and its explicit `mrd` or `pipeline` kind. Reopening an already-recorded file moves it to the front without consuming another slot; the sixth distinct successful source evicts the oldest one. A failed/missing MRD or invalid/missing PipelineDefinition does not change the MRU.
- Restore/routing boundary: startup accepts only well-formed records with an `mrd` or `pipeline` kind whose local path is still a regular file, normalizes/deduplicates them in stored order, caps them at five, and persists the cleaned list. Each combo-box row keeps the path and type in Qt item roles, so choosing a PipelineDefinition invokes the Pipeline parser rather than treating its `.json` as an MRD source. The editable bar still treats text entered on Enter as a local MRD path; its tooltip makes that distinction explicit. This remains local/read-only, does not accept URLs, and does not retain source payload, HDF5 state, credentials, reconstruction settings, or display derivatives.
- Corpus/documentation: `ViewerFileDialogSettingsScope` isolates both open-directory and MRU settings in the focused widget corpus. `PersistsFiveTypedRecentFilesAfterSuccessfulOpensOnly` covers successful-only insertion, MRD/Pipeline `kind` values, persisted JSON order, duplicate MRD promotion, six-source truncation to five, item-role types, reconstruction of `ViewerWindow`, actual Pipeline entry activation, and stale-file pruning on the following startup. The existing directory test now clears both related setting keys. The Viewer README documents the five-entry limit, cross-restart restore, type-preserving reopen, and stale-entry pruning.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` plus `clang-format --dry-run --Werror` over `viewer_window.cpp`, `viewer_window.hpp`, and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` of the Viewer test source; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `just link-check`; `git diff --check`; and `python3 tools/checks/check_execution_plan.py --write` plus `--check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only the existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids the independent Linux unit/app-test tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. Linux source/build/smoke/static evidence does not replace those gates.

### 13.57 P8-004 File-menu recent-files presentation evidence

- Scope and user-facing correction (2026-08-28): recent source access is a File operation. The former File-bar **Recent Files** button and combo-box picker are removed. The file bar is now a plain local-ISMRMRD path field with Clear Text; it cannot accidentally present or open a PipelineDefinition as typed MRD text.
- Delivered behavior: **File → Recent Files** is a dynamic submenu immediately following Open ISMRMRD file and Open pipeline. It is disabled when empty. Each action presents an explicit `ISMRMRD` or `PipelineDefinition` type plus the native-form path, retains the normalized local path and source type, and invokes the corresponding MRD or PipelineDefinition open route. The persistent record format, five-entry bound, successful-open-only insertion, deduplicated promotion, restart restore, and stale-file pruning from 13.56 remain unchanged; only its File-bar presentation is superseded by this entry.
- Lifecycle and corpus: menu refresh recreates actions after a successful open; an action defers its source open through a context-bound zero-delay timer, so refreshing the menu cannot delete the sender while its `triggered` signal is dispatched. The focused widget corpus isolates both file-dialog settings keys, asserts that the submenu belongs to File and starts disabled without records, verifies MRD/Pipeline action type data and five-entry order, activates a restored PipelineDefinition action, and verifies stale-record pruning. README and the in-application Viewer guide now direct users to **File → Recent Files**.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` plus `clang-format --dry-run --Werror` over `viewer_window.cpp`, `viewer_window.hpp`, and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` of the Viewer test source; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `just link-check`; `python3 tools/checks/check_execution_plan.py --write` plus `--check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only the existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.58 P8-004 dimension-column fill correction evidence

- Scope and root cause (2026-08-28): the arrShow-style compact dimension columns visually appeared to have white right-hand areas because their container, ordinary/disabled buttons, extent label, and abbreviations had separate near-white fills; the shared value selector also painted both editable indices and read-only `:` axes white. This was a presentation-only stylesheet defect, not a coordinate, plane-selection, geometry, or raw-data issue.
- Delivered behavior: shared `ArrShowDimensionStrip` now explicitly marks a value cell as an editable fixed-index input only when it is not a display axis. Only that cell is white; its active state remains blue. The surrounding column, abbreviation, `+`, read-only `:`, `−`, and extent cells are transparent and inherit the `controls` surface background. The active dimension retains its blue border rather than receiving an entire column fill. Because both Viewer pages consume the same strip, K-space and Image now follow exactly the same fill rule.
- Regression corpus: `UsesWhiteOnlyForEditableFixedIndexDimensionCells` renders an RO/PE/Co/Avg strip into a known DPR-1 `QImage`. It verifies read-only `:` and every non-input RO cell use the controls-panel background; a passive Co index cell is white; an active Avg input is blue while its abbreviation and extent retain the panel background. The test also asserts the semantic dynamic property that distinguishes editable indices from `:` axes.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` plus `clang-format --dry-run --Werror` over `arrshow_dimension_controls.cpp`, `viewer_theme.cpp`, and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` of the Viewer test source; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `just link-check`; `python3 tools/checks/check_execution_plan.py --write` plus `--check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only the existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.59 P8-004 dimension-cell text alignment evidence

- Scope and root cause (2026-08-28): the compact dimension strip must render each abbreviation, `+`, value/`:` character, `−`, and extent as the visual centre of its own small cell. The value control already used a centre-alignment flag, while the four `QToolButton` rows relied partly on platform defaults. This was a shared presentation-only concern; it does not alter selected dimensions, X/Y plane order, coordinates, raw data, or display derivatives.
- Delivered behavior: every button row now explicitly uses Qt `ToolButtonTextOnly`, whose Qt text-label path draws at the horizontal/vertical centre. The value cell explicitly records `AlignHCenter | AlignVCenter`. The vertical layout continues to give every child the complete cell-content width, so `RO`/`PE`, `+`, `:`, `−`, and numeric extents are all centered within the same geometry rather than being affected by icon-oriented button behavior. This is app-local and shared unchanged by K-space and Image Viewer. No glyph-specific pixel offset was introduced, avoiding font/DPI-specific alignment behavior.
- Regression corpus: the existing shared-strip widget test now asserts centre alignment on both editable and read-only value cells; explicit text-only mode on abbreviation, increment, decrement, and extent buttons; and full layout-content width for every row in a dimension column. The test thereby covers the fixed-index, active, and `:` cases without relying on brittle pixel/OCR assertions.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over `arrshow_dimension_controls.cpp` and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj-viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.60 P8-004 dimension-button full-width correction evidence

- Correction to 13.59 (2026-08-28): user visual review correctly found that the initial alignment correction was incomplete. Although the value/`:` `QLineEdit` filled its column, Qt kept each `QToolButton` at its text-derived width in the vertical layout: the local diagnostic observed button widths of 24/31/37 logical pixels beside a 40-pixel value cell. Each ostensibly centered abbreviation, `+`, `−`, or extent was therefore centered only in a left-hand sliver, leaving real blank space on its right. This is a widget size-policy defect, not a font, DPI, colour, data, coordinate, or plane-selection issue.
- Delivered behavior: the shared text-cell setup now assigns every `QToolButton` an `Expanding` horizontal size policy as well as text-only drawing. In the same offscreen diagnostic, abbreviation, increment, decrement, extent, and value/`:` now each occupy `QRect(..., 40, ...)`, exactly the complete column-content width. The existing Qt centre drawing therefore operates within the actual small cell. Both K-space and Image use this one strip; no separate page-specific workaround exists.
- Regression corpus: the shared-strip widget test now asserts `QSizePolicy::Expanding` for all four button row types and asserts each of the five row geometries equals its parent `QVBoxLayout::contentsRect()` width. This directly fails if a future button returns to text-width geometry; it retains the editable/read-only value alignment checks from 13.59.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over `arrshow_dimension_controls.cpp` and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj-viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; and `git diff --check` all exited 0. The direct local offscreen widget diagnostic confirmed every button/value cell reached the same 40 logical-pixel width. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.61 P8-004 fixed equal dimension-column width evidence

- Scope and root cause (2026-08-28): user review correctly requires the compact arrShow-style controls to be equal-width **columns**, not merely equal-width rows inside each column. `ArrShowDimensionControl` previously had a fixed horizontal policy but allowed a 34–42 logical-pixel range, so Qt could choose a different outer width from each control's label-dependent size hint. `RO`, `PE`, `Co`, and `Avg` therefore had no explicit equal-width contract.
- Delivered behavior: the shared control now has one fixed outer width of 42 logical pixels. Its existing one-pixel horizontal layout margins leave the same 40-pixel content rectangle for every abbreviation, `+`, value/`:`, `−`, and extent row. This preserves the compact current appearance while making all visible dimension columns deterministic and equal; the single implementation is consumed unchanged by K-space and Image Viewer.
- Regression corpus: `UsesWhiteOnlyForEditableFixedIndexDimensionCells` now also obtains the RO, PE, Co, and Avg control parents and requires every outer column to have identical actual width, `minimumWidth == maximumWidth`, and actual width equal to that fixed limit. It retains the 13.60 child-row full-width assertions, so both column-level and cell-level geometry are protected.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over `arrshow_dimension_controls.cpp` and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj-viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `python3 tools/checks/check_execution_plan.py --write` plus `--check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.62 P8-004 current-file-bar removal evidence

- Scope and user-facing correction (2026-08-28): the user rejected the always-visible top strip that echoed the currently opened source path. It consumed a full horizontal row for a duplicate path, a **Clear Text** command that did not close or change the source, and a second **READ-ONLY** badge. Source opening, reopening, and read-only status already have clearer homes in File, File → Recent Files, the hierarchy, General Object Info, and the status area.
- Delivered behavior: `ViewerWindow` no longer constructs `viewerFileBar`, `sourceFileBar`, `clearFileBarButton`, or `fileBarReadonlyBadge`; it also removes their Enter-to-open path route and all open/close/pipeline/context synchronization. The remaining File actions and typed five-entry MRU keep their existing successful-open-only, local/read-only behavior. The tree-root tooltip, General Object Info Path, and Info panel remain the on-demand full-path sources; the status-bar offline/read-only badge remains. This removes presentation redundancy without changing source parsing, container discovery, bounds, payload ownership, export, or saved MRU/directory preferences.
- Regression corpus and contract: the workbench structure test now requires the file bar, source input, clear button, and duplicate badge to be absent while still requiring the File → Recent Files submenu and the status-bar read-only badge. The selected-source test verifies the absent path field does not affect standard source inspection. The Viewer README and in-app guide no longer advertise an Enter-to-open path bar. ADR-012, P8-004 scope, and AC-VWR-006 now make the no-current-file-bar rule explicit.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over `viewer_window.cpp`, `viewer_window.hpp`, `viewer_theme.cpp`, and `viewer_presentation_tests.cpp`; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj-viewer --parallel 4`; the fully expanded no-tree `c++ -std=c++20 -fsyntax-only` Viewer test-source command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json`; `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `python3 tools/checks/check_execution_plan.py --write` plus `--check`; and `git diff --check` all exited 0. Product linking emitted only approved-Conan ICU staging lookup warnings; direct syntax emitted only existing deprecated Qt `QMouseEvent` constructor warnings; both smokes returned `ok:true`, and export remained `artifact_kind:"visualization-derivative"`.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.63 P8-004 Object Attribute Info and native HDF5 Attribute API removal evidence

- Scope and user-facing correction (2026-08-28): ordinary standard ISMRMRD files normally have no native HDF5 object Attributes, so the always-present **Object Attribute Info** page was structurally empty and did not describe standard MRD metadata. Per user direction it is removed rather than retained as an advanced empty diagnostic. The primary inspector order is now **General Object Info | K-space | XML | Image | Pipeline**; General Object Info is the sole default page. Standard image `MetaAttributes` remain a distinct ISMRMRD per-image semantic and continue to appear only in Image details.
- Delivered code and public-contract change: `ViewerWindow` no longer creates the attribute page/table/status or attempts a per-tree-selection Attribute read; `InspectionSession` no longer wraps that operation. The pre-release public `KSpaceJet::ismrmrd::InspectionReader` API removes `InspectionObjectKind`, `InspectionObjectLocator`, `InspectionAttributeValuePreviewState`, `InspectionObjectAttributeDescriptor`, `read_object_attributes()`, and the six Attribute-specific `InspectionReadLimits` members. Their HDF5 type/shape/preview implementation and synthetic-only fixture/tests are also removed, with no compatibility alias. Standard discovery, XML/header, acquisition, image, waveform-discovery, and Image `MetaAttributes` APIs remain unchanged.
- Regression corpus and docs: Viewer presentation tests assert the five-page order, General default, and absence of the former Attribute widgets; semantic-selection tests no longer manufacture a non-standard native attribute merely to exercise the removed page. Reader tests remove only the native-Attribute corpus. The Viewer README, reader contract, P8-004 scope, ADR-012, and AC-VWR-006 now consistently state that no native HDF5 Attribute page exists.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format -i` and `clang-format --dry-run --Werror` over all changed Viewer, reader, and focused-test C++ files; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_ismrmrd --parallel 4` and `--target ksj-viewer --parallel 4`; direct no-tree `c++ -std=c++20 -fsyntax-only` checks of both `tests/unit/libs/io/kspacejet-ismrmrd/inspection_reader_tests.cpp` and the fully expanded Viewer presentation-test command recorded in 13.34; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json` and `--export-smoke --format json` (both `ok:true`, with export still `artifact_kind:"visualization-derivative"`); `python3 -B tests/apps/application_logging_contract_tests.py`; `python3 -B tests/apps/viewer_windows_gui_contract_tests.py`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; `just link-check`; `python3 tools/checks/check_execution_plan.py --write` plus `--check`; and `git diff --check` all exited 0. Direct Viewer test syntax retains only pre-existing deprecated Qt `QMouseEvent` constructor warnings.
- Status: `P8-004` transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. P0-010 still forbids an independent Linux unit/app-test CMake tree, and no Windows Release tree exists for focused Viewer/reader CTest, terminal UI/export smoke, PE GUI-subsystem inspection, Explorer no-console observation, or user visual review on a real standard MRD source. The Linux source/build/smoke/static evidence does not replace those gates.

### 13.64 P2-002 input-container selector replacement evidence

- Scope and direct replacement (2026-08-28): the user authorized the reconstruction-framework Pipeline core. The existing strict DAG/parser/controlled resolver foundation remained usable, but its fixed `input_profile.dataset_group == "dataset"` contradicted the frozen standard-first MRD-container rule. `PipelineDefinition` now owns only source-selection intent: `{"container":{"mode":"auto"}}` or `{"container":{"mode":"explicit","path":"/absolute-hdf5-container"}}`; it never carries a source-file path, route flag, Provider path, scanner fact, or physical schedule.
- Delivered public semantics: `PipelineInputContainerMode` has exactly `automatic` and `explicit_path`; an automatic profile has no path, while an explicit profile has one 1..1,024-character absolute lexical HDF5 path. The strict parser and schema reject the removed `dataset_group` field, missing/unknown mode fields, `auto` plus a path, explicit without a path, relative/root/trailing-slash/redundant-separator paths, `.`/`..` path segments, and any private `ksj_*` container segment. Auto and explicit selectors are distinct canonical PipelineDefinition identities; the checked-in fixtures prove their distinct digests.
- Integration surface: every graph fixture and graph-internal PipelineDefinition document uses the new shape. `ksj pipeline validate` JSON mirrors the parsed nested selector and text describes it as `container: auto` or `container: explicit /path`. The parse-only Viewer summary/details expose `container_mode` and optional `container_path`, never `dataset_group`. The temporary Cartesian/non-Cartesian route façades emit the valid automatic authored profile but retain their existing route-local `--dataset` configuration until P2-007 replaces those façades; the two semantics are deliberately not conflated. README/schema/architecture documentation identifies P2-007 as the runtime-owned discovery/binding owner: `auto` must bind exactly one standard raw candidate and explicit must bind the named standard raw container.
- Focused corpus: graph tests cover auto/explicit parsing, canonical reparse, distinct digest, legacy/missing/unknown selectors, invalid mode combinations and unsafe paths; a valid explicit and invalid private-container fixture are added. CLI protocol coverage verifies auto JSON/text plus explicit JSON. Viewer presentation coverage verifies automatic and explicit summary/detail forms and the absence of the legacy field. P0-010 prevents running the corresponding CTest executables, but all changed graph test sources and the Viewer test source pass direct `c++ -std=c++20 -fsyntax-only` checks using their CMake include/define contracts.
- Performed Linux validation: `tools/devenv/linux/run.sh clang-format --dry-run --Werror` over all changed C++/test paths; `tools/devenv/linux/run.sh cmake --build out/build/linux-release --target ksj_recon_graph ksj_recon_runtime ksj ksj-viewer --parallel 4`; `python3 -B tests/apps/application_json_protocol_tests.py out/build/linux-release/bin/ksj out/build/linux-release/bin/ksj-gateway out/build/linux-release/bin/ksj-recon out/build/linux-release/bin/ksj-research out/build/linux-release/bin/ksj-viewer`; clean-environment `QT_QPA_PLATFORM=offscreen out/build/linux-release/bin/ksj-viewer --ui-smoke --format json`; and direct `ksj pipeline validate` calls for the automatic fixture, explicit `/dataset_2` fixture, dynamically authored legacy `dataset_group` fixture, and rejected private `ksj_pipeline` fixture all exited as expected. The two valid reports emitted distinct digests `sha256:12af390fb429446556e34732d7e7fa831cf6fdf6fd8580c8932d79f48948bdf2` and `sha256:1a92f697f9e408bb30bcdcba9b96e5b8ab108b91db0bfbca8a0af06458ebdaad`; the legacy and private documents returned documented `validation_error`. `python3 -B tools/type_registry/generate.py --project-root . --check`, `just link-check`, `python3 -B tools/checks/check_workspace_layout.py`, JSON syntax checks, and `git diff --check` also exited 0. Direct Viewer syntax emitted only existing deprecated Qt `QMouseEvent` constructor warnings.
- Status and unblock: this item transitions `IN_PROGRESS → VERIFYING → BLOCKED`, not `ACCEPTED`. The implementation and all available product/static evidence are present, but P0-010 forbids creating/running the independent Linux unit/app-test CMake tree. The exact unblock is P0-010's authenticated Qt rebuild/check sequence, then focused graph/Viewer/application CTest. P2-007 remains the required later runtime source-adapter work; it is not silently claimed by this authored-selector replacement. P2-003/P3 remain independently blocked by P0-006's missing MachinePolicy/TargetEnvelope authority.

### 13.65 P0-006 minimal development source-configuration evidence

- Scope and decision (2026-08-28): the user rejected exposing the future runtime's full resource, scheduling, ingress, sink, calibration, NUMA, and device model as editable development configuration. The direct replacement keeps exactly two root `config/` documents, each with only the small set of human decisions needed today.
- Delivered source shape: `machine-policy.json` has `id`, one `offline-reference` profile, and limits for host memory, CPU permits, I/O slots, and a GPU enabled/disabled switch. `target-envelope.json` has `id` and ceilings for XML, one acquisition, one frame, one image, samples/acquisition, channels/acquisition, and active scans. Both schemas use `additionalProperties: false`; the old `resource_capacity`, arrival/sink, calibration, scheduler, NUMA, and device-object shapes are rejected rather than retained as compatibility fields.
- Quantity and semantic boundary: byte capacities are exact compact strings such as `32G` and `4M`, with binary `K/M/G/T`; counts remain JSON integers. The checked-in `4M` acquisition ceiling is structurally sufficient for the conservative `8192 × 32 × 8`-byte complex-sample sanity calculation. `gpu_enabled: false` is a local prohibition, not a statement that a GPU backend exists; the named 32-channel ceiling is a development-target bound, not a framework/scanner limit.
- Regression corpus and documentation: valid MachinePolicy/TargetEnvelope fixtures use the small shape. The invalid MachinePolicy fixture proves a fixed task count is rejected; the invalid TargetEnvelope fixture proves the removed `arrival_envelope` is rejected. Root README, schema README, the P0-006 registry, and the pipeline architecture now distinguish this source configuration from the still-broader internal typed planning value.
- Performed Linux validation: `tools/devenv/linux/run.sh uv run --with jsonschema python - <<'PY' ... PY` ran `Draft202012Validator` over both checked-in configurations, both valid fixtures, both invalid fixtures, rejected `32MB`, `32 G`, `1.5G`, and a bare integer byte value, rejected a synthesized `arrival_envelope`, and checked the acquisition-size sanity relation. `python3 -m json.tool` validated the two schemas and two checked-in documents. `just link-check` (77 Markdown files / 178 local links), `python3 -B tools/checks/check_execution_plan.py --write`, `python3 -B tools/checks/check_execution_plan.py --check`, `python3 -B tools/checks/check_workspace_layout.py`, and `git diff --check` all exited 0.
- Status and limitation: P0-006 remains `BLOCKED`. No JSON loader, checked quantity parser, typed-model reconciliation, artifact digest, product owner/source/review, deployment policy, performance claim, GPU capability, or Gateway authority was added. A future loader must fail for missing authority; it may not synthesize the omitted internal fields.
...
先阅读第 0.4 节，然后以第 12 节逐项状态为准；本节不是第二份状态来源。`P8-001` 至 `P8-003` 已交付 Qt 6 Widgets `ksj-viewer`、真实 QApplication/platform-plugin deployment、标准 header/acquisition/image 的有界 public inspection reader，以及共享 reader/parser 的 metadata、k-space、image、pipeline 与显示派生 export；它不执行 reconstruction、不加载 Provider、不连接 gateway，也不创建第二种 MRI artifact。`P8-004` 的最终状态仍由第 12 节决定：Pipeline authored-DAG canvas 保持 parse-only、非 ExecutionPlan；Image/K-space 现共用 `ArrShowDisplay`/`InspectionCanvas` 的 bounded QImage foundation，涵盖 Complex default、Gray(256)、`martin_phase(256)`、independent normal/phase C/W、degree/radian phase、min/max/percentile、reset/relative/absolute C/W、probe、fit/zoom/pan、image ordinal 与 K-space sparse-coordinate stepping 和 live middle-drag/reset。Image complex components 只在 callback 内按需投影；K-space 有两条可替换的 `:` axis role（默认 readout × `kspace_encode_step_1`），任意 observed semantic dimension（包括 raw coil）可成为 X/Y，其他 field 是实际 fixed sparse coordinate；不作 RSS、log、FFT、gridding 或 reconstruction。当前 checkout 无 Windows Release build/test tree，P0-010 又记录用户暂停 Linux 验证；Linux application build/offscreen smoke 已通过，但必须先在 Windows Release 重建 focused test、运行 CTest/UI/export smoke，再由用户用真实 standard image artifact 和 sibling `cart_t1.mrd` 视觉复核。用户要求以 `ismrmrdviewer` 的 group-first inspection workflow 作为功能参考。13.63 已移除 Object Attribute Info 及其唯一的 native-HDF5-attribute reader API；General Object Info 保持结构信息，而 Image `MetaAttributes` 留在 Image details；剩余真实文件视觉复核。它继续由原生 `InspectionReader` 有界发现和验证可用的标准 group，并在 Qt workbench 中提供 group → metadata/acquisitions(k-space)/images navigation、header-only acquisition index、按需 coil/trajectory inspection 和 image cine/window-level；不能使用 Python、PySide2、h5py、Matplotlib 或任意 HDF5 浏览，不能把 raw payload 无界载入 UI。`P2-002` 已因固定 `dataset` profile 与标准-first container 原则冲突而 REOPENED；P8-004 接受后，P2-002 必须改为 auto-or-explicit raw-container selection。`P2-007` 仍将最终重建入口固定为 `ksj-recon --input <scan.mrd> --pipeline <pipeline.json> --output <image.mrd>`，没有 `reconstruct` 子命令或旧命令兼容层。`P0-007` 因用户明确要求暂缓所有 GitHub CI 内容而保持 BLOCKED，禁止查询、触发或修改远端资源；`P0-010` 因用户暂停 Linux 验证而保持 BLOCKED，不删除 `kspacejet-linux-test`；`P0-006` 仍阻塞产品 case、正式 artifact、性能、Gateway policy 和资格 claim。
