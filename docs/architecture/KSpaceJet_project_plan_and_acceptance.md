...
**本轮更新时间**：2026-08-28；`P8-004` 的 arrShow-style dimension-column fill 修正已完成并记录于 13.58，13.59 的 text-only centre drawing 已由 13.60 补正为真正 full-width button-cell geometry。用户随后要求每个 dimension 的**外层列宽**也必须一致；13.61 已将 shared control 固定为共同宽度并完成本 checkout 可执行的受限验证。用户随后要求去除顶部常驻当前文件路径栏；13.62 已删除它并完成本 checkout 可执行的受限验证。用户现已要求并完成移除对普通标准 MRD 通常为空的 Object Attribute Info：General Object Info 成为默认第一页，native-HDF5-attribute reader API 也已撤销，标准 Image `MetaAttributes` 保留在 Image details；最终验证记录于 13.63。
**当前执行任务**：无。`P8-004` 已完成 13.63 的本 checkout 受限验证并回到 `BLOCKED`；解除条件是建立 Windows Release tree、运行 focused Viewer/reader CTest 和 terminal UI/export smoke、核验 Windows GUI subsystem/Explorer 无控制台，并由用户以真实标准 MRD 完成视觉复核。不得写入 raw-data sibling，且不运行 GitHub CI。
