...
| P8 | 离线可视化与检查工具 | 3 | 4 | 75% | ACCEPTED: 3 · VERIFYING: 1 |
...
**更新时间**：2026-08-26，`P0-002` 已以用户指定的 Windows Release 路径完成 bootstrap、Conan/CMake configure、四个 application build/install、installed-help/version 和 DLL closure；`P0-007` 继续因用户要求暂缓 GitHub CI 而 BLOCKED。Linux `P0-010` 已实际越过 Qt/X11 apt prerequisite，但 Qt 6.8.3 在严格 `-std=c++20` 的 libstdc++ `__int128` trait 限制处失败；远端 Qt-only GNU C++20 Conan profile 修复已合并，当前仍因缺少 sudo 认证而 BLOCKED，等待 Qt rebuild、application build 和 check。用户此前完成 `P8-004` Object Attribute Info 的真实 Viewer 视觉复核并推动 native HDF5 Attribute reader 修正；随后旧的单条 `sample × channel` 幅度投影已被替换为真正的 raw Cartesian K-space Viewer：header-only frame indexing、`kspace_encode_step_1`/centered-readout grid、RSS/单 coil、重复/空 cell 语义、非 Cartesian 拒绝及 Qt/export provenance 测试均已完成。P8 的最新代码和 focused Windows Release evidence 已在 `9acb2ee feat(viewer): add MRD workbench and Cartesian k-space view` 提交；它现在处于 VERIFYING，等待最终 `cart_t1.mrd` K-space 视觉复核。图像 `MetaAttributes` 保持 Image details 语义。`P1-002` 的 runtime-owned 标准 ISMRMRD source/sink 边界已通过 focused Windows Release 验证并 ACCEPTED；`P1-008` 的统一 ISMRMRD semantic-frame ingress 与 `P1-009` 的 development-only 2-D radial gridding/analytic DCF Provider 也均已 ACCEPTED。用户已冻结 `ksj-recon --input <scan.mrd> --pipeline <pipeline.json> --output <image.mrd>` 为唯一未来重建入口，并进一步冻结标准-first MRD container 原则：不假设 `/dataset`，任何 private `ksj_*` group 均不得成为查看或重建的前提。`P2-002` 的固定 `dataset` profile 因此 REOPENED，待 P8 的 shared discovery evidence 后改为 auto-or-explicit container selection。
**当前执行任务**：`P8-004`（VERIFYING）。用户要求取消下方独立的 `XML | K-space | Pipeline` 页面栏已完成：现在只有一个 primary tab group，顺序为 `Object Attribute Info | General Object Info | K-space | XML | Image | Pipeline`；K-space 紧随 General，XML/K-space/Image 随当前 active standard semantic object 显隐，Pipeline 只有成功打开 PipelineDefinition 时显示。独立 typed-data surface、其 Window 菜单关闭动作和固定 tab-index export dispatch 均已删除；所有 display derivative export 以当前页面身份绑定。focused widget corpus、Windows Release build、双 focused CTest、UI smoke、format/diff/link 均已通过。下一精确动作仍是用户用更新后的 Viewer 打开 `E:\KSpaceJet-ismrmrd-data\datasets\zen-2d-cartesian-2025\cart_t1.mrd`，在 tree 中选择 `[RAW] /dataset_2`（2-D T1 的 256 acquisitions，而非 `/dataset_1` 的 AdjCoilSens），双击 `Acquisitions / K-space`，核对 unified tab layout、默认 RSS、centered-readout × `kspace_encode_step_1` 的 512 × 256 plane、frame/aggregation summary、空/重复 cell 计数和 Export JSON provenance。非 Cartesian trajectory 必须继续被明确拒绝，不能伪装成 Cartesian grid。不得写入 raw-data sibling，且不运行 Linux 或任何 GitHub CI 工作。
...
| P8-004 | VERIFYING | P8-003 | 用户指定本地源码 `E:\hdfview`、安装目录 `D:\HDFView` 与 `HDFGroup/hdfview` 为 UI/操作参考；递归、有界 standard MRD container discovery、HDFView-inspired Qt shell、semantic tree、native HDF5 Attribute/General Object inspector、no-content tree omission、Pipeline root 和 content-fit semantics table 均已交付。用户指出旧 `make_kspace_presentation(ordinal)` 仅是单条 `sample × channel` 幅度投影后，该 API/页面/命令已直接替换为 `make_cartesian_kspace_presentation()`：先用 `InspectionReader::for_each_acquisition_header()` 建有界 index，再按 reference 的 `encoding_space_ref`、`kspace_encode_step_2`、average/slice/contrast/phase/repetition/set/segment/user counters 聚合同帧 acquisitions；x 为 centered readout，y 为 `kspace_encode_step_1`，RSS/单 coil 和重复/空 display cell 都有显式 RMS 语义及 JSON/CSV provenance。非 Cartesian XML/trajectory 明确拒绝，未伪装 grid。synthetic reader/presentation/widget corpus、focused Windows Release CTest、Release viewer build/UI smoke、format/diff/link/plan checks 均通过；精确记录见 13.23。2026-08-26 用户视觉反馈要求删除下方 `XML | K-space | Pipeline` 标签栏，并将这些 data view 纳入 Object/General inspector 同级主标签；基线 `9355638911b379ddf0589dd2beaa2908bb18857f` / tree `6b0eecd73f78e830ac01dc7c2dbb480333f5c7c5`。该 direct replacement 已完成：独立 typed-data surface 和 Window close action 已删除，单一 primary tab group 固定为 Attribute / General / K-space / XML / Image / Pipeline，且所有 contextual tab 与 current page-identity export 都有 widget regression coverage。Windows Release build、双 focused CTest、Qt UI smoke、format/diff/link checks 均再次通过；精确记录见 13.24。 | 用户用更新后的 Viewer 打开真实 sibling `cart_t1.mrd`，选择 `[RAW] /dataset_2` 后双击 `Acquisitions / K-space`；核对无下方重复 tab bar、K-space 紧随 General、缺失 Pipeline 不显示、默认 RSS、512 × 256 raw Cartesian plane、frame/aggregation summary 和 JSON provenance。确认后才可 ACCEPTED。 | 不得把单条 `sample × channel` 幅度图称为 K-space；不把 non-Cartesian trajectory 伪装成 Cartesian grid；不复制/链接 HDFView Java/SWT code；不新增 QML/Quick/WebEngine、Python、Matplotlib、外部图标包、reconstruction、Provider/gateway 或新的 MRI artifact；不变成 generic HDF4/HDF5/NetCDF/FITS browser/editor，不支持 generic object/attribute edit/save、URL loading 或无界 hyperslab；正式 reconstructed image 只使用标准 ISMRMRD `image_x`，不创建 `/ksj_recon`、`/ksj_debug` 或 `/ksj_meta`；不将 `ksj_*` group 设为 input prerequisite；真实 raw data 只读且不进入 KSpaceJet。 |
