...
| P8-004 | 等待解除 P0-010 并提供 Windows Release tree；随后运行 focused Viewer CTest、terminal UI/export smoke、PE GUI-subsystem/Explorer no-console 检查和真实… |
...
| AC-VWR-006 | viewer 在 1280×800 及以上提供 HDFView 式 Qt Widgets 主窗口：File/Tools/Help 菜单、常用操作工具栏、当前文件栏、左侧 hierarchy tree、右侧单一 primary tabbed inspector/contextual data views 及跨宽度的 info/status panel。`Object Attribute Info`、`General Object Info` 与 K-space（紧随 General）同级；XML/Image 只在当前 active standard semantic object 可用时显示，Pipeline 只在已打开时显示；不得保留第二个下方 data-view tab strip 或缺失内容 placeholder。树 selection 只更新 General/ISMRMRD Header/Attributes inspector 与可用 tab，不 materialize raw payload；打开 bounded data view 使用对应 tab、`Inspect` 或 `Open As`，K-space/image rendering 仍明确发生。界面可现代化，但功能层级和信息密度必须可对照 HDFView；不得改变 P8-003 的只读数据、parser、export 或依赖边界。以 widget structure/state tests、真实 QApplication UI smoke 与用户视觉复核验收。 |
...
| P8-004 | 按用户反馈将 `ksj-viewer` 重构为 HDFView-inspired、MRD-aware Qt desktop workbench：File/Tools/Help、tool/file bar、standard MRD semantic tree、单一 primary tabbed object inspector/contextual data views、typed `Inspect`/`Open As…`、页化 acquisition/coil/trajectory、`image_x` cine/window-level/zoom/probe/histogram、信息状态区和空状态；`Object Attribute Info | General Object Info | K-space | XML | Image | Pipeline` 处于同一主标签组，K-space 紧随 General，XML/K-space/Image 按当前 active standard semantic object 显隐，Pipeline 仅在已打开时显示，不保留下方重复 tab bar 或缺失内容 placeholder；打开 MRD 默认仅显示语义树加 Attribute/General inspector，不显示固定 `Dataset overview` 或 `Image series` dashboard，进入 contextual tab/显式 inspect 才执行有界 data view，raw source 的零 image series 视为正常；以 HDF Group HDFView 的 UI 与操作逻辑为参考，但独立以 C++/Qt 实现。 | apps/kspacejet-viewer、libs/io/kspacejet-ismrmrd、tests/unit/apps、tests/unit/libs/io/kspacejet-ismrmrd、docs。 | P8-003。 | AC-VWR-006、AC-VWR-007、AC-VWR-008；recursive container/header 与 widget structure/state tests、Windows Release build/UI smoke 和用户视觉复核。 |
...
**本轮更新时间**：2026-08-28；`P8-004` 的 arrShow-style dimension-column fill 修正已完成并记录于 13.58，五个小格的文本/符号双向居中修正已完成并记录于 13.59。
**当前执行任务**：无。`P8-004` 的 shared `ArrShowDimensionStrip` 现显式让缩写、`+`、`−`、extent 使用 Qt text-only centre drawing，并让值/`:` 使用水平/垂直居中；每行填满列的 layout contents。该修正同样作用于 K-space 和 Image。Linux source/build/smoke/static evidence 已完成，但 P0-010 与缺失 Windows Release tree 仍使该项 BLOCKED；不得写入 raw-data sibling，且不运行 GitHub CI。
...
| P8-004 | BLOCKED | P8-003 | 13.28–13.59 的既有 HDFView/MRD/arrShow work 保持。K-space 与 Image 现在共用 `ArrShowDimensionStrip` 的 dimension-column/plane-selection foundation：K-space 的 two blue/red `:` tags 是可替换成员，column order 才定义 X/Y；Image 则锁定 native X/Y 为第一、第二 `:`，只允许后续 varying source dimension 作固定 index。Viewer 以 user-scoped settings 记住最后成功 local MRD/Pipeline parent directory，并保存同一类 source 的最多五项 typed MRU：仅成功打开入列、去重置顶、重启恢复，失效 local entry 在启动时清理。13.57 将 MRU access 改为 **File → Recent Files** dynamic submenu；13.58 使 dimension column 的非输入区域与 controls panel 一致，只有 editable index input 保持白底；13.59 显式统一所有五个 cell 的 centre drawing/alignment。 | 等待解除 P0-010 并提供 Windows Release tree；随后运行 focused Viewer CTest、terminal UI/export smoke、PE GUI-subsystem/Explorer no-console 检查和真实 standard source 的用户视觉复核。 | P0-010 仍禁止建立/运行 Linux unit/app-test tree，且当前无 Windows Release tree；本轮 Linux source/build/smoke/static contract 不能替代 Windows PE subsystem、terminal automation、Explorer 实测或真实 source 用户视觉复核。显示仍限 app-local bounded QImage derivative；不得引入 MATLAB runtime/source/assets、workspace mutation、destructive FFT/crop/ROI、Python/QML/Quick/WebEngine、reconstruction、Provider/gateway 或 MRI artifact。 |
