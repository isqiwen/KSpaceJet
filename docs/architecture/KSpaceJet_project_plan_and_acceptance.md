...
**更新时间**：2026-08-25，`P0-002` 已以用户指定的 Windows Release 路径完成 bootstrap、Conan/CMake configure、四个 application build/install、installed-help/version 和 DLL closure；`P0-007` 继续因用户要求暂缓 GitHub CI 而 BLOCKED。用户先授权修复 Linux `just prepare-release` 在 Qt/X11 宿主开发包检查处失败的问题，并进一步要求合并 `.vscode/tasks.json` 中可由同名跨平台 `just` recipe 驱动的 Linux/Windows task；`P0-010` 已完成该静态收敛并再次 BLOCKED，等待当前 Linux host 的 sudo 认证以实际回归 prepare/check。它保留 Conan 的只读系统包检查，改由 project-owned Linux bootstrap 显式处理受控 apt 依赖。用户已完成 `P8-004` 的真实 Viewer 视觉复核，并指出 `Object Attribute Info` 错将 ISMRMRD image `MetaAttributes` 当作 HDF5 object attributes；修复后的有界 native HDF5 Attribute reader、Viewer Inspector、synthetic corpus 和 Windows focused validation 已完成。随后按用户要求，旧的单条 `sample × channel` 幅度投影已被替换为真正的 raw Cartesian K-space Viewer：header-only frame indexing、`kspace_encode_step_1`/centered-readout grid、RSS/单 coil、重复/空 cell 语义、非 Cartesian 拒绝和对应 Qt/导出 provenance 测试均已完成，本项现处于 VERIFYING，等待用户在真实 `cart_t1.mrd` 上的最终视觉复核。图像 `MetaAttributes` 保持 Image details 语义。`P1-002` 的 runtime-owned 标准 ISMRMRD source/sink 边界已通过 focused Windows Release 验证并 ACCEPTED；`P1-008` 的统一 ISMRMRD semantic-frame ingress 与 `P1-009` 的 development-only 2-D radial gridding/analytic DCF Provider 也均已 ACCEPTED。用户已冻结 `ksj-recon --input <scan.mrd> --pipeline <pipeline.json> --output <image.mrd>` 为唯一未来重建入口，并进一步冻结标准-first MRD container 原则：不假设 `/dataset`，任何 private `ksj_*` group 均不得成为查看或重建的前提。`P2-002` 的固定 `dataset` profile 因此 REOPENED，待 P8 的 shared discovery evidence 后改为 auto-or-explicit container selection。
**当前执行任务**：`P8-004`（VERIFYING）。基线 commit/tree：`bd3107a feat: advance ISMRMRD reconstruction and viewer foundation` / 当前未提交 P8-004 worktree；真正的 Cartesian K-space Viewer 与 focused Windows Release evidence 已具备。下一精确动作是用户用更新后的 Viewer 打开 `E:\KSpaceJet-ismrmrd-data\datasets\zen-2d-cartesian-2025\cart_t1.mrd`，在 tree 中选择 `[RAW] /dataset_2`（2-D T1 的 256 acquisitions，而非 `/dataset_1` 的 AdjCoilSens），再双击 `Acquisitions / K-space`；核对默认 RSS、centered-readout × `kspace_encode_step_1` 的 512 × 256 plane、frame/aggregation summary、空/重复 cell 计数和 Export JSON provenance。非 Cartesian trajectory 必须继续被明确拒绝，不能伪装成 Cartesian grid。不得写入 raw-data sibling，且不运行 Linux 或任何 GitHub CI 工作。
...
| P0-010 | BLOCKED | P0-003 | 初始 Linux prerequisite 修复基线为 `bd3107a2f48e0b4fc402eb36abaf3f9b5941b9da` / tree `6bf2321797890d548fba118494deb8dc9ef06b04`，其交付已提交为 `f09d27e277eeaf8274a46d7caf9809f444665808`。本次 task consolidation 基线为 `f09d27e277eeaf8274a46d7caf9809f444665808` / tree `ca444c01a0088fb849a8e894a16a1268994b22be`；用户于 2026-08-24 明确要求合并所有可由同名跨平台 `just` recipe 驱动的 VS Code task。现已将 12 个后置 task 收敛为 6 个 Debug/Release 通用 task，保留 bootstrap；静态 mapping、JSON、justfile format/list、link/workspace/format/diff checks 均通过，完整记录见 13.12。 | 在当前 Linux terminal 先执行 `sudo -v`（不要提供或记录密码），再执行 `just prepare-release` 和 `just check`；将实际结果记录后重新打开本项并决定 VERIFYING/ACCEPTED 或继续 BLOCKED。 | `sudo -n true` 再次输出 `sudo: a password is required`，当前会话不能执行实际 apt 安装、Conan system-requirements check 或完整 `prepare/check`；本次仅调整 VS Code task 表面与文档，不改变 Qt feature、Conan dependency graph、全局 Conan 配置、GitHub CI、raw-data sibling 或任何产品/发布资格 claim。 |
