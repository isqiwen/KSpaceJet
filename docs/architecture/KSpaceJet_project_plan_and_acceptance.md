...
| P0-007 | IN_PROGRESS | P0-002 | 2026-08-22：已在隔离 branch `codex/p0-007-release-ci` 创建并推送 Release-only workflow，PR `#1` 已两次产生实际 `linux-release-quality` 与 `windows-release-build-install` contexts。第 1 次暴露 hosted runner 的 `winget`/旧 `just` 差异；第 2 次暴露 `just 1.21` 的条件 setting 解析限制与 Windows PowerShell 缺少 `Get-FileHash`。已完成兼容修复，本地 Windows `just check`、PowerShell parser/verify、YAML 解析均通过；完整逐次证据见第 13.13 节。 | 提交并推送 `justfile` 兼容调整和 .NET SHA-256 fallback，等待 PR `#1` 的第三次两个 Release check；只有均成功后才按实际 context 配置并读取复验 `main` classic protection。 | 不得将 Debug、clean-install qualification、benchmark、Provider load 或性能门禁混入本 P0 workflow；任何第三次 CI 失败先记录实际日志并在本项范围内修复，未成功前不得配置 required checks。 |
